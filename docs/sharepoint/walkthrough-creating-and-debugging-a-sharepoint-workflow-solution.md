...
## <a name="add-properties-to-the-sharepoint-shared-documents-library"></a>将属性添加到 SharePoint 共享文档库
 为了跟踪**共享文档**库中文档的审阅状态，我们将为 SharePoint 站点上的共享文档创建三个新属性： `Status`、`Assignee`和 `Review Comments`。 我们在**共享文档**库中定义这些属性。

#### <a name="to-add-properties-to-the-sharepoint-shared-documents-library"></a>向 SharePoint 共享文档库添加属性

1. 在 Web 浏览器中打开 SharePoint 站点，例如 http://\<系统名称 >/SitePages/Home.aspx。
...
3. 展开 "**视觉对象C#**  " 或 " **Visual Basic**" 下的 " **SharePoint** " 节点，然后选择 " **2010** " 节点。
...
1. 在*Workflow1.cs*或*workflow1.xaml*中，将以下字段添加到 `Workflow1` 类的顶部。 此字段在活动中用于确定工作流是否已完成。
...
2. 将以下方法添加到 `Workflow1` 类。 此方法检查文档列表的 `Document Status` 属性的值，以确定是否已查看文档。 如果 `Document Status` 属性设置为 `Review Complete`，则 `checkStatus` 方法会将 `workflowPending` 字段设置为**false** ，以指示工作流已准备就绪，可以完成操作。
...
3. 将以下代码添加到 `onWorkflowActivated`，并 `onWorkflowItemChanged` 方法来调用 `checkStatus` 方法。 工作流启动时，`onWorkflowActivated` 方法会调用 `checkStatus` 方法来确定是否已查看文档。 如果尚未查看，工作流将继续。 保存文档后，`onWorkflowItemChanged` 方法将再次调用 `checkStatus` 方法来确定文档是否已查看。 当 `workflowPending` 字段设置为**true**时，工作流将继续运行。
...
4. 将以下代码添加到 `isWorkflowPending` 方法，以检查 `workflowPending` 属性的状态。 每次保存文档时， **whileActivity1**活动将调用 `isWorkflowPending` 方法。 此方法检查 <xref:System.Workflow.Activities.ConditionalEventArgs> 对象的 <xref:System.Workflow.Activities.ConditionalEventArgs.Result%2A> 属性，以确定**WhileActivity1**活动是否应继续或完成。 如果将属性设置为**true**，则活动将继续。 否则，该活动将完成并完成工作流。
...
## <a name="test-the-sharepoint-workflow-template"></a>测试 SharePoint 工作流模板
 启动调试器时，[!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] 会将工作流模板部署到 SharePoint 服务器，并将工作流与 "**共享文档**" 列表相关联。 若要测试工作流，请从**共享文档**列表的文档中启动工作流的实例。
...
6. 在 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)]中，验证调试器在 `onWorkflowActivated` 方法旁边的断点处停止。
