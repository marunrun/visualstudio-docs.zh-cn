...
3. 在 "对话框" 树视图中，展开 " **Visual c #** " 或 " **Visual Basic** " （具体取决于您的项目语言），展开 " **SharePoint** " 节点，然后选择 " **2010** " 节点。
...
1. 在关联窗体（ExpenseReportAssocForm）中，找到具有的 `asp:Content` 元素 `ID="Main"` 。
...
1. 在启动窗体（ExpenseReportInitForm）中，找到包含的 `asp:Content` 元素 `ID="Main"` 。

2. 在此 content 元素中的第一行之后，添加以下代码以创建显示在关联窗体中输入的支出批准限制（*AutoApproveLimit*）的标签和文本框，并添加另一个标签和文本框以提示输入支出总计（*ExpenseTotal*）：
