...
### <a name="external-editors"></a>外部编辑器
 ...
 <xref:Microsoft.VisualStudio.Shell.Interop.IVsProject3.IsDocumentInProject%2A> 方法使用 <xref:Microsoft.VisualStudio.Shell.Interop.VSDOCUMENTPRIORITY> 枚举设置文档优先级。 如果指定了 `DP_External` 值，则外部编辑器可以打开该文件。
...
- 编辑器是否支持多个视图？ 如果希望编辑器的视图与默认编辑器同时可见，则必须支持多个视图。
   ...
   如果编辑器支持多个视图，是否打算为文档数据对象使用 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] 核心编辑器文本缓冲区实现（<xref:Microsoft.VisualStudio.TextManager.Interop.VsTextBuffer> 对象）？ 也就是说，你是否想要使用 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] 核心编辑器并行支持编辑器视图？ 执行此操作的功能是窗体设计器的基础。

- 如果需要托管外部编辑器，编辑器是否可以嵌入到 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]中？
...
说明如何创建与 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]的其余部分匹配的 UI 元素。
