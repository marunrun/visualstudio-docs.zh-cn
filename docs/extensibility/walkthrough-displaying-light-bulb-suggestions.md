...
轻型电灯泡是 Visual Studio 编辑器中的图标，可展开以显示一组操作，例如对内置代码分析器或代码重构标识的问题的修补程序。
 ...
 在视觉对象C#和 Visual Basic 编辑器中，你还可以使用 .NET Compiler Platform （"Roslyn"）来编写和打包自己的代码分析器，其中包含自动电灯泡显示的操作。 有关详细信息，请参见:
...
- [如何：编写 Visual Basic 诊断和代码修补程序](https://github.com/dotnet/roslyn/wiki/How-To-Write-a-Visual-Basic-Analyzer-and-Code-Fix)
  ...
  其他语言（例如C++ ）也为某些快速操作提供轻电灯泡，例如，创建该函数的存根实现的建议。

  灯泡的外观如下所示。 在 Visual Basic 或视觉对象C#中，当变量名称无效时，将在其下显示红色波形曲线。 如果将鼠标悬停在无效的标识符上，则光标附近会出现一个灯泡。
...
1. 创建C# VSIX 项目。 （在 "**新建项目**" 对话框中，依次选择 "**视觉对象C#** "、"扩展性"、" **VSIX 项目**"。）将解决方案命名为 `LightBulbTest`。
...
1. 在*LightBulbTest.cs*类文件中，删除 LightBulbTest 类。 添加一个名为**TestSuggestedActionsSourceProvider**的类，该类实现 <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedActionsSourceProvider>。 使用 "**测试建议的操作**" 和 "文本" <xref:Microsoft.VisualStudio.Utilities.ContentTypeAttribute> 的名称将其导出。
...
1. 添加一个实现 <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedActionsSource> 的类**TestSuggestedActionsSource** 。
...
4. 添加返回当前光标下的单词的私有方法。 以下方法将查看光标的当前位置，并向文本结构导航器询问单词范围。 如果光标位于某个单词上，则将在 out 参数中返回 <xref:Microsoft.VisualStudio.Text.Operations.TextExtent>;否则，`out` 参数 `null` 并且该方法返回 `false`。
...
6. 实现 <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedActionsSource.GetSuggestedActions%2A> 方法，该方法返回包含不同 <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedAction> 对象的 <xref:Microsoft.VisualStudio.Language.Intellisense.SuggestedActionSet> 对象的数组。 此方法在展开灯泡时进行调用。

    > [!WARNING]
    > 应确保 `HasSuggestedActionsAsync()` 和 `GetSuggestedActions()` 的实现一致;也就是说，如果 `HasSuggestedActionsAsync()` 返回 `true`，则 `GetSuggestedActions()` 应具有要显示的一些操作。 在许多情况下，`HasSuggestedActionsAsync()` 在 `GetSuggestedActions()` 之前调用，但这并不总是如此。 例如，如果用户通过按（**CTRL +** .）调用灯泡操作，则只调用 `GetSuggestedActions()`。
...
8. 若要完成实现，请添加 `Dispose()` 和 `TryGetTelemetryId()` 方法的实现。 你不想要执行遥测操作，因此只需返回 `false`，并将 GUID 设置为 `Empty`。
...
1. 在项目中，添加对*VisualStudio*的引用并将 "**复制本地**" 设置为 `False`。
...
11. 不要忘记执行相同的操作，`LowerCaseSuggestedAction` 将显示文本更改为 "将 ' {0} ' 转换为小写"，并将调用 <xref:System.String.ToUpper%2A> 为 <xref:System.String.ToLower%2A>。
...
5. 单击灯泡时，应显示两个建议的操作，以及所选操作的预览。
     ...
     ![测试灯泡，展开](../extensibility/media/testlightbulbexpanded.gif "TestLIghtBulbExpanded")
