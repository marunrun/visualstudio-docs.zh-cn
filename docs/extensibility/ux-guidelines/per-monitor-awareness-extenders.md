...
使用 CreateWindow ( # A1 或 CreateWindowEx ( # A3 创建 windows 时，一种常见的模式是在坐标 (0，0)  () 的主要显示位置，并将其移到最终位置。 但这样做可能会导致窗口触发 DPI 更改的消息或事件，这可能重新触发其他 UI 消息或事件，并最终导致意外行为或呈现。
...
即使在由于 Windows 默认行为) 的情况下，成功地将 windows 的 DpiAwarenessContexts 与不同的 (，用户仍可能会发现缩放问题，因为不同的 DpiAwarenessContexts 规模不同。 因此，用户可能会在 UI 上看到对齐/模糊文本或图像问题。

解决方案是为应用程序中的所有窗口和控件设置正确的 DpiAwarenessContext 范围。

### <a name="top-level-mixed-mode-tlmm-dialogs"></a>顶级混合模式 (TLMM) 对话框
创建顶级窗口（如模式对话框）时，请务必确保线程在窗口 (之前处于正确状态，并) 创建它的句柄。 可以在托管的本机或 DpiAwareness 中的 EnterDpiScope 帮助程序中使用 CDpiScope 帮助器将线程置于系统感知。  (TLMM 通常应在非 WPF 对话框/windows 上使用。 ) 
...
大多数作为主消息循环或事件链一部分发生的 UI 计算工作都应该已经在正确的 DPI 感知上下文中运行。 但是，如果在这些主工作流之外完成了坐标或大小调整计算 (例如在空闲时间任务期间，或在 UI 线程中执行），则当前的 DPI 感知上下文可能会错误地导致 UI misplacement 或错误大小调整问题。 为 UI 工作将线程置于正确状态通常会解决此问题。
 
#### <a name="opt-out-of-clmm"></a>选择退出 CLMM
如果非 WPF 工具窗口正在迁移为完全支持 PMA，则需要退出 CLMM。 为此，需要实现新接口： IVsDpiAware。
...
对于托管语言，实现此接口的最佳位置是在派生自 *ToolWindowPane*的同一个类中。 对于 c + +，实现此接口的最佳位置是从 vsshell 实现 *IVsWindowPane* 的同一个类。

接口上的 Mode 属性返回的值是一个 __VSDPIMODE (并强制转换为托管) 中的 uint：
