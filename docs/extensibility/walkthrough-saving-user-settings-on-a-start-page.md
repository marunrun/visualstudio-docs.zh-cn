...
此演练中实例化的设置存储是 <xref:Microsoft.VisualStudio.Shell.Interop.IVsWritableSettingsStore> 接口的实例，该实例在调用时读取并写入以下注册表位置： **HKCU\Software\Microsoft\VisualStudio\14.0 \\ \<CollectionName>**

在 Visual Studio 的实验实例中运行时，设置将读取和写入到**HKCU\Software\Microsoft\VisualStudio\14.0Exp \\ \<CollectionName >。**
...
5. 在 "设计" 窗格中，单击控件的主区域，然后按 "**删除**"。
     ...
     此步骤将删除 <xref:System.Windows.Controls.Border> 元素和其中的所有内容，并且仅保留顶层 <xref:System.Windows.Controls.Grid> 元素。

6. 从 "**工具箱**" 中，将 "<xref:System.Windows.Controls.StackPanel>" 控件拖动到网格。

7. 现在，将 <xref:System.Windows.Controls.TextBlock>、<xref:System.Windows.Controls.TextBox> 和按钮拖到 <xref:System.Windows.Controls.StackPanel> 中。

8. 为 <xref:System.Windows.Controls.TextBox> 添加**x：Name**特性，并为 <xref:System.Windows.Controls.Button> 提供 `Click` 事件，如下面的示例中所示。
...
1. 在 "XAML" 窗格中，右键单击 <xref:System.Windows.Controls.Button> 元素的 `Click` 属性，然后单击 "**导航到事件处理程序**"。

     此步骤将打开*MyControl.xaml.cs*，并为 `Button_Click` 事件创建存根处理程序。
...
3. 添加私有 `SettingsStore` 属性，如下面的示例中所示。
    ...
    ```
     ...
     此属性首先从用户控件的 <xref:System.Windows.FrameworkElement.DataContext%2A> 中获取对 <xref:EnvDTE80.DTE2> 接口的引用，该接口包含自动化对象模型，然后使用 DTE 获取 <xref:Microsoft.VisualStudio.Shell.Interop.IVsSettingsManager> 接口的实例。 然后，它将使用该实例来返回当前用户设置。

4. 按如下所示填写 `Button_Click` 事件。
...
您可以修改此用户控件，通过使用不同事件处理程序中的不同值来保存和检索任意数量的自定义设置，以获取和设置 `SettingsStore` 属性。 只要对 <xref:Microsoft.VisualStudio.Shell.Interop.IVsWritableSettingsStore.SetString%2A> 的每个调用使用不同的 `propertyName` 参数，值就不会在注册表中相互覆盖。
