...
|[SccSetOption](../extensibility/sccsetoption-function.md)|用于设置各种选项的泛型函数。 每个选项都以 `SCC_OPT_xxx` 开头，并具有其自己定义的值集。|
...
|[SccGetEvents](../extensibility/sccgetevents-function.md)|检查调用方所请求的文件的状态（通过 `SccQueryInfo`）。|
...
|[SccPopulateList](../extensibility/sccpopulatelist-function.md)|检查文件列表的当前状态。 此外，如果文件与 `nCommand` 的条件不匹配，则使用 `pfnPopulate` 函数通知调用方。|
...
## <a name="functions-that-support-additional-capability-version-12-of-the-source-control-plug-in-api"></a>支持其他功能的函数（源代码管理插件 API 版本1.2）
...
## <a name="functions-that-support-advanced-capability-version-13-of-the-source-control-plug-in-api"></a>支持高级功能的函数（源代码管理插件 API 版本1.3）
...
## <a name="requirements"></a>要求
 ...
 （在环境 SDK common 包含文件夹中提供，默认情况下为 *[drive]* \Program Files\VSIP 8.0 \ EnvSDK\common\inc; 也在包含 MSSCCI 示例的 VSIP 文件夹中提供 *[drive]* \PROGRAM Files\VSIP 8.0 \ MSSCCI）。
