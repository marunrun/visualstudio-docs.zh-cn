...
2. 在 `<Symbols>` 部分中，查找 `name` 属性 `guidFirstToolWindowPackageCmdSet` 的 `<GuidSymbol>` 节点。 将以下两个 `<IDSymbol>` 元素添加到此节点中 `<IDSymbol>` 元素的列表中，以定义工具栏和工具栏组。
...
3. 在 "`<Buttons>`" 部分的正上方，创建一个类似于下面的 `<Menus>` 部分：
    ...
    有几种不同类型的菜单。 此菜单是工具窗口中的一个工具栏，由其 `type` 属性定义。 @No__t_0 和 `id` 设置构成了工具栏的完全限定 ID。 通常，菜单的 `<Parent>` 为包含组。 但是，工具栏定义为其自身的父级。 因此，`<Menu>` 和 `<Parent>` 元素使用相同的标识符。 @No__t_0 特性只是 "0"。

4. 工具栏类似于菜单。 例如，正如菜单可能包含命令组，工具栏也可能具有组。 （在菜单上，命令组由水平线分隔。 在工具栏上，组不是由视觉分隔符分隔的。）

    添加包含 `<Group>` 元素的 `<Groups>` 部分。 这会定义在 `<Symbols>` 部分中声明其 ID 的组。 将 `<Groups>` 部分添加到 `<Menus>` 节的紧后面。
...
2. 在 `<Buttons>` 部分中添加 Button 元素。 此元素将显示在工具窗口中的工具栏上，并带有**搜索**（放大镜）图标。
...
1. 在*FirstToolWindowPackage.cs*中，查找 `FirstToolWindowPackage` 类的 <xref:Microsoft.VisualStudio.Shell.ProvideToolWindowAttribute> 属性，该类将 FirstToolWindow 类型传递给构造函数。 若要指定默认位置，必须在下面的示例中向构造函数添加更多参数。
    ...
    第一个命名参数 `Style`，其值为 `Tabbed`，这意味着该窗口将是现有窗口中的一个选项卡。 停靠位置由 `Window` 参数指定，n 这种情况下，**解决方案资源管理器**的 GUID。
