...
或者，如果快捷菜单不会访问 Visual Studio 功能，则可以使用用户控件中的 XAML 元素的 <xref:System.Windows.FrameworkElement.ContextMenu%2A> 属性。 有关详细信息，请参阅[ContextMenu](/dotnet/framework/wpf/controls/contextmenu)。
...
1. 创建一个名为 `TWShortcutMenu` 的 VSIX 项目，并向其添加一个名为**快捷菜单**的工具窗口模板。 有关创建工具窗口的详细信息，请参阅[使用工具窗口创建扩展](../extensibility/creating-an-extension-with-a-tool-window.md)。
...
6. 在*ShortcutMenuControl*中，将 <xref:System.Windows.UIElement.MouseRightButtonDown> 事件添加到顶层 <xref:System.Windows.Controls.UserControl> 元素。 XAML 文件现在应如下所示：
...
9. 按如下所示实现 `MyToolWindowMouseRightButtonDown` 事件。
    ...
    这会创建快捷菜单的 <xref:System.ComponentModel.Design.CommandID> 对象、标识鼠标单击的位置，并通过使用 <xref:Microsoft.VisualStudio.Shell.OleMenuCommandService.ShowContextMenu%2A> 方法打开该位置中的快捷菜单。
