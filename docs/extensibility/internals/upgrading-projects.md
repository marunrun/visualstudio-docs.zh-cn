...
ms.locfileid: "79301625"
---
# <a name="upgrading-projects"></a>升级项目

对项目模型的更改从 Visual Studio 的一个版本更改为下一个版本可能需要升级项目和解决方案，以便它们可以在较新版本上运行。 提供[!INCLUDE[vsipsdk](../../extensibility/includes/vsipsdk_md.md)]可用于在您自己的项目中实现升级支持的接口。

## <a name="upgrade-strategies"></a>升级策略

要支持升级，项目系统实施必须定义并实施升级策略。 在确定策略时，您可以选择支持并行 （SxS） 备份、复制备份或两者。

- SxS 备份意味着项目仅复制需要升级的文件，添加适当的文件名后缀，例如".old"。

- 复制备份意味着项目将所有项目项复制到用户提供的备份位置。 然后升级原始项目位置的相关文件。

## <a name="how-upgrade-works"></a>升级的工作原理

在较新版本的 Visual Studio 中创建的解决方案时，IDE 会检查解决方案文件以确定是否需要升级。 如果需要升级，将自动启动**升级向导**以引导用户完成升级过程。

当解决方案需要升级时，它会查询每个项目工厂的升级策略。 该策略确定项目工厂是支持复制备份还是 SxS 备份。 信息将发送到**升级向导**，该向导收集备份所需的信息，并将适用的选项呈现给用户。

### <a name="multi-project-solutions"></a>多项目解决方案

如果解决方案包含多个项目，并且升级策略不同，例如当仅支持 SxS 备份C++项目和仅支持复制备份的 Web 项目时，项目工厂必须协商升级策略。

解决方案查询每个项目工厂<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgradeViaFactory>。 然后，它会<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgradeViaFactory.UpgradeProject_CheckOnly%2A>调用以查看全局项目文件是否需要升级并确定支持的升级策略。 然后调用**升级向导**。

用户完成向导后，<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgradeViaFactory.UpgradeProject%2A>在每个项目工厂调用执行实际升级。 为了便于备份，IVsProjectUpgradeViaFactory 方法提供服务<xref:Microsoft.VisualStudio.Shell.Interop.SVsUpgradeLogger>来记录升级过程的详细信息。 无法缓存此服务。

更新所有相关全局文件后，每个项目工厂可以选择实例化项目。 项目实施必须支持<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgrade>。 然后<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgrade.UpgradeProject%2A>调用 该方法以升级所有相关项目项。

> [!NOTE]
> 该方法<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgradeViaFactory.UpgradeProject%2A>不提供 SV 升级记录器服务。 此服务可以通过调用<xref:Microsoft.VisualStudio.OLE.Interop.IServiceProvider.QueryService%2A>获得。

## <a name="best-practices"></a>最佳实践

使用该服务<xref:Microsoft.VisualStudio.Shell.Interop.SVsQueryEditQuerySave>检查是否可以在编辑文件之前对其进行编辑，并且可以在保存文件之前保存它。 这将有助于备份和升级实现在源代码管理下处理项目文件、权限不足的文件等。

在<xref:Microsoft.VisualStudio.Shell.Interop.SVsUpgradeLogger>备份和升级的所有阶段使用该服务，提供有关升级过程成功或失败的信息。

有关备份和升级项目的详细信息，请参阅 vsshell2.idl 中的 IVProjectUpgrade 注释。
...
仍支持使用 <xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgrade> 接口的旧机制，但该机制将会把项目系统作为项目打开的一部分进行概念上的升级。 因此<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgrade>，即使调用或实现了该接口，Visual <xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgradeViaFactory> Studio 环境也会调用该接口。 此方法允许你使用 <xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgradeViaFactory> 来实现复制、仅规划升级的部分并通过 <xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgrade> 接口委派要就地（可能在新位置）完成的剩余工作。

有关 的<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgrade>实现示例，请参阅[VSSDK 示例](https://github.com/Microsoft/VSSDK-Extensibility-Samples)。
...
1. 用户尝试打开一个项目时，<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgrade.UpgradeProject%2A> 方法将由环境在项目打开后、在项目上执行任何其他用户操作前调用。 如果已提示用户升级解决方案，则 <xref:Microsoft.VisualStudio.Shell.Interop.__VSUPGRADEPROJFLAGS.UPF_SILENTMIGRATE> 标志将传入 `grfUpgradeFlags` 参数。 如果用户直接打开项目（例如使用 **"添加现有项目"** 命令），则不会传递<xref:Microsoft.VisualStudio.Shell.Interop.__VSUPGRADEPROJFLAGS.UPF_SILENTMIGRATE>标志，并且项目需要提示用户升级。
...
3. 如果项目需要将项目类型升级到新版本，则它必须确定是否通过调用 <xref:Microsoft.VisualStudio.Shell.Interop.IVsQueryEditQuerySave2.QueryEditFiles%2A> 方法和为 `rgfQueryEdit` 参数传入 <xref:Microsoft.VisualStudio.Shell.Interop.tagVSQueryEditFlags> 值来修改项目文件。 该项目接下来需要执行以下操作：
    ...
    - 如果 `pfEditCanceled` 参数中返回的 `VSQueryEditResult` 值是 <xref:Microsoft.VisualStudio.Shell.Interop.tagVSQueryEditResult.QER_EditNotOK> 且 `VSQueryEditResult` 值设置了 <xref:Microsoft.VisualStudio.Shell.Interop.tagVSQueryEditResultFlags.QER_ReadOnlyNotUnderScc> 位，则 <xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgrade.UpgradeProject%2A> 必须返回失败，因为用户应该自己解决权限问题。 该项目接下来应执行以下操作：
         ...
         通过调用<xref:Microsoft.VisualStudio.Shell.Interop.IVsUIShell.ReportErrorInfo%2A>并将<xref:Microsoft.VisualStudio.Shell.Interop.VSErrorCodes.VS_E_PROJECTMIGRATIONFAILED>错误代码返回给<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgrade>用户，向用户报告错误。
...
- 如果你不处理自己的项目重载，则需为 <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchy.GetProperty%2A> (<xref:Microsoft.VisualStudio.Shell.Interop.__VSHPROPID.VSHPROPID_HandlesOwnReload>) 返回 `false`。 在这种情况下，在返回（QEF_ForceEdit_NoPrompting、QEF_DisallowInMemoryEdits）之前<xref:Microsoft.VisualStudio.Shell.Interop.IVsQueryEditQuerySave2.QueryEditFiles%2A>，环境将创建项目的另一个新实例，例如 Project2，如下所示：
...
如果在未实现的项目系统内添加或管理项目，则可能需要参与项目升级过程。 Crystal 报表是可添加到项目系统的项的示例。

通常，项目项实施者希望利用已完全实例化并升级的项目，因为他们需要知道项目引用是什么以及哪些其他项目属性可以做出升级决策。

### <a name="to-get-the-project-upgrade-notification"></a>获取项目升级通知

1. 在<xref:Microsoft.VisualStudio.Shell.Interop.UIContextGuids80.SolutionOrProjectUpgrading>项目项实现中设置标志（在 vsshell80.idl 中定义）。 当 Visual Studio 外壳确定项目系统正在升级时，这将导致项目项 VSPackage 自动加载。

2. 通过<xref:Microsoft.VisualStudio.Shell.Interop.IVsSolutionEventsProjectUpgrade><xref:Microsoft.VisualStudio.Shell.Interop.IVsSolution2.AdviseSolutionEvents%2A>该方法通知接口。

3. 在<xref:Microsoft.VisualStudio.Shell.Interop.IVsSolutionEventsProjectUpgrade>项目系统实现完成其升级操作并创建新的升级项目后，将触发该接口。 根据方案<xref:Microsoft.VisualStudio.Shell.Interop.IVsSolutionEventsProjectUpgrade>，接口在<xref:Microsoft.VisualStudio.Shell.Interop.IVsSolutionEvents3.OnAfterOpenSolution%2A>后触发 ， 或<xref:Microsoft.VisualStudio.Shell.Interop.IVsSolutionEvents3.OnAfterOpenProject%2A><xref:Microsoft.VisualStudio.Shell.Interop.IVsSolutionEvents3.OnAfterLoadProject%2A>方法。

### <a name="to-upgrade-the-project-item-files"></a>升级项目项目文件

1. 您必须在项目项实现中仔细管理文件备份过程。 这尤其适用于并排备份，其中`fUpgradeFlag`<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgradeViaFactory.UpgradeProject%2A>方法的参数设置为<xref:Microsoft.VisualStudio.Shell.Interop.__VSPPROJECTUPGRADEVIAFACTORYFLAGS.PUVFF_SXSBACKUP>，其中已备份的文件沿指定为".old"的侧文件放置。 备份的文件早于升级项目时的系统时间，可以指定为过时文件。 此外，除非采取特定步骤来防止这种情况，否则它们可能会被覆盖。

2. 当项目项目收到项目升级通知时，**仍显示可视化工作室转换向导**。 因此，应使用<xref:Microsoft.VisualStudio.Shell.Interop.IVsUpgradeLogger>接口的方法向向导 UI 提供升级消息。
