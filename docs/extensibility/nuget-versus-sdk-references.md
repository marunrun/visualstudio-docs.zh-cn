...
---
# <a name="nuget-versus-sdk-as-a-project-reference"></a>NuGet 与 SDK 作为项目参考

本文旨在帮助开发人员选择将其软件打包为 NuGet 包还是作为软件开发工具包 （SDK）。 具体来说，它讨论了在 Visual Studio 项目中引用它们时两者之间的差异。

- [NuGet](/nuget)是一个开源包管理系统，它简化了将库合并到项目中的过程。 对于 .NET（包括 .NET Core），NuGet 是 Microsoft 支持的共享代码的机制。 NuGet 定义了如何创建、托管和使用 .NET 的包，并为每个角色提供工具。 在 Visual Studio 中，您可以使用[包管理器](/nuget/consume-packages/install-use-packages-visual-studio)用户界面将 NuGet 包添加到项目中。

- [SDK](../extensibility/creating-a-software-development-kit.md)是 Visual Studio 视为单个引用项的文件的集合。 "可视化工作室"中的"参考管理器"对话框在选择 **"添加参考**"时列出与当前项目相关的所有 SDK。 将 SDK 添加到项目时，可以通过 IntelliSense、工具箱窗口、设计器、对象浏览器、MSBuild、部署、调试和打包访问该 SDK 的所有内容。

## <a name="which-mechanism-should-i-use"></a>应使用哪种机制？

下表可帮助你比较 SDK 与 NuGet 的引用功能。

| Feature | SDK 支持 | SDK 说明 | NuGet 支持 | NuGet 说明 |
| - | - | - |---------------| - |
...
| MSBuild 自动使用的 .h 或 .lib 文件。 | Y | *SDKName.props*文件告诉 Visual Studio 如何设置 Visual C++ 目录，等等，以便自动 *.h*或 *.lib*文件使用。 | N | |
| MSBuild 自动使用 .js 或 .css 文件****。 | Y | 在**解决方案资源管理器中**，可以展开 JavaScript SDK 引用节点以显示单独的 *.js*或 *.css*文件，然后通过将这些文件拖动到其源文件来生成`<source include/>`标记。 SDK 支持 F5 和包自动安装。 | Y | |
...
| 该机制可指定“不复制”。 | Y | 根据文件是否被丢弃在 *_redist*或 *_designtime*文件夹中，您可以控制要复制到使用应用程序的包中的文件。 | N | 需要声明包清单中要复制的文件。 |
...
| 该机制支持为本机 WinMD 指定注册信息。 | Y | 您可以在*SDKManifest.xml*中指定 .winmd 文件和 .dll 文件之间的相关性。 | N | |
...
| 该机制部署非引用文件（例如，将部署要在其上运行 [!INCLUDE[win8_appname_long](../debugger/includes/win8_appname_long_md.md)] 应用测试的测试框架）。 | Y | 如果将文件拖放到*\redist*文件夹中，则会自动部署这些文件。 | Y | |
