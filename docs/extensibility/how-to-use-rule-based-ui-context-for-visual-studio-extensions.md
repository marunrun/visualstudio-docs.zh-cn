---
title: 如何：对 Visual Studio 扩展使用基于规则的 UI 上下文 |Microsoft Docs
ms.date: 11/04/2016
ms.topic: conceptual
...
ms.locfileid: "74904002"
---
# <a name="how-to-use-rule-based-ui-context-for-visual-studio-extensions"></a>如何：将基于规则的 UI 上下文用于 Visual Studio 扩展

激活某些已知 <xref:Microsoft.VisualStudio.Shell.UIContext>时，Visual Studio 允许加载 Vspackage。 但是，这些 UI 上下文并不精细，这会使扩展创作者不会选择，而是选择在真正想要 VSPackage 加载的位置之前激活的可用 UI 上下文。 有关众所周知的 UI 上下文的列表，请参阅 <xref:Microsoft.VisualStudio.Shell.KnownUIContexts>。
...
|ActiveProjectCapability：\<表达式 >|当活动项目功能与提供的表达式匹配时，术语为 true。 表达式可以是类似于 VB &#124; CSharp 的内容。|
|SolutionHasProjectCapability：\<表达式 >|与上面类似，但当解决方案具有与表达式匹配的任何已加载项目时，term 为 true。|
...
<xref:Microsoft.VisualStudio.Shell.ProvideExtensibleUIContextRuleAttribute.%23ctor%2A> 文档中的构造函数显示可扩展 UI 上下文规则的语法。
