...
|特性|说明|  
|---------------|-----------------|  
|`Name`|必需。 包文件的名称。 这是在 `Command` 定义包安装条件时，元素将引用的名称。 此值还用作表中的键， `Strings` 以检索 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 将用于描述包的工具（如）的本地化名称。|  
...
## <a name="example"></a>示例  
 下面的代码示例为可 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 再发行组件包及其依赖项（如 Windows Installer）定义包。  
