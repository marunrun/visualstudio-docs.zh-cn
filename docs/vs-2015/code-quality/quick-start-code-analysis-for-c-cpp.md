---
title: 快速入门： C-C + + 的代码分析 |Microsoft Docs
ms.date: 11/15/2016
...
## <a name="configure-rule-sets-for-a-project"></a><a name="BKMK_ConfigureRuleSets"></a> 为项目配置规则集  
...
3. 若要在每次使用所选配置生成项目时运行代码分析，请选中 " **生成时启用 c/c + + 代码分析** " 复选框。 还可以通过打开 "**分析**" 菜单，然后选择 "在*项目名称***上运行代码分析"** 来手动运行代码分析。  
  
4. 在 " **运行此规则集** " 列表中，执行下列操作之一：  
  
    - 选择要使用的规则集。  
  
    - 选择 **\<Browse...>** 指定列表中不存在的现有自定义规则集。  
...
## <a name="run-code-analysis"></a><a name="BKMK_Run"></a> 运行代码分析  
...
## <a name="analyze-and-resolve-code-analysis-warnings"></a><a name="BKMK_Analyze"></a> 分析和解决代码分析警告  
...
3. 选择 " **禁止显示消息** "，然后选择 **"在源中"**。  
   ...
   禁止显示消息会插入禁止显示 `#pragma warning (disable:` *WarningId* `)` 代码行警告的 WarningId。  
  
## <a name="creating-work-items-for-code-analysis-warnings"></a><a name="BKMK_Creating_work_items_for_code_analysis_warnings"></a> 为代码分析警告创建工作项  
