...
## <a name="rule-description"></a>规则说明
 序列化过程中不会自动包含在继承 <xref:System.Runtime.Serialization.ISerializable?displayProperty=fullName> 接口的类型中声明的实例字段。 若要包含字段，类型必须实现 <xref:System.Runtime.Serialization.ISerializable.GetObjectData%2A> 方法和序列化构造函数。 如果不应序列化字段，请将 <xref:System.NonSerializedAttribute> 属性应用于这些字段，以显式指示决定。
