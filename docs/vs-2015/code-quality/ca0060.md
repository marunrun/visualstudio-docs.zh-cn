...
如果分析工具无法加载正在分析的程序集间接引用的程序集，则会出现此警告。 "间接引用" 指的是一个分析程序集的程序集引用所引用的引用程序集。 例如，如果代码分析 (FxCop) 正在分析程序集 A，并且程序集 A 引用程序集 B，而程序集 B 引用程序集 C，而程序集 B 引用程序集 c，则程序集 c 是间接引用，而程序集 B 是直接引用。
...
- 在 c + + 中，可以通过在 VCPROJ 中使用 Vcproj 来引用程序集，重写命令行以便 \<Some.dll> 仅在源中传递/FU 或添加 #using。
