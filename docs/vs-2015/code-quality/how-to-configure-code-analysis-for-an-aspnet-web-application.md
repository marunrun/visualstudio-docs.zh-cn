...
在 [!INCLUDE[vsPreShort](../includes/vspreshort-md.md)] 和中， [!INCLUDE[vsUltShort](../includes/vsultshort-md.md)] 可以从要应用于 Web 应用程序的代码分析 *规则集* 列表中进行选择 [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] 。 默认规则集是 Microsoft Mininimum 建议规则。 您可以选择要应用于网站的另一个规则集。
...
6. 在 " **运行此规则集** " 下拉列表中，执行以下步骤：
    ...
    - 选择 **\<Browse>** 此项可以指定列表中不存在的现有自定义规则集。
