...
## <a name="how-to-fix-violations"></a>如何解决冲突
 若要修复与此规则的冲突，请重命名重载的方法，使名称唯一。 或者，通过将可访问性更改为 `internal` （`Friend` 在 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] 中）或将 <xref:System.Runtime.InteropServices.ComVisibleAttribute?displayProperty=fullName> 特性设置为 `false`，使接口对 COM 不可见。
