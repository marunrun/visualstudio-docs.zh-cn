...
## <a name="rule-description"></a>规则说明
 [使用 PermitOnly 方法](https://msdn.microsoft.com/8c7bdb7f-882f-45b7-908c-6cbaa1767649)和 <xref:System.Security.CodeAccessPermission.Deny%2A?displayProperty=fullName> 安全操作应仅由对 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 安全性具有高级了解的用户使用。 应当对使用这些安全操作的代码进行安全检查。
...
- 通常可以通过多种方式指定用于构造基于路径的权限的值。 拒绝访问一种形式的路径不会拒绝对所有窗体的访问。 例如，如果 \\ \Server\Share 的文件共享映射到网络驱动器 X：，则若要拒绝对共享上的文件的访问，必须拒绝 \\ \Server\Share\File、X:\File 以及访问该文件的每个其他路径。
...
**linkdemand：调用方的 deny 在具有断言权限的 LinkDemand 上不起作用。** 
**Linkdemand：调用方的 Deny 对于受 LinkDemand 保护的代码不起作用。** 
**linkdemand：此拒绝不起作用LinkDemand 保护的代码。**
