...
## <a name="cause"></a>原因
 公共值类型不会重写 <xref:System.Object.Equals%2A?displayProperty=fullName>，或不实现相等运算符（= =）。 此规则不检查枚举。
...
### <a name="description"></a>描述
 下面的示例通过重写 <xref:System.ValueType.Equals%2A?displayProperty=fullName> 并实现相等运算符（= =，！ =）来修复以前的冲突。
