...
4. 请确保在对话框顶部的组合框中选择了 " **.NET Framework 4** "。 <xref:System.Windows.Controls.DataGrid>本演练中使用的控件仅在 .NET Framework 4 中可用。
...
5. 在 " **选择你的数据连接** " 页上，执行下列操作之一：
   ...
   - 单击 " **新建连接** "，并创建与 AdventureWorksLT 数据库的连接。
     ...
     请确保已选中 "将 **实体连接设置保存 App.Config 为** " 选项，然后单击 " **下一步**"。
...
## <a name="creating-data-bound-controls-that-display-the-orders"></a>创建显示订单的数据绑定控件
 通过 `SalesOrderHeaders` 将实体从 " **数据源** " 窗口拖到 WPF 设计器来创建显示订单记录的控件。
...
## <a name="creating-a-datagrid-that-displays-the-order-details"></a>创建显示订单详细信息的 DataGrid
 <xref:System.Windows.Controls.DataGrid>通过 `SalesOrderDetails` 将实体从 "**数据源**" 窗口拖到 WPF 设计器来创建显示订单详细信息的控件。
...
3. 对于 **SalesOrderDetails** 节点的以下每个子节点，单击节点旁边的下拉菜单，并选择 " **无**"：
   ...
   - **ModifiedDate**
     ...
     此操作可防止 Visual Studio 将此数据包含在 <xref:System.Windows.Controls.DataGrid> 下一步中创建的控件中。 对于本演练，假定最终用户不需要查看此数据。

4. 从 " **数据源** " 窗口中，将子 **SalesOrderDetails** 节点拖至 **WPF 设计器**中的窗口。

    Visual Studio 将生成 XAML 来定义新的数据绑定 <xref:System.Windows.Controls.DataGrid> 控件，并在设计器中显示该控件。 Visual Studio 还会更新 `GetSalesOrderHeadersQuery` 代码隐藏文件中的生成方法，以便在 **SalesOrderDetails** 实体中包含数据。
...
1. 按 **F5**。
    ...
    - 对于在 " **销售订单 ID** " 组合框中选择的每个订单，详细订单信息将显示在中 <xref:System.Windows.Controls.DataGrid> 。
