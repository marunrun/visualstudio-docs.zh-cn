...
5. 在菜单栏上，选择 "**文件**" "  >  **保存 SqlQuery_1 .sql**"。

     此时将显示 " **文件另存为** " 对话框。

6. 在 " **文件名" 框中** ，输入 `SampleImportScript.sql` ，记下要保存文件的位置，然后选择 " **保存** " 按钮。

7. 在菜单栏上，选择 "**文件**  >  **关闭解决方案**"。

     接下来，创建一个数据库项目，然后从已创建的脚本导入架构。

## <a name="create-a-database-project-and-import-a-schema"></a><a name="CreateProject"></a> 创建数据库项目并导入架构
...
## <a name="deploy-the-database"></a><a name="DeployDatabase"></a> 部署数据库
 按 **F5** 键时，默认情况下，会将数据库 (或发布) 到 LocalDB 数据库。 您可以通过打开项目的 "属性" 页，选择 " **调试** " 选项卡，然后更改连接字符串，将数据库部署到其他位置。
