...
2. （可选）选择要创建的控件。 默认情况下， **Data Sources** <xref:System.Windows.Controls.DataGrid> 如果你的项目面向 .NET Framework) 的早期版本 .NET Framework 4) 或 (，则 "数据源" 窗口中表示数据表或对象的每个项都设置为创建 (<xref:System.Windows.Controls.ListView> 。
