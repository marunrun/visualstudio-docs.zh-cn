...
|处理设计时模板或编译运行时（预处理）模板时，"表达式太复杂"。<br /><br /> 尝试检查运行时模板生成的代码时，[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 停止工作。|文本块太长。 T4 将文本块转换为字符串连接表达式，并为每个模板行使用一个字符串文本。 非常长的文本块可以 overstep 编译器的大小限制。|使用表达式块（例如）分解长文本块：<br /><br /> `<#= "" #>`|
