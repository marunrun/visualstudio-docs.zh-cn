...
|`[ValidationMethod (ValidationCategories.Menu)]`|指定将执行验证的时间。 如果要组合多&#124;个选项，请使用按位 "或" （）。<br /><br /> `Menu` = 由“验证”菜单调用。<br /><br /> `Save` = 在保存模型时调用。<br /><br /> `Open` = 在打开模型时调用。 `Load` = 在保存模型时调用，但如果违背规则，则警告用户此操作可能重新打开模型。 也可以在加载时（解析模型前）调用。|
