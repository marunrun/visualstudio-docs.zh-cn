...
## <a name="what-can-you-do-with-a-domain-specific-language"></a>使用域特定语言可以执行什么操作？
 ...
 设计 DSL 后，可以将其作为 Visual Studio 集成扩展的一部分分发 (VSIX) 包。 用户使用中的 DSL [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ：
...
## <a name="deploying-the-dsl"></a>部署 DSL
 若要允许其他用户使用域特定语言，请将 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 扩展 (VSIX) 文件中。 这是在构建 DSL 解决方案时创建的。
