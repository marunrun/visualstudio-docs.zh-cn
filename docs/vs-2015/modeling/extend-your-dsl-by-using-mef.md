...
8. 生成并运行解决方案，验证它是否仍然正常工作。
   ...
   DSL 现在已启用 MEF。 可以将菜单命令、笔势处理程序和验证约束作为 MEF 扩展来编写。 可以在 DSL 解决方案中将这些扩展与其他自定义代码一起编写。 此外，你或其他开发人员可以编写 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 扩展 DSL 的单独扩展。
...
1. 创建一个新类库项目。 为此，请在 "**新建项目**" 对话框中，选择 " **Visual Basic** " 或 " **Visual c #** "，然后选择 **"类库"。**
...
4. 在同一解决方案中创建 VSIX 项目。 为此，请在 " **新建项目** " 对话框中，展开 " **Visual Basic** " 或 " **Visual c #**"，单击 " **扩展性**"，然后选择 " **VSIX 项目**"。
...
11. 若要测试扩展，请按 **F5**。 在的实验实例中 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ，创建或打开 DSL 的示例文件。
...
### <a name="gesture-handlers"></a>笔势处理程序
 笔势处理程序可处理从内部或外部的任何位置拖到关系图上的对象 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 。 以下示例允许用户将文件从 Windows 资源管理器拖到关系图上。 它创建包含文件名的元素。
...
## <a name="see-also"></a>另请参阅
 [发布 Visual Studio 扩展](../extensibility/shipping-visual-studio-extensions.md) [Managed Extensibility Framework (MEF) ](https://msdn.microsoft.com/library/6c61b4ec-c6df-4651-80f1-4854f8b14dde)如何：[在域特定语言中](../modeling/validation-in-a-domain-specific-language.md)[添加拖放处理程序](../modeling/how-to-add-a-drag-and-drop-handler.md)验证
