...
## <a name="see-if-an-item-supports-validation"></a><a name="SupportsValidation"></a> 查看项是否支持验证
...
## <a name="include-other-net-assemblies-and-projects-for-validation"></a><a name="IncludeReferences"></a> 包括用于验证的其他 .NET 程序集和项目
...
## <a name="validate-code-manually"></a><a name="ValidateManually"></a> 手动验证代码
...
1. 右键单击关系图图面，然后单击 " **验证体系结构**"。
    ...
    > [!NOTE]
    > 默认情况下，层关系图)  ( 上的 " **生成操作** " 属性设置为 " **验证** "，以使关系图包含在验证过程中。
...
### <a name="manage-validation-errors"></a><a name="ManageErrors"></a> 管理验证错误
...
## <a name="validate-code-automatically"></a><a name="ValidateAuto"></a> 自动验证代码
...
- [对生成过程使用默认模板](https://msdn.microsoft.com/library/43930b12-c21b-4599-a980-2995e3d16e31)
...
## <a name="troubleshoot-layer-validation-issues"></a><a name="TroubleshootingValidation"></a> 层验证问题疑难解答
...
## <a name="understanding-and-resolving-layer-validation-errors"></a><a name="UnderstandingValidationErrors"></a> 了解和解决层验证错误
...
|**错误语法**|**错误描述**|
|----------------------|---------------------------|
|AV0001：无效的依赖项： *Artifact1* (*ArtifactType1*) --> *Artifact2* (*ArtifactType2*) <br /><br /> 层： *LayerName1*、 *LayerName2* &#124; 依赖关系： *DependencyType*|*LayerName1*中的*Artifact1*不应与*LayerName2*上的*Artifact2*相关，因为*LayerName1*没有直接依赖于*LayerName2*。|
|AV1001：无效的命名空间： *项目*<br /><br /> 层： *LayerName* &#124; 必需的命名空间： *NamespaceName1* &#124; 当前命名空间： *NamespaceName2*|*LayerName* 要求其关联的项目必须属于 *NamespaceName1*。 *项目* 在 *NamespaceName2*中，而不是 *NamespaceName1*。|
|AV1002：依赖于禁止的命名空间： *Artifact1* (*ArtifactType1*) &#124; *Artifact2* (*ArtifactType2*) <br /><br /> 层： *LayerName* &#124; 禁止的命名空间： *NamespaceName* &#124; 依赖关系： *DependencyType*|*LayerName* 要求其关联的项目不能依赖于 *NamespaceName*。 *Artifact1* 不能依赖于 *Artifact2* ，因为 *Artifact2* 在 *NamespaceName*中。|
|AV1003：在禁止的命名空间： *项目* (*ArtifactType*) <br /><br /> 层： *LayerName* &#124; 禁止的命名空间： *NamespaceName*|*LayerName* 要求其关联的项目不能属于 *NamespaceName*。 *项目* 属于 *NamespaceName*。|
