...
## <a name="the-standard-profiles"></a>标准配置文件
 安装后，在 Visual Studio 支持的版本中提供以下配置文件。

|Profile|目标|
|-------------|-------------|
|[UML 标准配置文件 L2](#L2)|可用于添加元素或关系附加信息的一组标准构造型。|
|[UML 标准配置文件 L3](#L3)|可用于添加元素或关系附加信息的一组标准构造型。|
|[C#简介](#NetProfile)|如果你希望 UML 模型中的类或其他元素能够表示程序代码，可以通过应用 C# 配置文件中的其中一个构造型来指示。<br /><br /> 这些构造型还会向模型元素添加属性。|
...
|构造型|适用对象|含义|
|----------------|----------------|-------------|
|辅助|实例|通常通过实现其他逻辑支持另一个类的类。 其他类可能具有 «焦点» 构造型。|
...
|可执行文件|项目|可执行 «文件»。|
|文件|项目|物理文件。|
|焦点|实例|定义核心业务逻辑且由多个 «辅助» 类支持的类。|
|框架|Package|此包定义可重用的设计模式。|
|实现|组件|«规范» 的实现。|
|implementationClass|实例|此类描述一个实现，并且每个运行时实例都有一个固定的实现类。 与 «类型» 对比。|
...
|类型|实例|指定对象的行为，但不声明如何实现。 如果符合规范，则对象是类型的成员。|
|utility|实例|静态函数的集合。 该类没有任何实例。|
...
|构造型|适用对象|属性|
|----------------|----------------|----------------|
|**C#班级**|UML 类<br /><br /> 组件|**Clr 特性**<br /><br /> **为 Partial**<br /><br /> **是密封的**<br /><br /> **为静态**<br /><br /> **不安全**<br /><br /> **包可见性**|
|**C#结构**|UML 类<br /><br /> 组件|**Clr 特性**<br /><br /> **为 Partial**<br /><br /> **不安全**<br /><br /> **包可见性**|
|**C#全局成员**|UML 类<br /><br /> 组件|**Clr 特性**|
|**C#交互**|UML 接口|**Clr 特性**<br /><br /> **为 Partial**<br /><br /> **包可见性**|
|**C#枚举**|UML 枚举|**ClrAttributes**<br /><br /> **基类型**|
|**C#名称**|UML 包|**Clr 特性**<br /><br /> **基名称**<br /><br /> **using 命名空间**|
