...
- 为要访问的 DSL 指定指令处理器。 这会加载 DSL 的程序集，以便在文本模板的代码中使用其域类、属性和关系。 它还会加载您指定的模型文件。
  ...
  从 DSL 最小语言模板创建新的 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 解决方案时，将在调试项目中创建类似于以下示例的 `.tt` 文件。
...
- 虽然编写代码片段的语言为C#，但你可以生成任何类型的文本。 您也可以通过将属性 `language="VB"` 添加到 `template` 指令来在 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] 中编写代码。

- 若要调试模板，请将 `debug="true"` 添加到 `template` 指令中。 如果发生异常，则模板将在 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 的另一个实例中打开。 若要在代码中的特定点中断调试器，请插入语句 `System.Diagnostics.Debugger.Break();`
...
```
 ...
 指令的名称（在本示例中为 `MyLanguage`）派生自 DSL 的名称。 它调用作为 DSL 的一部分生成的*指令处理器*。 可以在**Dsl\GeneratedCode\DirectiveProcessor.cs**中找到它的源代码。

 DSL 指令处理器执行两个主要任务：

- 它有效地将程序集和导入指令插入到引用 DSL 的模板中。 这允许您在模板代码中使用您的域类。

- 它加载您在 `requires` 参数中指定的文件，并在 `this` 中设置引用已加载模型的根元素的属性。
...
> 此方法使您可以读取同一模板中的多个模型，但不支持 ModelBus 引用。 若要读取由 ModelBus 引用了的模型，请参阅[在文本模板中使用 Visual Studio ModelBus](../modeling/using-visual-studio-modelbus-in-a-text-template.md)。
 ...
 如果要从同一文本模板访问多个模型，则必须为每个模型调用一次生成的指令处理器。 必须在 `requires` 参数中指定每个模型的文件名。 你必须在 `provides` 参数中指定要用于根域类的名称。 必须为每个指令调用中的 `provides` 参数指定不同的值。 例如，假定您有三个名为 Library、School 和 .xyz 的模型文件。 若要从同一文本模板访问它们，必须编写三个类似于下面的指令调用。
...
## <a name="loading-models-dynamically"></a>动态加载模型
 ...
 若要加载该文件，最有效的方法是使用 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ModelBus。 在典型方案中，文本模板将使用特定于 DSL 的指令以常规方式加载第一个模型。 该模型将包含对另一个模型的 ModelBus 引用。 您可以使用 ModelBus 打开引用的模型并访问特定的元素。 有关详细信息，请参阅[在文本模板中使用 Visual Studio ModelBus](../modeling/using-visual-studio-modelbus-in-a-text-template.md)。

 在不太常见的情况下，你可能想要打开一个只具有文件名且不在当前 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 项目中的模型文件。 在这种情况下，可以使用[如何：在程序代码中打开文件中的模型](../modeling/how-to-open-a-model-from-file-in-program-code.md)中所述的方法来打开文件。
