...
- 为要访问的 DSL 指定指令处理器。 这会加载 DSL 的程序集，以便在文本模板的代码中使用其域类、属性和关系。 它还会加载您指定的模型文件。
  ...
  `.tt` [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 从 DSL 最小语言模板创建新的解决方案时，将在调试项目中创建类似于以下示例的文件。
...
- 虽然编写代码片段的语言是 c #，但你可以生成任何类型的文本。 也可以 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] 通过将属性添加到指令来在中编写代码 `language="VB"` `template` 。

- 若要调试模板，请将添加 `debug="true"` 到 `template` 指令中。 如果发生异常，则模板将在另一个实例中打开 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 。 如果希望在代码中的特定点中断调试器，请插入语句 `System.Diagnostics.Debugger.Break();`
...
## <a name="loading-models-dynamically"></a>动态加载模型
 ...
 在不太常见的情况下，你可能想要打开的模型文件只有一个文件名，并且该文件可能不在当前 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 项目中。 在这种情况下，可以使用 [如何：在程序代码中打开文件中的模型](../modeling/how-to-open-a-model-from-file-in-program-code.md)中所述的方法来打开文件。
