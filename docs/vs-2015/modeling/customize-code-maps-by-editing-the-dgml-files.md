...
ms.locfileid: "72663134"
---
# <a name="customize-code-maps-by-editing-the-dgml-files"></a>Customize code maps by editing the DGML files
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

若要自定义代码图，可以编辑代码图的定向关系图标记语言 (.dgml) 文件。 例如，可以编辑元素来指定自定义样式，向代码元素和链接分配属性和类别，或将文档（或 URL）链接到代码元素（或链接）。 有关 DGML 元素的详细信息，请参阅[定向关系图标记语言（DGML）引用](../modeling/directed-graph-markup-language-dgml-reference.md)。
...
4. 在 `<Links>` 部分，确保对于组代码元素与其子代码元素之间的每个关系都存在具有以下特性的 `<Link/>` 元素：
   ...
   ```
    ...
    有关 `Category` 特性的详细信息，请参阅向[代码元素和链接分配类别](#AssignCategories)。
...
1. 如果 `Coverage` > 80，则将 `Background` 属性设置为绿色。

2. 否则，如果 `Coverage` > 50，则将 `Background` 属性设置为基于 `Coverage` 属性值的橙色底纹。
...
## <a name="AssignProperties"></a>向代码元素和链接分配属性
...
3. 执行下表中的任务之一：
   ...
   2. 在 `<Properties>` 部分：
      ...
      3. 添加 `IsReference` 特性并将其设置为 `True`，以使引用显示在代码元素的 "**跳到引用**" 快捷菜单上。
...
## <a name="see-also"></a>请参阅
 [映射解决方案中的依赖项](../modeling/map-dependencies-across-your-solutions.md)[使用代码图调试应用程序](../modeling/use-code-maps-to-debug-your-applications.md)[使用代码图分析器发现潜在问题](../modeling/find-potential-problems-using-code-map-analyzers.md)[浏览和重新排列代码](../modeling/browse-and-rearrange-code-maps.md)图[定向图形标记语言（DGML）引用](../modeling/directed-graph-markup-language-dgml-reference.md)