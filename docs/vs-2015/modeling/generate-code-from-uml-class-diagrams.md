...
3. 展开 "**构造型**" 属性，以查看可以设置的其他属性。
   ...
   类型、特性、操作和关联的**说明**属性将写入生成的代码中的 `<summary>` 注释。 将链接到类型的注释元素写入 `<remarks>` 注释。

## <a name="varying-the-generated-code"></a>改变生成的代码
 生成的代码随每个类型、特性或操作的属性的不同而不同。 例如，如果您将类的 "**是" 为 "抽象**" 属性设置为 "true"，则在生成的类上将显示 `abstract` 关键字。 如果将属性的重**数**设置为 0 .. **\*** ，则生成的属性将具有 `IEnumerable<>` 类型。
...
2. 在 "**属性**" 窗口中，选择 "**文本模板绑定**" 属性中的省略号**按钮（"..."** ）。
...
4. 在该对话框中，设置文本模板绑定的属性。
   ...
   |    **Property**    |                                                                                                                                                                                                                                                                                                                    **描述**                                                                                                                                                                                                                                                                                                                    |
   |--------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
   |        “属性”        |                                                                                                                                                                                                                                                  此绑定的名称。 若要替代继承自包含包或模型的某个绑定，请使用与要替代的绑定相同的名称。                                                                                                                                                                                                                                                  |
   |     Overwrite      |                                                                                                                                                                                                                                                                                                      如果为 True，则替代任何现有代码。                                                                                                                                                                                                                                                                                                       |
   |    Target Name     | 生成的文件的名称。<br /><br /> 可以在此字符串中插入表达式，如 `{Name}` 或 `{Owner.Name}`。 例如，可以编写： `{Owner.Name}_{Name}`。 在模型元素上计算表达式。 它可使用元素而非方法的属性。 若要查找可以使用的属性，请查看 **\* VisualStudio**中的类型属性。 \* \*Important： \* \* `{Name}` 或 `{Owner.Name}` 只能在 "**目标名称**" 属性中使用。 若要更改生成的类的名称，你必须修改模板。 有关详细信息，请参阅[编写文本模板](#writing)。 |
   ...
   |  目标目录  |                                                                                          在其中生成目标文件的文件夹。 该路径是项目文件夹的相对路径。<br /><br /> 可以使用 `{PackageStructure}` 表达式插入与包含程序包的名称相对应的路径。 默认值为 `\GeneratedCode\{PackageStructure}`。 还可以包含环境变量，例如 %TEMP% 或 %HomePath%。 **重要提示：** 只能在 "**目标目录**" 属性中使用 `{PackageStructure}`。                                                                                          |
...
- 括在括号中的模板部分 `<# ... #>` 由 "**生成代码**" 命令执行。 括号外部的模板部分将复制到结果文件。 区分生成的代码和生成的文本很重要。 生成的文本可采用任何语言。
