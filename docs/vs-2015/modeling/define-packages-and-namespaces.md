...
## <a name="in-this-topic"></a>本主题内容
 ...
 [从一个命名空间到另一个命名空间的引用](#References)

 [包的属性](#Properties)

## <a name="Namespaces"></a>名称
...
## <a name="References"></a>从一个命名空间到另一个命名空间的引用
...
- 使用完全限定名 `SalesCommon::CustomerAddress` 指定特性的类型。 仅当 `CustomerAddress` 将其 "**可见性**" 属性设置为 "**公共**" 时，才能执行此操作。
...
|**限定名称**|*Container* ：：*包名称*|全名，以包含此包的包或模型的名称为前缀。 有关详细信息，请参阅[命名空间](#Namespaces)。|
|**储存**|（空）|链接到此包的配置文件的列表。 这些配置文件提供可应用于包内元素的构造型。 有关详细信息，请参阅[通过配置文件和构造型自定义模型](../modeling/customize-your-model-with-profiles-and-stereotypes.md)。|
|**可见性**|**COMClassAttribute**|包在其父包外部的可见性。|
