...
- 使用**合并节点**（5）将两个或多个在**决策节点**上分支的替代流组合在一起。
...
### <a name="describing-data-flow-with-object-nodes"></a>使用对象节点描述数据流
 ...
 ![对象节点可以显示在操作之间传递的数据](../modeling/media/uml-actguidedata.png "UML_ActGuideData")
...
#### <a name="buffering-data-in-object-nodes"></a>在对象节点中对数据进行缓冲
 ...
 ![缓冲对象节点中的数据](../modeling/media/uml-actguidebuffer.png "UML_ActGuideBuffer")
...
### <a name="describing-data-flow-with-input-and-output-pins"></a>使用输入和输出插针描述数据流
 ...
 ![输入插针和输出插针是操作参数](../modeling/media/uml-actguidepins.png "UML_ActGuidePins")
...
### <a name="Subactivities"></a>用调用行为的操作描述子活动
 ...
 ![单独的活动图显示详细操作](../modeling/media/uml-actguidedetail.png "UML_ActGuideDetail")
...
- 在子活动图中，为调用操作上的每个输入插针和输出插针创建一个**活动参数节点**（2）。 为每个节点指定与其所对应的插针相同的名称。
...
- 在子活动图中绘制连接线，这些连接线将显示流入或流出每个活动参数节点的对象流。
  ...
  ![调用行为的 pin 映射到活动参数](../modeling/media/uml-actguidesub.png "UML_ActGuideSub")
...
- 选择描述如何仅将进入流的某些对象传输到接收操作。
  ...
  ![在另一个关系图中定义的对象转换](../modeling/media/uml-actguidetransform.png "UML_ActGuideTransform")
