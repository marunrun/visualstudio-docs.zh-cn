...
> [!NOTE]
> 请务必保持文件之间的一致性。 例如，如果使用源控件回滚 .uml 文件中的更改，则应该同时回滚 .*diagram 和 .layout 文件中的相应更改。 中表示的元素。如果不是在一个 uml 文件中也表示 \*diagram 文件，则该文件将会丢失。
...
### <a name="Exclusive"></a>需要对模型具有独占访问权限的更改
