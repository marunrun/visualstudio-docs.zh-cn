...
- 执行 *组件 (1*) 是与系统交互的一类人员、组织、设备或外部软件组件。 示例参与者是 **客户**、 **餐馆**、 **温度传感器**、 **信用卡授权者。**

- *用例* (2) 表示一个或多个参与者在实现特定目标时执行的操作。 示例用例为 " **订餐**"、" **更新" 菜单**、" **处理付款**"。

   在用例图中，用例与执行它们的参与者相关联 (3)。

- *系统 (4) *是要开发的任何内容。 系统可以是小型软件组件，其中的参与者只是其他软件组件；系统也可以是完整的应用程序；系统还可以是部署在多台计算机和设备上的大型分布式应用程序套件。 例如，" **订餐" 网站**、" **餐饮交付业务**"、" **网站版本 2**"。

   用例图可以显示系统或其子系统支持的用例。

## <a name="basic-steps-for-drawing-use-case-diagrams"></a><a name="BasicSteps"></a> 绘制用例图的基本步骤
...
2. 将 "执行组件 **" 从 "** 工具箱" 拖到关系图上 (将其置于任何系统边界) 之外。
...
## <a name="drawing-actors-and-use-cases"></a><a name="Actors"></a> 绘制参与者和用例
...
### <a name="inheritance-between-actors"></a>参与者之间的继承
 ![显示继承的用例关系图](../modeling/media/uml-ucguideinherit.png "UML_UCGuideInherit")
...
### <a name="multiplicities-between-actors-and-use-cases"></a>参与者与用例之间的重数
 ...
 ![与参与者一对一的用例](../modeling/media/uml-ucguidemulti1.png "UML_UCGuideMulti1")
...
> 如果用例图上关联的重数都为 **1**，则隐藏该重数。
 ...
 ![显示多对多重数的用例](../modeling/media/uml-ucguidemulti2.png "UML_UCGuideMulti2")

- 若要声明同一类的多个执行组件可以在用例中出现一次，请将关联的参与者端的重数设置为**1 ... \* **
...
## <a name="describing-use-cases-in-detail"></a><a name="Details"></a> 详细描述用例
 用例表示：
...
### <a name="linking-use-cases-to-work-items"></a>将用例链接到工作项
 如果你的项目使用 [!INCLUDE[vstsTfsRosarioLong](../includes/vststfsrosariolong-md.md)] 并且你具有 [!INCLUDE[esprtfc](../includes/esprtfc-md.md)] ，则可以将每个用例链接到中的一个工作项 [!INCLUDE[esprfound](../includes/esprfound-md.md)] 。 若要了解如何创建这些链接，请参阅 [链接模型元素和工作项](../modeling/link-model-elements-and-work-items.md)。
...
## <a name="structuring-use-cases"></a><a name="Structuring"></a> 结构化用例
...
- [用“包括”显示用例的详细信息](#Include)

- [使用“泛化”共享目标](#Inheritance)

- [使用“扩展”分离变体用例](#Extend)

### <a name="showing-the-details-of-a-use-case-with-include"></a><a name="Include"></a> 用 Include 显示用例的详细信息
...
> 不应造成会导致用例包括自身的包括关系循环。 循环可能会产生错误。
 ...
 ![通过包含分解的用例](../modeling/media/uml-ucguideinclude.png "UML_UCGuideInclude")
...
#### <a name="defining-the-order-of-the-detailed-steps"></a><a name="Steps"></a> 定义详细步骤的顺序
...
- 活动图中的操作与被包括用例有相同的名称。
  ...
  ![显示在链接活动图中的用例步骤](../modeling/media/uml-ucguidesteps.png "UML_UCGuideSteps")

### <a name="sharing-goals-with-generalization"></a><a name="Inheritance"></a> 用通用化共享目标
 使用泛化关系来表明， *专用* 用例是实现另一个 *通用* 用例所表达目标的一种特殊方法。 开放箭头应指向更通用的用例。

 ![显示泛化关系的用例](../modeling/media/uml-ucguidegeneral.png "UML_UCGuideGeneral")
...
### <a name="separating-variant-cases-with-extend"></a><a name="Extend"></a> 用扩展分隔变体事例
...
4.  (在示例中 **注册新用户** 的扩展的说明) 应包括在主用例方案中发生的位置的详细信息，以及在什么情况下。 将其视为修改主用例的描述。
...
## <a name="using-subsystem-boundaries"></a><a name="Subsystems"></a> 使用子系统边界
...
### <a name="system-versions"></a>系统版本
 ...
 ![显示系统的不同版本的子系统](../modeling/media/uml-ucguidesystem.png "UML_UCGuideSystem")
