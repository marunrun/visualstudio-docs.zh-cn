...
* 还可以启用**层交互分析**，这种方法提供了有关在可与一个或多个数据库通信的多层应用程序函数中同步 ADO.NET 调用的执行时间的更多信息。 您可以使用任意分析方法收集层交互数据。 有关层交互分析的详细信息，请参阅[层交互视图](https://msdn.microsoft.com/library/azure/dd557764.aspx)。

## <a name="configuring-profiling-settings"></a>配置分析设置
下图显示了如何从“发布 Azure 应用程序”对话框配置性能分析设置的方法。

![配置事件探查设置](./media/vs-azure-tools-performance-profiling-cloud-services/IC526984.png)
...
为云服务中的角色的每个实例创建分析会话。 若要从 Visual Studio 中查看每个会话的分析报告，可查看“服务器资源管理器”窗口，然后选择“Azure 计算”节点以选择角色的一个实例。 然后可以查看分析报告，如下图所示。

![从 Azure 查看事件探查报告](./media/vs-azure-tools-performance-profiling-cloud-services/IC748914.png)
...
3. 要查看实例的分析报告，请选择服务中的角色，打开特定实例的快捷菜单，并选择“查看分析报告”。
    ...
    接下来，将从 Azure 中下载 .vsp 报告文件，下载状态显示在“Azure 活动日志”中。 下载完成后，分析报告会显示在名为 <Role name\><Instance Number\><identifier\>.vsp 的 Visual Studio 编辑器选项卡中。 随即将显示报告的汇总数据。
