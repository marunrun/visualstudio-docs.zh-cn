...
若要捕获有关角色实例何时发生故障的信息，请选中“启用故障转储的传输”复选框。 （因为 ASP.NET 处理大多数异常，所以这通常仅对辅助角色有用。）若要增加或减少专用于故障转储的存储空间的百分比，请更改 "**目录配额（%）** " 值。 可以更改将故障转储存储到其中的存储容器，然后选择要捕获“完整”转储还是“微型”转储。
...
**我无法从 RoleEntryPoint 等方法获取跟踪信息。怎么了？**

RoleEntryPoint 的方法在 WAIISHost.exe 上下文而非 IIS 中调用。 web.config 中通常会启用跟踪的配置信息在此不适用。 若要解决此问题，请向 Web 角色项目添加一个 .config 文件，并使其名称匹配包含 RoleEntryPoint 节点的输出程序集。 在默认 web 角色项目中，.config 文件的名称应为 Waiishost.exe。将以下行添加到此文件中：
