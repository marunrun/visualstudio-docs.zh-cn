...
1. 在 Windows 窗体设计器的 "工具箱" 中，选择 " (**组件**" 类别中的 "**计时器**") ，然后选择 "ENTER" 键，或双击该计时器，向窗体添加计时器控件。 该计时器的图标名为 **Timer1**，应显示在窗体下的空间中，如下图所示。

     ![计时器](../ide/media/express-timer.png "Express_Timer") 记

    > [!NOTE]
    > 如果工具箱是空的，请确保在打开工具箱前选择窗体设计器，而不是窗体的后台代码。

2. 选择 **Timer1** 图标以选中该计时器。 在“属性”  窗口中，从查看事件切换到查看属性。 然后将计时器的 **Interval** 属性设置为 **750**，但保留 **Enabled** 属性的设置“False”  。 **Interval** 属性将通知计时器两个计时周期** 之间的等待时长，或何时触发 Tick 事件。 值为 750 时，将通知计时器等待四分之三秒（750 毫秒）后触发 Tick 事件。 只有在玩家选择第二个标签后，你才能调用 `Start()` 方法启动计时器。

3. 选择 Windows 窗体设计器中的计时器控件图标，然后选择 ENTER 键，或双击该计时器，以添加空的 **Tick** 事件处理程序。 用下列代码替换该代码，或手动将下列代码输入到事件处理程序。
...
- 若要继续学习下一个教程，请参阅 [步骤7：保持对可见](../ide/step-7-keep-pairs-visible.md)。

- 若要返回上一个教程步骤，请参阅 [步骤5：添加标签引用](../ide/step-5-add-label-references.md)。
