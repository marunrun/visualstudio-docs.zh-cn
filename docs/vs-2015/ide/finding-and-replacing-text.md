...
## <a name="find-and-replace-control"></a>“查找和替换”控件
 ...
 通过选择“查找”文本框旁边的按钮，可以访问替换选项****。 若要一次替换一个，请选择“替换”文本框旁边的“替换下一个”按钮********。 若要替换所有匹配项，请选择 "全部 **替换** " 按钮。
...
### <a name="searching-tool-windows"></a>搜索工具窗口
 可以通过在 "**编辑**" 菜单上选择 "**查找和替换**"，或 (CTRL + F) 来使用代码或文本窗口（如 "**输出**" 窗口和 "**查找结果**" 窗口）中的 "**查找**" 控件。
...
### <a name="creating-custom-search-folder-sets"></a>创建自定义搜索文件夹集
 您可以通过选择 "**选择搜索文件夹**" 按钮，在 "**查找**范围" 框旁 (其**外观) 来**定义搜索范围。 在“选择搜索文件夹”对话框中，可以指定在其中进行搜索的一组文件夹，并且可以保存规范，供以后重复使用****。 只有将远程计算机的驱动器映射到本地计算机后，才可以指定远程计算机上的文件夹。
