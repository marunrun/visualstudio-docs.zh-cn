...
## <a name="remarks"></a>备注
 默认情况下，外接程序的命令名称等于 *\<AddInSolutionName>* 。连接<em>. \<AddInSolutionName> </em>，并在 Connect.cs 中显示为 `commandName` 方法的参数 `Exec` 。 还可以通过开始在 Visual Studio 的“命令”窗口中键入外接程序的名称并使用 Intellisense 填充其余部分来验证命令名称。
