 ...
 **当项目位置不受信任时警告用户**如果尝试在不完全受信任的位置（例如，在 UNC 路径或 HTTP 路径上）创建新项目或打开现有项目，将显示一条消息。 使用此选项以指定每次在不完全受信任的位置尝试创建或打开一个项目时是否显示该消息。
 ...
 **重命名文件时提示重命名符号**选定后，将显示一个消息框，询问是否 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 还应将项目中的所有引用重命名为代码元素。
