...
## <a name="statement-completion"></a>语句结束
 自动列出成员选定后，在编辑器中键入时，可通过 IntelliSense 显示可用成员、属性、值或方法的弹出列表。 从弹出列表中任选一项，即可将其插入到代码中。 选择此选项将启用“隐藏高级成员”选项****。

 选中时隐藏高级成员，通过仅显示最常用的项来缩短弹出语句完成列表。 其他项将从该列表中筛选出去。
...
> 这些行号不会添加到代码中，也不会打印出来。 它们仅供参考。
 ...
 启用单击 URL 导航选中此项后，鼠标光标将在编辑器中传递 URL 时变为手形。 您可以单击 URL 以在 Web 浏览器中显示所指示的页。
