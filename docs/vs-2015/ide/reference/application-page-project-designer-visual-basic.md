 ...
 **图标**设置要用作程序图标的 .ico 文件。 选择“\<浏览...>”  以浏览现有图形。 有关详细信息，请参阅 [/win32icon](https://msdn.microsoft.com/library/aecaab01-9353-46c5-941c-6edabd4eff92)（或 [/win32icon（C# 编译器选项）](https://msdn.microsoft.com/library/756d9b6d-ab07-41b7-ba58-5bd88f711138)）。 若要以编程方式访问此属性，请参阅 <xref:VSLangProj.ProjectProperties.ApplicationIcon%2A>。
 ...
 **启用应用程序框架**指定项目是否将使用应用程序框架。 此选项的设置会影响“启动窗体”  /“启动对象”  中可用的选项。
 ...
 **启用 XP 视觉样式**启用或禁用 Windows XP 视觉样式，也称为*WINDOWS Xp 主题*。 Windows XP 视觉样式可启用一些控件，例如带有圆角和动态颜色的控件。 默认为已启用。 有关 Windows XP 视觉样式的详细信息，请参阅 [Windows XP 功能和 Windows 窗体控件](https://msdn.microsoft.com/bc7fab94-fce9-4bf1-a8ad-a5837c91c3c0)。

 **生成单个实例应用程序**选中此复选框可阻止用户运行多个应用程序实例。 此复选框的默认设置为清除状态。 此设置允许运行应用程序的多个实例。

 **关机时保存我的设置**选中此复选框以指定在用户关闭计算机时保存应用程序的 `My.Settings` 设置。 默认设置为已启用。 如果此选项处于禁用状态，可通过调用 `My.Settings.Save` 手动保存应用程序设置。

 **身份验证模式**选择 " **windows** " （默认设置），以指定使用 windows 身份验证来识别当前登录的用户。 可以使用 `My.User` 对象在运行时检索此信息。 如果要提供自己的代码来对用户进行身份验证，而不是使用默认的 Windows 身份验证方法，则选择“由应用程序定义”  。

 **关机模式**选择 "**启动窗体关闭时**" （默认值）以指定在设置为启动窗体的窗体关闭时应用程序退出，即使其他窗体已打开。 选择“当最后一个窗体关闭时”  以指定应用程序在最后一个窗体关闭或者显式调用 `My.Application.Exit` 或 `End` 语句时退出。
 ...
 **查看应用程序事件**单击此按钮可显示事件代码文件，您可以在其中为应用程序框架事件写入事件 `Startup`、`Shutdown`、`UnhandledException`、`StartupNextInstance` 和 `NetworkAvailabilityChanged`。 还可以重写某些应用程序框架方法。 例如，可以通过重写 `OnInitialize` 来更改初始屏幕的显示行为。
 ...
 **编辑 XAML**单击此按钮可在 XAML 编辑器中打开和修改应用程序定义文件（应用程序 .xaml）。 单击此按钮时，Application.xaml 将在应用程序定义节点处打开。 可能需要编辑此文件才能执行某些任务，例如定义资源。 如果应用程序定义文件不存在，项目设计器将创建一个。

 **查看应用程序事件**单击此按钮可在代码编辑器中显示 `Application` 分部类文件（app.xaml）。 如果该文件不存在，项目设计器将创建一个具有适当类名和命名空间的此类文件。
