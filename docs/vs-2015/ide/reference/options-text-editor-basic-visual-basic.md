...
- 重新设置日期格式
  ...
  **自动插入 Interface 和 MustOverride 成员** 当你 `Implements` 为类提交语句或 `Inherits` 语句时，文本编辑器会插入必须分别实现或重写的成员的原型。
 ...
 **启用引用和关键字的突出显示** 文本编辑器可以突出显示某个符号的所有实例或子句中的所有关键字 `If..Then` ，如、 `While...End While` 或 `Try...Catch...Finally` 。 可通过按下 Ctrl+Shift+向下键或 Ctrl+Shift+向上键在突出显示的引用或关键字之间浏览。
