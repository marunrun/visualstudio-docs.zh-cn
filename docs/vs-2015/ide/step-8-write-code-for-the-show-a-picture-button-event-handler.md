...
- 如果用户打开一个图片文件，则程序将在 PictureBox 中显示该图片。
  ...
  ![视频链接](../data-tools/media/playvideo.gif "PlayVideo")有关本主题的视频版本，请参阅 [教程1：在 Visual Basic 中创建图片查看器-视频 4](https://msdn.microsoft.com/vbasic/gg315355.aspx) 或 [教程1：用 c # 创建图片查看器-视频 4](https://msdn.microsoft.com/vcsharp/gg278412.aspx)。 这些视频使用 Visual Studio 的早期版本，因此在一些菜单命令和其他用户界面元素上略有差异。 但是，概念和过程与当前版本的 Visual Studio 大同小异。
...
2. 在两个大括号 { } 之间的空行上键入一个 `i`。  (在 Visual Basic 中，在 "私有 Sub ..." ) " **IntelliSense** " 窗口将打开，如下图所示。

     ![带有 Visual C&#35; 代码的 IntelliSense](../ide/media/express-ifintellisense.png "Express_IfIntellisense") 带有 Visual c # 代码的 IntelliSense

3. “IntelliSense”窗口应该会突出显示“if”一词********。  (如果没有，请输入小写形式 `f` ，它将会出现。 ) 请注意 " **IntelliSense** " 窗口旁有一个小*工具*框的显示方式，其中包含说明 " **If 语句的代码片段**"。  (在 Visual Basic 中，该工具提示还指明这是一个代码段，但使用略微不同的措辞。 ) 您想使用该代码片段，因此请选择 TAB 键以将 **其插入到** 您的代码中。 然后再次选择 Tab 键即可使用“if”代码片段****。 （如果选择其他位置后“IntelliSense”窗口消失，那么请按 Backspace 删除“i”并重新键入它，此时“IntelliSense”窗口将再次打开************。）
...
4. 接下来，使用 IntelliSense 输入更多代码，打开“打开文件”对话框****。 如果用户选择“确定”按钮，则 PictureBox 将加载用户选择的文件****。 下面的步骤演示如何输入代码，尽管步骤很多，但只需几次击键即可完成：
    ...
    3. 键入一个句点 (`.`)  (多个程序员调用这一点。 ) 因为在 **openFileDialog1**后键入了一个点，则会打开一个 **IntelliSense** 窗口，其中填充了 **OpenFileDialog** 组件的所有属性和方法。 在 Windows 窗体设计器中选择这些属性时，这些属性将显示在 " **属性** " 窗口中。 您还可选择将告知组件要执行的操作（例如打开对话框）的方法。
    ...
    8. 键入一个点，在“IntelliSense”窗口中打开 DialogResult 值****。 输入字母 `O` ，然后选择 TAB 键以插入 **"确定"**。
...
- 若要继续学习下一个教程，请参阅 [步骤9：检查、注释和测试代码](../ide/step-9-review-comment-and-test-your-code.md)。

- 若要返回上一个教程步骤，请参阅 [步骤7：向窗体添加对话框组件](../ide/step-7-add-dialog-components-to-your-form.md)。
