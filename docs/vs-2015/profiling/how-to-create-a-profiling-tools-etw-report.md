...
Windows 事件跟踪 (ETW) 报告列出 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 分析工具的性能会话中所记录的 ETW 事件。 在二进制 (.etl) 文件中收集 ETW 数据。 有关此报表的详细信息，请参阅 [Windows (ETW 事件跟踪) 报表](../profiling/event-tracing-for-windows-etw-report.md)。  
  
> [!NOTE]
> 不能在 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 的接口中显示 ETW 报告。  
  
- 有关如何使用的接口收集 ETW 数据的信息 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ，请参阅 [如何：收集 WINDOWS (Etw 事件跟踪) 数据](../profiling/how-to-collect-event-tracing-for-windows-etw-data.md)。  
