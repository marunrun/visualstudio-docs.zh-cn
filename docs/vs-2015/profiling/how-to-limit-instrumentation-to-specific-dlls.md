...
4. 在 "要 **分析以下哪些可用目标？**" 中，选择 .dll 项目的名称，然后单击 " **下一步**"。  
...
4. 在该向导的第二页上，执行下列步骤：  
    ...
    - 键入要在 **dll 路径**中分析的 .dll 文件的路径和文件名。 还可以单击省略号按钮 (...) 在“要分析的动态链接库”对话框中查找该文件。 请注意，必须指定接下来将选择的可执行 (.exe) 文件将启动的 .dll 文件的副本。  
  
    - 键入将在 **可执行路径**中执行 .dll 的可执行 () 文件的路径和文件名。 还可以单击省略号按钮 (...) 在“要启动的可执行文件”对话框中查找该文件。  
...
7. 可选。 若要添加更多 .dll 文件，请右键单击 " **目标** "，然后选择 " **添加目标二进制**文件"。 在“添加目标二进制文件”对话框中选择这些文件。  
