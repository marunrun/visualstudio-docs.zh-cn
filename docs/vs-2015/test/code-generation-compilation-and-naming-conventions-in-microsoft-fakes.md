...
## <a name="code-generation-and-compilation"></a><a name="BKMK_Code_generation_and_compilation"></a> 代码生成和编译

### <a name="configuring-code-generation-of-stubs"></a><a name="BKMK_Configuring_code_generation_of_stubs"></a>配置存根的代码生成
...
### <a name="type-filtering"></a><a name="BKMK_Type_filtering"></a> 类型筛选
...
### <a name="stubbing-concrete-classes-and-virtual-methods"></a><a name="BKMK_Stubbing_concrete_classes_and_virtual_methods"></a> 用作存根具体类和虚方法
...
### <a name="internal-types"></a><a name="BKMK_Internal_types"></a> 内部类型
 Fakes 代码生成器将为对生成的 Fakes 程序集可见的类型生成填充码类型和存根类型。 要使已填充的程序集的内部类型对 Fakes 和你的测试程序集可见，请将 <xref:System.Runtime.CompilerServices.InternalsVisibleToAttribute> 属性添加到向生成的 Fakes 程序集和测试程序集提供可见性的已填充的程序集代码中。 下面是一个示例：
...
```
 ...
 你可以为 Fakes 程序集指定不同的公钥，如为填充程序集创建的密钥，方法是在 **.snk** `KeyFile` `Fakes` \\ `Compilation` **Fakes**文件的元素中指定包含替代密钥的 .snk 文件的完整路径。 例如：
...
### <a name="optimizing-build-times"></a><a name="BKMK_Optimizing_build_times"></a>优化生成时间
...
### <a name="avoiding-assembly-name-clashing"></a><a name="BKMK_Avoiding_assembly_name_clashing"></a> 避免程序集名称冲突
...
## <a name="fakes-naming-conventions"></a><a name="BKMK_Fakes_naming_conventions"></a> Fakes 命名约定

### <a name="shim-type-and-stub-type-naming-conventions"></a><a name="BKMK_Shim_type_and_stub_type_naming_conventions"></a> 填充码类型和存根类型命名约定
 **命名空间**
...
### <a name="shim-delegate-property-or-stub-delegate-field-naming-conventions"></a><a name="BKMK_Shim_delegate_property_or_stub_delegate_field_naming_conventions"></a> 填充码委托属性或存根委托字段命名约定
 字段命名的**基本规则**，从空的名称开始：
...
- 除非存在重载多义性，否则将忽略**返回类型**。 如果是这样，将在名称末尾追加返回类型

### <a name="parameter-type-naming-conventions"></a><a name="BKMK_Parameter_type_naming_conventions"></a> 参数类型命名约定

|假定为|追加的字符串是…|
|-----------|-------------------------|
|一 **种类型**`T`|T<br /><br /> 已放置命名空间、嵌套结构和泛型 tic。|
|**Out 参数**`out T`|`TOut`|
|**ref 参数** `ref T`|`TRef`|
|**数组类型**`T[]`|`TArray`|
|**多维数组**类型 `T[ , , ]`|`T3`|
|**指针**类型 `T*`|`TPtr`|
|**泛型类型**`T<R1, …>`|`TOfR1`|
|类型**的泛型类型参数** `!i``C<TType>`|`Ti`|
|方法**的泛型方法参数** `!!i``M<MMethod>`|`Mi`|
|**嵌套类型**`N.T`|先追加 `N`，然后追加 `T`|

### <a name="recursive-rules"></a><a name="BKMK_Recursive_rules"></a> 递归规则
 下面的规则按递归方式应用：
