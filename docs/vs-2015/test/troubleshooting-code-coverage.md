...
## <a name="what-you-should-see"></a>你应该看到的结果
 如果在 "测试" 菜单上选择 " **分析代码覆盖率** " 命令，并且生成和测试成功运行，则应该在 "代码覆盖率" 窗口中看到结果的列表。 你可能必须展开项目以查看详细信息。
...
### <a name="pdb-symbol-files-are-unavailable"></a>.pdb（符号）文件不可用
 分析打开编译目标文件夹 (通常 bin\debug) ，并验证对于每个程序集，.dll 或 .exe 文件所在的目录中是否存在 .pdb 文件。

 解释代码覆盖率引擎要求每个程序集都有其关联的 .pdb 文件在测试运行期间可访问。 如果某个特定程序集没有 .pdb 文件，则不会分析该程序集。

 .pdb 文件必须从与 .dll 或 .exe 文件同一个生成中产生。

 解决方法确保生成设置生成 .pdb 文件。 如果在生成项目时未更新 .pdb 文件，则打开项目属性，选择 " **生成** " 页，选择 " **高级** "，然后检查 " **调试信息**"。
...
### <a name="using-an-instrumented-or-optimized-binary"></a>使用已检测或已优化的二进制文件
 分析确定二进制文件是否已完成任何形式的高级优化（如按配置优化），或者是否已由分析工具（如 vsinstr.exe 或 vsperfmon.exe）检测。
...
### <a name="custom-runsettings-file-with-incorrect-exclusions"></a>使用了错误排除的自定义 .runsettings 文件
 ...
 解决方法： `Include` 从 .runsettings 文件中删除所有节点，然后删除所有 `Exclude` 节点。 如果这样做修复了问题，请将这些节点分阶段放回去。
