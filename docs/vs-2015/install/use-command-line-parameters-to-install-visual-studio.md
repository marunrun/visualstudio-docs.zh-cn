...
有关 Visual Studio 的最新文档，请参阅 [使用命令行参数安装 Visual studio](/visualstudio/install/use-command-line-parameters-to-install-visual-studio)。

在命令提示符下安装 Visual Studio 2015 时，可以使用以下命令行参数（也称为开关）。

> [!NOTE]
> 请确保使用的是实际安装程序，而不是引导程序文件。 例如，请确保使用 **`vs_enterprise.exe`** 而不是 Vs_enterprise_*guid.empty*。 可以从 [My.VisualStudio.com](https://my.visualstudio.com/downloads?q=visual%20studio%20enterprise%202015)下载安装程序。
