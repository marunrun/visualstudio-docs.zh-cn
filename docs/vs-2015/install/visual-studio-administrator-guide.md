...
只要每台目标计算机满足[最低安装要求](https://visualstudio.microsoft.com/vs/older-downloads/)，就可以在网络上部署 Visual Studio 2015。 可以通过使用 /layout 开关运行安装文件（如[创建 Visual Studio 的脱机安装](../install/create-an-offline-installation-of-visual-studio.md)页所述），然后将其从本地计算机复制到网络共享来创建一个网络共享。 如果你使用的是 ISO，则可以安装 ISO 并共享它，或者将 ISO 复制到网络共享。  
...
|主题|描述|  
|-----------|-----------------|  
|[如何：安装特定版本的 Visual Studio](../install/how-to-install-a-specific-release-of-visual-studio.md)|描述如何安装 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]当前版本的特定配置。|  
|[如何：创建和运行 Visual Studio 的无人参与安装](../install/how-to-create-and-run-an-unattended-installation-of-visual-studio.md)|描述如何在无人参与模式下安装 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]。|  
|[如何：在部署 Visual Studio 时自动应用产品密钥](../install/how-to-automatically-apply-product-keys-when-deploying-visual-studio.md)|介绍如何在部署到多台计算机时应用产品密钥。|  
|[帮助查看器管理员指南](../ide/help-viewer-administrator-guide.md)|提供有关如何管理具有或没有 internet 访问权限的网络环境的本地帮助安装的信息。|  
|[安装 Visual Studio](../install/install-visual-studio-2015.md)|提供描述如何安装 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]的主题的说明和链接。|