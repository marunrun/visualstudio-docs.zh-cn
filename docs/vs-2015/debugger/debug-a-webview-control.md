---
title: 调试 Web 视图控件 |Microsoft Docs
ms.date: 11/15/2016
...
- 在 [JavaScript 控制台](../debugger/javascript-console-commands.md)窗口中选择 `WebView` 实例中显示为目标的网页或 `iFrame`，然后使用控制台命令与网页交互。 控制台提供对当前脚本执行上下文的访问。  
...
4. 对于非 Express 版本的 Visual Studio (可选) ，通过选择 " **工具**"、" **选项**"、" **调试**"、" **实时**"，然后禁用脚本的 JIT 调试来禁用实时 (JIT) 调试。  
