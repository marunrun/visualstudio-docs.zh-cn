...
1. 在下面的 c # 控制台应用程序中，Main 方法将在块中引发 **AccessViolationException** `try/catch` ：  
...
### <a name="setting-the-debugger-to-continue-on-user-unhandled-exceptions"></a><a name="BKMK_UserUnhandled"></a> 将调试器设置为在用户未经处理的异常上继续  
...
3. 可以为特定异常（选择异常，右键单击，然后选择/取消选择 **“在用户代码中未经处理时继续操作”**）或为整个类别的异常（例如，所有公共语言运行时异常）更改此设置。  
   ...
   ![用户&#45;分隔符异常时中断](../debugger/media/exceptionunhandledbyuser.png "ExceptionUnhandledByUser")  
...
> [!TIP]
> 请检查你的拼写！ " **异常设置** " 窗口不会检查是否存在添加的异常。 因此，如果键入 **sytem.uritemplatematchexception**，则将获得该异常 (而不是 **sytem.uritemplatematchexception**) 的条目。  
