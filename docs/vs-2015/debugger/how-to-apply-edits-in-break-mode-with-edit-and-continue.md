...
- 使用 "**附加到**" 调试应用程序，而不是使用 "**调试**" 菜单上的 "**开始**" 运行应用程序。  
  
- 调试优化后的代码。  
  
- 当目标为 64 位应用程序时，调试托管代码。 如果希望使用“编辑并继续”，必须将目标平台设置为 x86  (_项目_**属性**、" **编译** " 选项卡、" **高级编译器** " 设置。 ) 。  
...
2. 进行所需的并且合法的代码更改。  
     ...
     有关详细信息，请参阅 [Visual Basic "编辑并继续" 中不受支持的编辑](../debugger/unsupported-edits-in-visual-basic-edit-and-continue.md)。  
