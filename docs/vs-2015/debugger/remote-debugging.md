...
1. 在要调试的设备或服务器计算机上 (而不是运行 Visual Studio) 的计算机上，获取正确版本的远程工具。
    ...
    |Visual Studio 2015 (旧) |[远程工具](https://my.visualstudio.com/Downloads?q=remote%20tools%20visual%20studio%202015)|如果系统提示，请加入免费的 Visual Studio Dev Essentials 组，或者只需使用有效的 Visual Studio 订阅进行登录。 然后，如有必要，请重新打开该链接。|
...
2. 在 "下载" 页上，选择与你的操作系统 (x86、x64 或 ARM 版本) 的工具版本，然后下载远程工具。
...
如果尝试将远程调试器 ( # A0) 复制到远程计算机并运行它，请注意，仅当下载这些工具时才会安装 **远程调试器配置向导** (**rdbgwiz.exe**) ，如果你希望远程调试器作为服务运行，则更是如此。 有关详细信息，请参阅下面的 [ (可选) 将远程调试器配置为服务](#bkmk_configureService) 。

### <a name="to-run-the-remote-debugger-from-a-file-share"></a>从文件共享运行远程调试器

你可以在已安装 Visual Studio 2015 社区版、专业版或企业版的计算机上找到远程调试器 (**msvsmon.exe**) 。 在许多情况下，设置远程调试的最简单方法是从文件共享运行 ( # A0) 的远程调试器。 有关使用限制，请参阅远程调试器的 "帮助" 页 (远程调试器) 中的 " **帮助/用法** "。

1. 在目录中查找与你的 Visual Studio 版本匹配的 **msvsmon.exe** 。 对于 Visual Studio 2015：
...
3. 在远程计算机上，运行 **msvsmon.exe**。 请按照[设置说明](#bkmk_setup)操作。

> [!TIP] 
> 对于命令行安装和命令行参考，请在安装了 Visual Studio 的计算机上的命令行中键入，以查看 **msvsmon.exe** 的帮助页面， ``msvsmon.exe /?`` (或者在远程调试器) 中转到 " **帮助/用法** "。
...
1. 定位远程调试器应用程序。  (打开 "开始" 菜单并搜索 " **远程调试器**"。 ) 
    ...
    如果在远程服务器上运行远程调试器，则可以右键单击远程调试器应用程序，然后选择 "以 **管理员身份运行** " (或者，你可以将远程调试器作为服务运行) 。如果未在远程服务器上运行，只需正常启动。
  
2. 首次启动远程工具时 (或) 配置它之前，将显示 " **远程调试配置** 对话框" 对话框。  
  
    ![RemoteDebuggerConfWizardPage](../debugger/media/remotedebuggerconfwizardpage.png "RemoteDebuggerConfWizardPage")  
  
3. 如果未安装 Windows 服务 API (仅在 Windows Server 2008 R2) 上执行此操作，请选择 " **安装** " 按钮。  
...
6. 配置完成后，将显示远程调试器窗口。
   ...
   ** \<Visual Studio installation directory> \Common7\IDE\Remote 调试器 \\<x86、x64 或 Appx\msvsmon.exe**。  
...
- 若要使其他用户能够连接到远程调试器，请选择 " **工具"/"权限**"。 你必须拥有管理员特权才能授予或拒绝权限。
  ...
  > 可以在其他用户帐户（不同于你在 Visual Studio 计算机上使用的用户帐户）下运行远程调试器，但你必须将该用户帐户添加到远程调试器权限中。 
   ...
   或者，可以从命令行启动远程调试器， ** \<username> /allow**参数为： **msvsmon/allow \<username@computer> **。
...
7. 单击 **“完成”** 。  
   ...
   此时，远程调试器正作为服务运行。 可以通过转到 **"控制面板"/"服务"** 并查找 " **Visual Studio 2015 远程调试器**" 来对此进行验证。  
...
## <a name="remote-debug-an-aspnet-application"></a>远程调试 ASP.NET 应用程序  
 ...
 如果正在调试 ASP.NET Core 的应用，请参阅 [发布到 IIS](https://docs.asp.net/en/latest/publishing/iis.html)。 若要在 IIS 上发布 ASP.NET Core，需要执行不同的步骤。 成功发布 ASP.NET Core 应用后，可以 [像使用其他 ASP.NET 应用](../debugger/remote-debugging-aspnet-on-a-remote-iis-7-5-computer.md)一样对其进行远程调试，只不过需要附加到的进程是 dnx.exe 而不是 w3wp.exe。
...
11. 在 Visual Studio 计算机上，你应看到在断点处已停止执行。  
    ...
    如果具有需要由应用程序使用的非代码文件，则需要将其包含在 Visual Studio 项目中。 为 **解决方案资源管理器**中 (的其他文件创建项目文件夹，然后单击 " **添加/新建文件夹**"。 ) 然后将文件添加到 **解决方案资源管理器**中的文件夹 (，单击 "添加" **/"现有项**"，然后选择 ") "。 在每个文件的“属性”页中，将“复制到输出目录”设置为“始终复制”  。  
...
6. 选择 " **使用远程计算机**"，然后在文本框中键入 **MJO： 4020** 。  (4020 是) 的远程调试器窗口中显示的端口号。  
  
7. 请确保未选中“启用本机代码调试”。  
  
8. 生成项目。  
  
9. 在远程计算机上创建一个文件夹，该文件夹与 Visual Studio 计算机上的 "**调试**" 文件夹的路径相同： ** \<source path> \MyWPF\MyWPF\bin\Debug**。  
...
11. 请确保远程调试器正在目标计算机上运行。 如果未 (，请在 "**开始**" 菜单中搜索 "**远程调试器**"。 ) 远程调试器窗口将如下所示。  
...
## <a name="remote-debugging-on-windows-store-and-azure-apps"></a><a name="bkmk_winstoreAzure"></a> 在 Windows 应用商店和 Azure 应用上进行远程调试  
...
- [调试 Visual Studio 中的 .NET 后端](https://blogs.msdn.microsoft.com/azuremobile/2014/03/14/debugging-net-backend-in-visual-studio/)  
  
- 介绍如何在 Azure 网站上进行远程调试 ([第 1](https://azure.microsoft.com/blog/2014/05/06/introduction-to-remote-debugging-on-azure-web-sites/)部分、第 [2](https://azure.microsoft.com/blog/2014/05/07/introduction-to-remote-debugging-azure-web-sites-part-2-inside-remote-debugging/)部分、第 [3 部分](https://azure.microsoft.com/blog/2014/05/08/introduction-to-remote-debugging-on-azure-web-sites-part-3-multi-instance-environment-and-git/)) 。  
  
## <a name="see-also"></a>另请参阅  
 [在 Visual Studio 中调试](../debugger/debugging-in-visual-studio.md)   
 [配置 Windows 防火墙以进行远程调试](../debugger/configure-the-windows-firewall-for-remote-debugging.md)   
