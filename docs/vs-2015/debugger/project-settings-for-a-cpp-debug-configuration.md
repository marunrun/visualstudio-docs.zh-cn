...
可以在 " **属性页** " 对话框中更改 C 或 Visual C++ 调试配置的项目设置，如 [如何：设置调试和发布配置](../debugger/how-to-set-debug-and-release-configurations.md)中所述。 下表显示“属性页”对话框中与调试器有关的设置的位置。  
  
> [!WARNING]
> 用 c + + 编写的 Windows 应用商店应用程序和组件的 **配置属性/调试** 类别中的调试项目设置不同。 请参阅在 Windows 开发中心 [ (VB、c #、c + + 和 XAML) 启动调试会话 ](../debugger/start-a-debugging-session-for-a-store-app-in-visual-studio-vb-csharp-cpp-and-xaml.md) 。  
...
|**远程服务器名称**（远程 Windows 调试器）|指定您要在其上调试应用程序的计算机（不是您的计算机）的名称。<br /><br /> RemoteMachine Build 宏设置为此属性的值;有关详细信息，请参阅 [用于生成命令和属性的宏](https://msdn.microsoft.com/library/239bd708-2ea9-4687-b264-043f1febf98b)。|  
|**连接**（远程 Windows 调试器）|允许您在远程调试的标准与非身份验证连接类型之间切换。 在“远程服务器名称”框中指定远程计算机的名称。 连接类型包括：<br /><br /> -   带 Windows 身份验证的远程访问<br />-   **不带身份验证的远程 (仅限本机) **<br /><br /> 注意：不带身份验证的远程调试可能会使远程计算机容易受到安全侵犯。 Windows 身份验证模式更安全。<br /><br /> 有关详细信息，请参阅 [远程调试安装](../debugger/remote-debugging.md)。|  
