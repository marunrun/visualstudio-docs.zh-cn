...
“图形管道阶段”窗口可帮助了解 Direct3D 图形管道的每个阶段如何转换各个绘图调用。  
 ...
 ![3&#45;D 对象经过管道阶段。](../debugger/media/gfx-diag-demo-pipeline-stages-orientation.png "gfx_diag_demo_pipeline_stages_orientation")  
...
### <a name="vertex-shader-preview"></a>顶点着色器预览  
 在 " **图形管道阶段** " 窗口中选择 "顶点着色器" 阶段时，将显示 " **输入缓冲区** " 面板。 在其中可以找到有关提供给顶点着色器的顶点列表的详细信息（在它们经过输入装配器组合之后）。  
  
 ![顶点着色器阶段输入缓冲区查看器](../debugger/media/gfx-diag-vertex-shader-inbuffers.png "gfx_diag_vertex_shader_inbuffers")  
