...
#### <a name="timeline"></a>时间线  
 ...
 ![时间线显示了绘图调用开销。](../debugger/media/pix-frame-analysis-timeline.png "pix_frame_analysis_timeline")  
...
#### <a name="hot-draw-calls"></a>“热点”绘图调用  
 ...
 ![此 DrawIndexed 调用具有热和冷两种变量。](../debugger/media/pix-frame-analysis-hot-calls.png "pix_frame_analysis_hot_calls")  
...
#### <a name="statistical-significance"></a>统计意义  
 ...
 ![绘图调用变体的统计相关性](../debugger/media/pix-frame-analysis-summary-stats.png "pix_frame_analysis_summary_stats")  
...
#### <a name="platforms-that-support-hardware-counters"></a>支持硬件计数器的平台  
 ...
 ![受支持时会显示硬件计数器。](../debugger/media/pix-frame.png "pix_frame")  
...
|**点纹理筛选**|针对所有相应的纹理采样，将筛选模式设置为 `DXD11_FILTER_MIN_MAG_MIP_POINT`（点纹理筛选）。<br /><br /> 有关详细信息，请参阅[点、双线性、三线性和各向异性纹理筛选变体](../debugger/point-bilinear-trilinear-and-anisotropic-texture-filtering-variants.md)。|  
|**双线性纹理筛选**|针对所有相应的纹理采样，将筛选模式设置为 `DXD11_FILTER_MIN_MAG_LINEAR_MIP_POINT`（双线性纹理筛选）。<br /><br /> 有关详细信息，请参阅[点、双线性、三线性和各向异性纹理筛选变体](../debugger/point-bilinear-trilinear-and-anisotropic-texture-filtering-variants.md)。|  
|**三双线性纹理筛选**|针对所有相应的纹理采样，将筛选模式设置为 `DXD11_FILTER_MIN_MAG_MIP_LINEAR`（三线性纹理筛选）。<br /><br /> 有关详细信息，请参阅[点、双线性、三线性和各向异性纹理筛选变体](../debugger/point-bilinear-trilinear-and-anisotropic-texture-filtering-variants.md)。|  
|**各向异性纹理筛选**|针对所有相应的纹理采用，将筛选模式设置为 `DXD11_FILTER_ANISOTROPIC`，并将 `MaxAnisotropy` 设置为 `16`（16x 各向异性纹理筛选）。<br /><br /> 有关详细信息，请参阅[点、双线性、三线性和各向异性纹理筛选变体](../debugger/point-bilinear-trilinear-and-anisotropic-texture-filtering-variants.md)。|  
