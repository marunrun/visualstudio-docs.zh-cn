...
使用“调用堆栈”窗口可以查看当前堆栈上的函数或过程调用。  
 ...
 一个黄色箭头标识执行指针当前所位于的堆栈帧。 默认情况下，这是其信息显示在 "源"、" **反汇编**"、" **局部变量**"、" **监视** **" 和 "** 自动" 窗口中的框架。 如果要将上下文更改为堆栈上的另一个帧，可以在 " **调用堆栈** " 窗口中执行此操作。  
  
 当调试符号对部分调用堆栈不可用时，" **调用堆栈** " 窗口可能无法显示调用堆栈的正确信息。 将出现以下表示法：  
  
 [下面的帧可能不正确和/或缺失，没有为 name.dll 加载符号]  
  
 在托管代码中，默认情况下为。 " **调用堆栈** " 窗口将隐藏非用户代码的信息。 在隐藏信息处出现以下表示法：  
...
- 在 " **调试** " 菜单上，选择 " **Windows** "，然后单击 " **调用堆栈**"。  
  
### <a name="to-change-the-optional-information-displayed"></a>更改显示的可选信息  
  
- 右键单击 "**调用堆栈**" 窗口，然后设置或清除 "**显示 \<**_the information that you want_**> **"。  
...
1. 在“调用堆栈”窗口中，打开快捷菜单。 选择 **"在代码图上显示调用堆栈"**。  (键盘： **CTRL**  +  **SHIFT**  +  **`**)   
     ...
     请参阅 [在调试时映射调用堆栈上的方法](../debugger/map-methods-on-the-call-stack-while-debugging-in-visual-studio.md)。  
...
- 在 " **调用堆栈** " 窗口中，选择函数，右键单击并选择 " **运行到光标处**"。  
  
### <a name="to-set-a-breakpoint-on-the-exit-point-of-a-function-call"></a>在函数调用的退出点上设置断点  
  
- 请参阅[在调用堆栈函数处设置断点](../debugger/using-breakpoints.md#BKMK_Set_a_breakpoint_in_the_call_stack_window)。  
  
### <a name="to-load-symbols-for-a-module"></a>加载模块符号  
  
- 在 " **调用堆栈** " 窗口中，右键单击显示要重新加载其符号的模块的帧，然后选择 " **加载符号**"。  
...
## <a name="see-also"></a>另请参阅  
 ["调用堆栈" 窗口中的混合代码和缺失信息](../debugger/mixed-code-and-missing-information-in-the-call-stack-window.md)   
 [如何：更改调试器窗口的数值格式](https://msdn.microsoft.com/library/cd593847-a625-411d-a430-b798346ef18f)   
