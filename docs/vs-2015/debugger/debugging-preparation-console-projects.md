...
- C++ 控制台应用程序 (Win32)  
  ...
  同所有项目属性一样，这些参数将在调试会话之间和 Visual Studio 会话之间保留。 因此，如果控制台应用程序是以前调试过的应用程序，请记住，在 " ** \<Project> 属性页**" 对话框中输入以前会话中的自变量。  
  
  控制台应用程序使用“控制台”窗口接受输入以及显示输出消息。 若要向“控制台”窗口写入内容，则应用程序必须使用 Console 对象而不是 Debug 对象 。 若要向“Visual Studio 输出”窗口写入内容，请照常使用 Debug 对象。 确保知道应用程序正在写入的位置，否则可能在错误的位置中查找消息。 有关详细信息，请参见 [Console 类](https://msdn.microsoft.com/library/system.console.aspx)、[Debug 类](https://msdn.microsoft.com/library/system.diagnostics.debug.aspx)和[输出窗口](../ide/reference/output-window.md)。  
