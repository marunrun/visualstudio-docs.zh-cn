---
title: 切换 &lt;T &gt; 活动设计器 |Microsoft Docs
ms.date: 11/15/2016
...
---
# <a name="switchlttgt-activity-designer"></a>切换 &lt;T &gt; 活动设计器
<xref:System.Activities.Statements.Switch%601> 活动计算指定表达式并执行活动集合中其关联键与计算所得值匹配的活动。

 **切换 \<T >** 活动设计器用于在 [!INCLUDE[wfd1](../includes/wfd1-md.md)] 中创建和配置 <xref:System.Activities.Statements.Switch%601> 活动。
...
## <a name="how-to-use-the-switcht-activity-designer"></a>如何使用开关 \<T > 活动设计器
 可以在 "**工具箱**" 的 "**控制流**" 类别中找到 "**切换 \<T" >** 活动设计器，可通过单击 [!INCLUDE[wfd2](../includes/wfd2-md.md)] 上的 **"工具箱**" 选项卡（或者，从视图中选择 "**工具栏**"菜单，或按 CTRL + ALT + X。）将其放入 [!INCLUDE[wfd2](../includes/wfd2-md.md)] 后，它将显示 "**选择类型**" 对话框，让用户指定 1 活动中使用的泛型类型*t* 。 默认值为**Int32**。 选择泛型类型*T*后，会将 **\<T >** 设计器的 Switch 添加到工作流设计器中。
