...
ms.locfileid: "75851425"
---
# <a name="browse-and-select-a-net-type-dialog-box"></a>“浏览并选择 .NET 类型”对话框
当你选择 "**浏览类型 ...** " 时，在 "**属性**" 窗口、对话框或设计器（例如变量设计器） 在数据类型列表中，是 " **浏览并选择 .Net 类型** " 对话框 (以缩写形式引用为 "类型浏览器" ) 。 在此对话框中，可以从程序集和项目的树视图中选择类型。
...
> [!NOTE]
> 类型浏览器可以显示 Visual Basic 交错数组类型，但不显示多维数组类型。 有关详细信息，请参阅 [交错数组](https://msdn.microsoft.com/library/hkhhsz9t(VS.90).aspx) 和 [多维数组](https://msdn.microsoft.com/library/d2de1t93(VS.90).aspx) 。
...
2. 键入**Generic** <xref:System.GenericUriParser> 命名空间中的泛型匹配项，而不是类型 <xref:System.Collections.Generic> 。 若要在 <xref:System.Collections.Generic> 命名空间中搜索类型，请键入该命名空间的完全限定名称。
