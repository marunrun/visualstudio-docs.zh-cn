...
### <a name="using-the-rethrow-activity-designer"></a>使用 ReThrow 活动设计器
 重新**引发**活动设计器可在 "**工具箱**" 的 "**错误处理**" 类别中找到，可通过单击 (左侧的 "**工具箱**" 选项卡访问，也可以 [!INCLUDE[wfd2](../includes/wfd2-md.md)] 从 "**视图**" 菜单中选择 "**工具栏**" 或按 CTRL + ALT + X。 ) 

 重新 **引发** 活动设计器可以从 " **工具箱** " 拖放到 [!INCLUDE[wfd2](../includes/wfd2-md.md)] 图面上通常放置活动的任何位置，例如中 <xref:System.Activities.Statements.Sequence> 。 这将创建一个 <xref:System.Activities.Statements.Rethrow> 活动，其中包含 Throw 的默认 **DisplayName** 。 <xref:System.Activities.Activity.DisplayName%2A>可以在 "重新**抛**出" 活动设计器的标头中或在属性网格的 " **DisplayName** " 框中编辑值。
