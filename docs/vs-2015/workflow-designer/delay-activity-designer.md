...
### <a name="using-the-delay-activity-designer"></a>使用 Delay 活动设计器
 "**延迟**" 活动设计器可在 "**工具箱**" 的 "**基元**" 类别中找到，可通过单击 "[!INCLUDE[wfd2](../includes/wfd2-md.md)] 的"**工具箱**"选项卡（或者，从"**视图**"菜单中选择"**工具栏**"或按 CTRL+ ALT + X。）

 可以将 "**延迟**" 活动设计器从 "**工具箱**" 拖放到 [!INCLUDE[wfd2](../includes/wfd2-md.md)] 图面上通常放置活动的任何位置，如 <xref:System.Activities.Statements.Sequence> 内。 这将创建具有 Delay 的默认 <xref:System.Activities.Statements.Delay> 的 <xref:System.Activities.Activity.DisplayName%2A> 活动。 可以在 "**延迟**" 活动设计器的标头中或在属性网格的 " **DisplayName** " 框中编辑 <xref:System.Activities.Activity.DisplayName%2A>。
