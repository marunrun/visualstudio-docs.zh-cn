 ...
 **FlowDecision**设计器可在 "**工具箱**" 的 "**流程图**" 类别中找到，可通过单击 [!INCLUDE[wfd2](../includes/wfd2-md.md)] 上的 "**工具箱**" 选项卡（或者，从 "**视图**" 菜单中选择 "**工具栏**" 或按 CTRL +ALT + X。）

 可以将 " **FlowDecision** " 设计器从 "**工具箱**" 拖放到 "**流程图**" 活动设计器内的 [!INCLUDE[wfd2](../includes/wfd2-md.md)] 图面上。 这**会在 <xref:System.Activities.Statements.Flowchart> 活动中创建**一个标记为 "<xref:System.Activities.Statements.FlowDecision>" 的标记。 将鼠标悬停在设计器上，将显示两个分支的**True**和**False**方控点。
...
## <a name="see-also"></a>请参阅
 [流程图](../workflow-designer/flowchart-activity-designers.md) [：](../workflow-designer/flowchart-activity-designer.md) [FlowSwitch \<T >](../workflow-designer/flowswitch-t-activity-designer.md)