...
现在需要[安装 Python](https://www.python.org/download/)。 Python 由社区维护，其主页是 python.org。Continuum 分析生成名为 Anaconda 的免费捆绑包，其中包含 Python 和很多有用的库（特别是对于科学和数据处理），而 Enthought 会生成一个名为 Canopy 的类似的捆绑包。 你只需安装其中一个产品。 如果不能确定安装哪一个，请从 [Anaconda](https://www.anaconda.com/products/individual) 开始，它会为你提供最新的 Python 和大部分难以安装的包。
