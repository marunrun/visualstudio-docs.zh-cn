...
3. 将新创建的子菜单添加到 `<Menus>` 部分。  
    ...
    ```  
     ...
     父对象的 GUID/ID 对指定在 [将菜单添加到 Visual Studio 菜单栏](../extensibility/adding-a-menu-to-the-visual-studio-menu-bar.md)时生成的菜单组，并且是顶级菜单的子菜单。  
...
3. 添加 SubItemCallback ( # A1。 这是单击子菜单中的新命令时调用的方法。  
