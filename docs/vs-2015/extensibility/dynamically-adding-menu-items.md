...
1. 在 DynamicMenuPackageGuids.cs 中，添加命令集的 GUID 和命令 ID：  
...
1. 现在实现 match 谓词。 我们需要确定两项内容：第一项、命令 ID 是否有效 (它是否大于或等于声明的命令 ID) ; 其次，是否指定可能的项目 (它小于解决方案) 中的项目数。  
...
## <a name="setting-the-vspackage-to-load-only-when-a-solution-has-multiple-projects"></a>将 VSPackage 设置为仅在解决方案有多个项目时加载  
...
## <a name="testing-the-set-startup-project-command"></a>测试 "设置启动项目" 命令  
 现在可以测试代码了。  
