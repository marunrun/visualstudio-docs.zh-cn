...
### <a name="creating-a-topic-f1-support"></a> (F1 支持创建主题) 
...
- 对于 TOC，请添加该主题的节点顺序。 当父节点的子节点数为 n 时，将按子节点的位置定义。 例如，本主题是4个子主题的第4步。 ) 
 ...
 **主题正文**
 ...
 主体 (不包含主题的页眉和页脚) 将包含页面链接、附注部分、可折叠区域、代码片段和语言特定文本部分。  请参阅品牌部分，了解有关提供的主题的这些区域的信息。
 ...
 **F1 支持**
 ...
 在 Visual Studio 中，选择 F1 将生成从 IDE 中的光标位置提供的值，并使用提供的值 (基于游标位置来填充 "属性包"。 当游标超出功能 x 时，功能 x 是活动/焦点，并用值填充属性包。  选择 F1 后，将填充属性包，并且 Visual Studio F1 代码会查看客户默认帮助源是本地的还是联机的 (默认) 为 "联机"。然后，基于 "用户" 设置 " (联机" 的 "用户设置"，创建相应的字符串。 "shell execute" (请参阅 exe 启动参数的帮助管理员指南。如果本地帮助是默认值，则) 为属性包中的 "本地帮助查看器 + 关键字 (s 的参数"，或参数) 列表中包含关键字的 MSDN URL。
...
1. 在 Help 2.1 注册表项下设置 **VendorContent** 值：
...
- 在 \<locale> Help viewer 2.1 应用程序根中安装了 (Branding_ .mshc) 的后备署名包 (示例： C:\Program Files (x86) \Microsoft help Viewer\v2.1 通过 Help viewer 语言包) 。  这适用于未安装产品品牌包 (未安装任何内容) 或安装的品牌包损坏的情况。  使用应用根后备署名包时，将忽略 Visual Studio 元素 (徽标和反馈) 。
...
- 或者，当 SelfBranded = false 并且在 .MSHA 中定义了唯一的品牌包，并且在安装内容时可用
  ...
  Branding.xml 文件包含一个列表元素，这些元素用于在主题包含时一致地呈现主题中的特定项 \<meta name="Microsoft.Help.SelfBranded" content="false"/> 。  下面列出了 Branding.xml 文件中的 Visual Studio 元素列表。  此列表旨在用作 ISO Shell 用户的模板，在该模板中，他们修改这些元素 (例如徽标、反馈和版权) ，以满足其自己的产品品牌需求。
...
功能： **未安装内容控制** 使用：文本元素 (字符串) 用于呈现 contentnotinstalled.htm
...
功能： **主题找不到控件** 使用：文本元素 (字符串) 用于呈现 topicnotfound.htm
...
- 署名 css –包含用于呈现的 css 元素，其中 SelfBranded = false

- Printer –包含用于呈现的 css 元素，其中 SelfBranded = false

  署名 .css 文件包括 Visual Studio 中的 Visual Studio 主题演示文稿定义 (需要注意的是，包服务的 Branding_ 中包含的 \<locale> .mshc 可能会更改) 。
...
### <a name="deploying-a-set-of-topics"></a>部署一组主题
 ...
 本入门教程中，.MSHA 的名称是 Helpcontentsetup.msha。 .msha (文件的名称可以是任何扩展名为的文件。.MSHA) 。 下面) 的 Helpcontentsetup.msha (示例应包含 cab 或 MSHCs 列表。  文件类型必须在 .MSHA 中保持一致， (不支持) 的 .MSHA 和 CAB 文件类型组合。 对于每个 CAB 或 .MSHC，都应有 \<div class="package"> .。。\</div>  (参见下面) 的示例。
...
3. 将下面的 Helpcontentsetup.msha 创建为文本文件 (记事本用于创建文件) 并将其保存到前面所述的文件夹 (参见步骤 1) 。
   ...
   若要获取 Visual Studio 品牌包，请将 Branding_en-.mshc 文件复制到 C:\Program 文件 (x86) \Microsoft Help Viewer\v2.1\ 复制到工作文件夹。
...
2. [Visual Studio 2013 隔离 Shell 再发行](https://aka.ms/VS2013/IsoShell-LP/all)
...
3. [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] (Microsoft 下载) 获取 ISO Shell 可再发行组件。

4. 在 Visual Studio 中，创建一个基于独立 Shell 的帮助扩展，例如本演练稍后介绍的 Contoso 帮助扩展。

5. 将扩展和 ISO Shell 可再发行组件包装到部署 MSI (应用程序安装) 。 本演练不包含安装步骤。
...
- 创建一个名为 CatalogType.xml 的文件，并将其添加到该文件夹中。 该文件应包含以下代码行：
  ...
  在注册表中定义内容存储。 对于集成 Shell，请将 VisualStudio12 更改为产品目录名称：

- HKLM\SOFTWARE\Wow6432Node\Microsoft\Help\v2.1\Catalogs\VisualStudio12

   Key： LocationPath 字符串值： C:\ProgramData\Microsoft\HelpLibrary2\Catalogs\VisualStudio12\

- HKLM\SOFTWARE\Wow6432Node\Microsoft\Help\v2.1\Catalogs\VisualStudio12\en-US

   Key： CatalogName 字符串值： [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] 文档
...
8. 在要将 Contoso 部署到的计算机上，安装) ISO Shell 以上下载的 (。

9. 在 \\ \Program 文件 (x86) 中创建文件夹 \\ ，并将其命名为 `Contoso` 。

10. 将 ContosoHelpShell release 文件夹中的内容复制到 \\ \Program 文件 (x86) \contoso\ 文件夹中。
...
13. 创建 CatalogType.xml 并添加到内容存储 (上一步骤) 包含：
...
14. 添加以下注册表项：
     ...
     Key： CatalogName 字符串值： [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] 文档。 对于 ISO Shell，这是你的目录的名称。

15. 将 (cab 或 .MSHC 和 .MSHA) 的内容复制到本地文件夹。

16. 用于测试内容存储的集成 Shell 命令行示例。 对于 ISO Shell，请根据需要更改目录和 launchingApp 值以匹配产品。

      "C:\Program Files (x86) \Microsoft 帮助 Viewer\v2.1\HlpViewer.exe"/catalogName VisualStudio12/helpQuery method = "page&id = ContosoTopic0"/launchingApp Microsoft，VisualStudio，12。0

17. 从 Contoso 应用根) 启动 Contoso 应用程序 (。 在 ISO Shell 中，选择 " **帮助** " 菜单项，然后将 " **设置帮助首选项** " 更改为 " **使用本地帮助**"。

18. 在 shell 中，选择 " **帮助** " 菜单项，然后单击 " **查看帮助**"。 本地帮助查看器应启动。 选择 " **管理内容** " 选项卡。在 " **安装源**" 下，选择 " **磁盘** " 选项按钮。 选择 " **...** " 按钮，并浏览到包含 Contoso 内容的本地文件夹， (复制到上述步骤) 中的本地文件夹。 选择 Helpcontentsetup.msha. .msha。 Contoso 现在应显示为书籍中的一本书。 选择 " **添加**"，然后选择)  (右下角的 " **更新** " 按钮。

19. 在 Contoso IDE 中，选择 F1 键来测试 F1 功能。

### <a name="additional-resources"></a>其他资源

有关运行时 API，请参阅 [Windows 帮助 API](https://msdn.microsoft.com/library/windows/desktop/hh447318\(v=vs.85\).aspx)。
