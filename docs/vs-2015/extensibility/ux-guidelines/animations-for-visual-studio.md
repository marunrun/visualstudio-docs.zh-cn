...
- 在较小的空间中使用少量移动 (移动) ，喜欢淡化和更改移动对象。

- 使用在大范围屏幕房地产上发生的动画。 无论大小如何，这种形式的动画都将分散给用户。

- 使用与用户当前所关注的对象相关或与之交互的动画。

- 使用需要用户交互的动画来重置状态，例如强制用户响应闪烁的通知，使其停止闪烁。 以任何方式与它们进行交互应该足以消除它们。
...
- 补偿需要更长时间的转换的一种方法是将其分为两部分;例如，动画的第一部分可以是空内容容器 (最多500毫秒) 后跟容器的内容淡入到容器 (最多500毫秒) 。

- 对于可以计算的加载时间，首选的进度指示器 (按百分比完成进度指示器) 。

- 对于无法计算的加载时间， (加载或工作指示器) 的繁忙指示器（如光标或嵌入旋转动画）是合适的。

### <a name="animation-as-communicator"></a>作为 communicator 的动画
 在 Visual Studio 的 UI 中，动画仅用作一种通信工具。  它用于传递多种信息，如 UI 中的结构更改;例如，当菜单打开或关闭时。 动画可帮助直观显示复杂系统的时间相关行为，如安装进度可视化或用于吸引警报和通知。

 UI 动画的工作方式通常有四种：可视化、吸引关注、模拟并指示响应时间/进度。
...
- 使用时间滑块、角拐和往复式滑轨以及传输控制 (播放、停止和暂停) ，以时间表示更改。
...
#### <a name="attract-attention"></a>吸引关注
 如果目标是将用户的注意力绘制到多个元素之外，或者为了提醒用户更新信息，则可能会有合适的动画。 例如，应用程序起始页可能会使用入门按钮，该按钮在页面加载后将滑入到位。
...
### <a name="overview"></a>概述
 Visual Studio 中的动画旨在为特定函数提供服务，而不会妨碍用户工作效率。 要遵循的一般动画特征包括：
...
- 宽松，不十万火急
  ...
  下图显示了建议在 Visual Studio 中使用的动画样式。 最常使用的动画和细小动画（如淡入/淡出）是最常用的。 移动动画的应用程序（如展开和收缩、X 和 Y 位置更改和旋转）会受到限制。
...
#### <a name="appear-and-disappear"></a>显示并消失
 使用此模式时，元素可以在无需过渡动画的情况下切换到无视图视图，并返回回来：

 ![在 Visual Studio 中显示&#47;消失动画](../../extensibility/ux-guidelines/media/1202-b-appearanddisappear.png "1202-b_AppearAndDisappear")

##### <a name="correct-usage"></a>正确使用
 需要立即显示或消失的全新 UI 元素，以便用户既不分散也不会受到阻碍。 此外，缓慢的运动动画可能会被视为性能拖动，这不会随显示和消失的样式而发生。
...
#### <a name="fade-in-and-fade-out"></a>淡入和淡出
 ...
 ![在 Visual Studio 中淡化&#45;&#47;淡化&#45;out 动画](../../extensibility/ux-guidelines/media/1202-c-fadeinfadeout.png "1202-c_FadeInFadeOut")

##### <a name="correct-usage"></a>正确使用
 这是最常推荐的 UI 动画。 这是一种微妙的影响，它增加了不中断流的兴趣。 在某些情况下，用户甚至可能不会意识到有动画，只需感觉平滑的 UI 系统。
...
#### <a name="color-blend-from-a-to-b"></a>从 A 到 B 的颜色混合
 ...
 ![Visual Studio 中的颜色混合动画](../../extensibility/ux-guidelines/media/1202-d-colorblend.png "1202-d_ColorBlend")
...
#### <a name="expand-and-contract"></a>展开和收缩
 ...
 ![在 Visual Studio 中展开&#47;约定动画](../../extensibility/ux-guidelines/media/1202-e-expandcontract.png "1202-e_ExpandContract")
...
#### <a name="x-y-position-change"></a>X Y 位置更改
 ...
 ![X&#47;Y 位置更改动画在 Visual Studio 中](../../extensibility/ux-guidelines/media/1202-f-xypositionchange.png "1202-f_XYPositionChange")
...
- 持续时间：0秒
  ...
  ![Visual Studio 中的选项卡打开动画](../../extensibility/ux-guidelines/media/1202-h-tabopen.png "1202-h_TabOpen")
...
- 持续时间：200毫秒
  ...
  ![Visual Studio 中的选项卡关闭动画](../../extensibility/ux-guidelines/media/1202-i-tabclose.png "1202-i_TabClose")
...
- 持续时间：200毫秒
  ...
  ![Visual Studio 中的选项卡重新排序动画](../../extensibility/ux-guidelines/media/1202-j-tabreorder.png "1202-j_TabReorder")
...
- 持续时间：200毫秒
  ...
  ![Visual Studio 中的关闭浮动文档动画](../../extensibility/ux-guidelines/media/1202-k-closefloatingdocument.png "1202-k_CloseFloatingDocument")
...
- 持续时间：200毫秒
  ...
  ![Visual Studio 中的窗口状态过渡动画](../../extensibility/ux-guidelines/media/1202-l-windowstatetransition.png "1202-l_WindowStateTransition")
...
- 持续时间：200毫秒
  ...
  ![Visual Studio 中的菜单打开动画](../../extensibility/ux-guidelines/media/1202-m-menuopen.png "1202-m_MenuOpen")
...
- 持续时间：200毫秒
  ...
  ![Visual Studio 中的菜单关闭动画](../../extensibility/ux-guidelines/media/1202-n-menuclose.png "1202-n_MenuClose")
...
- 持续时间：0秒
  ...
  ![自动&#45;隐藏 Visual Studio 中的工具窗口动画](../../extensibility/ux-guidelines/media/1202-o-autohidetoolwindowreveal.png "1202-o_AutoHideToolWindowReveal")
