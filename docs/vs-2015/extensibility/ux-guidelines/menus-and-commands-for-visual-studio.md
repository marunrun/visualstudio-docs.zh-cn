...
### <a name="overview"></a>概述
 ...
 当用户的上下文发生更改时（例如从设计窗口切换到代码编辑窗口），与新上下文无关的功能将消失。 同时，新功能与相关动态信息（如属性和工具箱选项）一起显示。 用户不应注意到可用命令集的交换。 如果用户被显示或消失，则需要调整 UI 设计。 用户的当前上下文始终以一种或多种方式表示，例如在 IDE 标题栏、属性窗口或 "属性页" 对话框中。
...
- 飞出菜单是可接受的，但不得超过一个级别–从不使用级联浮出控件。
...
- 如果限定符产生多个有用的命令，请使用存储最后一个设置的拆分按钮：
     ...
     ![Visual Studio 中的“拆分”按钮](../../extensibility/ux-guidelines/media/0501-c-splitbuttons.png "0501-c_SplitButtons")
...
- 命令是一种专用命令，用户可能会使用 **"自定义 ..."** 将其添加到工具栏 对话框中的字段。
