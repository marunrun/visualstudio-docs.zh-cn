...
### <a name="overview"></a>概述
 Visual Studio 中使用的两种主要窗口类型是文档编辑器和工具窗口。 很少但可能是大的无模式对话框。 尽管这些在 shell 中都是无模式的，但是它们的模式在本质上是不同的。 本主题介绍文档窗口、工具窗口和无模式对话框之间的差异。 [对话框中介绍](../../extensibility/ux-guidelines/application-patterns-for-visual-studio.md#BKMK_Dialogs)了模式对话框模式。

### <a name="comparing-window-usage-patterns"></a>比较窗口使用模式
 **文档窗口** 几乎始终显示在文档中。 这为文档编辑器提供了一个 "中心阶段"，用于排列补充工具窗口。

 **工具窗口**最常显示为一个单独的小窗口，该窗口可以是可见、隐藏或自动隐藏的（针对 IDE 的边缘折叠）。 但是，有时会通过取消选中窗口上的 " **窗口"/"停靠** " 属性，在文档中显示它们。 这将导致更多的房地产，同时也是一种常见的设计决策：尝试集成到 Visual Studio 时，您必须确定您的功能是否应显示工具窗口或文档窗口。

 Visual Studio 中不鼓励有**无模式对话框**。 在很大程度上，它们是按定义–浮动工具窗口，应这样实现。 如果固定到 shell 一侧的正常工具窗口的大小过大限制，则允许无模式对话框。 在用户可能会将对话框移至辅助监视器的情况下，也可以使用它们。
...
|**Instances**|*多实例*<br /><br /> 可以同时打开多个编辑器和编辑不同的文件，而某些编辑器还允许在多个编辑器中打开相同的文件 (使用 **窗口 > 新的窗口** 命令) "。<br /><br /> 单个编辑器可以同时编辑一个或多个文件， (项目设计器) 。|*单实例或多实例*<br /><br /> 内容将更改以反映上下文 (如属性浏览器中所示) 或将焦点/上下文推送到其他 windows (任务列表解决方案资源管理器) 。<br /><br /> 单实例工具窗口和多实例工具窗口都应与活动文档窗口相关联，除非有很有说服力的原因不是。|*单实例*|
...
|类型|工具窗口|函数|
|----------|-----------------|--------------|
|**层次结构**|解决方案资源管理器|显示项目、杂项文件和解决方案项中包含的文档列表的层次结构树。 项目中项的显示由拥有项目类型的包定义 (例如，基于引用、基于目录或混合模式的类型) 。|
|**层次结构**|类视图|类和文档工作集中各个元素的层次结构树，独立于文件本身。|
|**层次结构**|服务器资源管理器|显示解决方案中所有服务器和数据连接的层次结构树。|
|**层次结构**|文档大纲|活动文档的层次结构。|
|**网格**|属性|该网格显示所选对象的属性列表，以及值选取器来编辑这些属性。|
|**网格**|任务列表|允许用户创建/编辑/删除任务和注释的网格。|
|**内容**|帮助|允许用户从 "如何实现" 访问获得帮助的各种方法的窗口 视频到 MSDN 论坛。|
|**内容**|动态帮助|显示适用于当前所选内容的帮助主题的链接的工具窗口。|
|**内容**|对象浏览器|一个包含两列的框架集，其中包含左窗格中的分层对象组件列表，以及右侧列中对象的属性和方法。|
|**对话框**|查找，高级查找|此对话框允许用户在解决方案内的各种文件中查找或查找和替换。|
|**其他**|工具箱|用于存储将放置到设计图面上的元素的工具窗口，为所有设计器提供一致的拖放。|
|**其他**|起始页|用户使用 Visual Studio 的门户，可以访问开发人员新闻、Visual Studio 帮助和最新项目的源。 用户还可以通过将 StartPage .xaml 文件从 "Common7\IDE\StartPages\" Visual Studio program files 目录复制到 Visual Studio 文档目录中的 StartPages 文件夹来创建自定义起始页，然后在 Visual Studio 或其他代码编辑器中手动编辑或打开它。|
|**调试器：** 专门用于调试任务和监视活动的一组 windows|自动||
|**调试器：** 专门用于调试任务和监视活动的一组 windows|即时||
|**调试器：** 专门用于调试任务和监视活动的一组 windows|输出|如果要声明文本事件或状态，则可以使用 "输出" 窗口。|
|**调试器：** 专门用于调试任务和监视活动的一组 windows|内存||
|**调试器：** 专门用于调试任务和监视活动的一组 windows|断点||
|**调试器：** 专门用于调试任务和监视活动的一组 windows|运行||
|**调试器：** 专门用于调试任务和监视活动的一组 windows|文档||
|**调试器：** 专门用于调试任务和监视活动的一组 windows|调用堆栈||
|**调试器：** 专门用于调试任务和监视活动的一组 windows|局部变量||
|**调试器：** 专门用于调试任务和监视活动的一组 windows|监视||
|**调试器：** 专门用于调试任务和监视活动的一组 windows|反汇编||
|**调试器：** 专门用于调试任务和监视活动的一组 windows|寄存器||
|**调试器：** 专门用于调试任务和监视活动的一组 windows|线程数||
 ...
 **在 Visual Studio 中，创建强名称密钥是一个简单对话框的示例。**
...
|-------------------------|------------------------------------|-----------------------------------------|
|Tab 控件|逻辑分组对话框页面到相关集<br /><br /> 在对话框中的相关控件) 页上，在对话框中包含的列数不超过五个 (或制表符<br /><br /> 选项卡标签必须为 short：一个或两个单词，可轻松地识别内容<br /><br /> 常见系统对话框样式<br /><br /> 示例： **文件资源管理器 > 项属性**|可能很难进行描述性简短标签<br /><br /> 通常不会在一个对话框中缩放过去的五个选项卡<br /><br /> 如果一个行的选项卡太多，则不合适;使用备用分层方法<br /><br /> 不可扩展|
|边栏导航|可以容纳超过选项卡的多个类别的简单切换设备<br /><br /> 不 (层次结构的类别的简单列表) <br /><br /> 可扩展<br /><br /> 示例： **自定义 ... > Add 命令**|如果少于三个组，则不能使用水平空间。<br /><br /> 任务可能更适合下拉|
...
|向导|通过遍历基于任务的顺序步骤来帮助完成任务。 该向导表示高级任务，而各个面板代表完成总体任务所需的子任务。<br /><br /> 当任务跨越 Ui 边界时非常有用，因为当用户需要使用多个编辑器和工具窗口完成任务时，<br /><br /> 当任务需要分支时有用<br /><br /> 当任务包含步骤间的依赖关系时有用<br /><br /> 在一个对话框中显示多个具有一个决策分叉的类似任务以减少不同的相似对话框数时，此方法非常有用。|不适用于不需要顺序工作流的任何任务<br /><br /> 如果向导执行的操作过多，可能会使用户感到不知所措和混淆<br /><br /> 向导在本质上是有限的屏幕|
 ...
 **在 Outlook 中公开附加 UI 的走廊概念**

##### <a name="adaptive-ui"></a>自适应 UI
 显示或隐藏基于使用情况或用户的自报告体验的 UI 是在隐藏其他部分的同时提供必要 UI 的另一种方法。 在 Visual Studio 中不建议这样做，因为用于决定何时显示或隐藏 UI 的算法可能比较棘手，并且对于某些事例集，规则将始终错误。
...
- 基于引用和混合的项目系统支持添加链接 (或引用) 到源项的概念。 当这些项目是拖放操作的目标时 (在按住 **Ctrl + Shift**) 时，它应导致对要添加到项目中的项的引用。
  ...
  因为基于目标引用的项目不应创建源项的独立副本，所以它还会误导您在这些类型的项目之间允许复制操作。 同样，不应允许使用 Ctrl + Shift 拖动到基于目录的目标项目，因为基于目录的项目无法保留引用。 在不支持拖放操作的情况下，IDE 应禁止放置，并向用户显示 "不删除" 光标 (如下) 中的指针表中所示。

  为了正确实现拖放行为，拖放操作的源项目需要传达其性质 (例如，是基于引用还是基于目录的） ) 目标项目。 此信息由源提供的剪贴板格式指示。 作为拖动 (或剪贴板复制操作的源) 项目应分别提供 **CF_VSREFPROJECTITEM**或 **CF_VSSTGPROJECTITEMS** ，具体取决于该项目是基于引用还是基于目录。 这两种格式都具有相同的数据内容，这与 Windows **CF_HDROP**格式类似，不同之处在于，字符串列表（而不是文件名）是**Projref**字符串的双**精度结尾列表**， (根据适当的) 从**IVsSolution：： GetProjrefOfItem**或 **： GetProjrefOfProject**返回。

  作为) 拖 (放操作的目标，项目应同时接受 **CF_VSREFPROJECTITEMS** 和 **CF_VSSTGPROJECTITEMS**，尽管拖放操作的具体处理方式因目标项目和源项目的性质而异。 源项目是否通过提供 **CF_VSREFPROJECTITEMS** 或 **CF_VSSTGPROJECTITEMS**来声明其本质。 Drop 的目标理解自己的性质，因此具有足够的信息来决定是否应执行移动、复制或链接。 用户还可以通过按 ctrl、Shift 或同时按 Ctrl 键和 Shift 键来修改应执行的拖放操作。 放置目标正确指示将在其 **system.windows.dragdrop.dragenter>** 和 **拖动** 方法中提前执行的操作非常重要。 **解决方案资源管理器**会自动知道源项目和目标项目是否为同一个项目。
...
|鼠标指针|命令|说明|
|-------------------|-------------|-----------------|
|![鼠标“不放下”图标](../../extensibility/ux-guidelines/media/0706-01-mousenodrop.png "0706-01_MouseNoDrop")|不删除|无法将项拖放到指定位置。|
|![鼠标“复制”图标](../../extensibility/ux-guidelines/media/0706-02-mousecopy.png "0706-02_MouseCopy")|复制|项将被复制到目标位置。|
|![鼠标“移动”图标](../../extensibility/ux-guidelines/media/0706-03-mousemove.png "0706-03_MouseMove")|移动|项将移动到目标位置。|
...
|||源项：引用/链接|源项：物理项或文件系统 (CF_HDROP) |
|-|-|----------------------------------|-------------------------------------------------------------|
|No 修饰符|操作|移动|链接|
|No 修饰符|目标|添加对原始项的引用|添加对原始项的引用|
|No 修饰符|Source|删除对原始项的引用|保留原始项|
|No 修饰符|结果|**DROPEFFECT_MOVE** 作为以下操作返回 **：:D rop** ，项仍保留在存储中的原始位置|**DROPEFFECT_LINK** 作为以下操作返回 **：:D rop** ，项仍保留在存储中的原始位置|
|Shift + 拖动|操作|移动|不删除|
|Shift + 拖动|目标|添加对原始项的引用|不删除|
|Shift + 拖动|Source|删除对原始项的引用|不删除|
|Shift + 拖动|结果|**DROPEFFECT_MOVE** 作为以下操作返回 **：:D rop** ，项仍保留在存储中的原始位置|不删除|
|Ctrl + 拖动|操作|复制|不删除|
