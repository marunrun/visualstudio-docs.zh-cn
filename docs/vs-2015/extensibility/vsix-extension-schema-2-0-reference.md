...
## <a name="package-manifest-schema"></a>包清单架构
 清单 XML 文件的根元素为，其中 `<PackageManifest>` 单个属性为 `Version` 清单格式的版本。 如果对该格式进行了重大更改，版本格式将会更改。 本主题介绍清单格式版本2.0，该格式是通过将 `Version` 属性设置为值 version = "2.0" 在清单中指定的。
...
- `<Identity>` -此项定义此包的标识信息，并包括以下属性：
  ...
  - `Language` –此属性是包的默认语言，并对应于此清单中的文本数据。 此属性遵循资源程序集的 CLR 区域设置代码约定，例如 en-us、en、fr。 您可以指定 `neutral` 来声明将在任何 Visual Studio 版本上运行的非特定语言的扩展。 默认值是 `neutral`。
...
- `InstalledByMsi` –此可选特性指定是否由 MSI 安装此包。 MSI 安装的包由 MSI (程序和功能进行安装和管理，而不是由 Visual Studio 扩展管理器使用) 。  默认情况下，此属性为 "false"，指定 MSI 不安装包。
...
- `<InstallationTarget>` –此元素控制 VSIX 安装程序安装包的位置。 如果该属性的值 `Scope` 为 "ProductExtension"，则包必须面向安装了清单文件作为其内容的一部分的 SKU，以将其可用性播发到扩展。 `<InstallationTarget>`当 `Scope` 特性具有显式或默认值 "ProductExtension" 时，元素具有以下特性：
...
- `<Dependency>` 元素–此子元素具有以下属性：
  ...
  - `Id` –此属性必须为依赖包的唯一 ID。 此标识值必须与 `<Metadata><Identity>Id` 此包所依赖的包的属性相匹配。 `Id`属性遵循命名空间约定： Company.Product.Feature.Name。 属性只能包含字母数字字符，且限制为100个字符。
...
- `<Asset>` -此元素包含以下属性和元素：
  ...
  - `Type` –这是此元素表示的扩展或内容类型。 每个 `<Asset>` 元素必须具有单个 `Type` ，但多个 `<Asset>` 元素可能具有相同的 `Type` 。 根据命名空间约定，此特性应表示为完全限定名称。 已知类型为：
