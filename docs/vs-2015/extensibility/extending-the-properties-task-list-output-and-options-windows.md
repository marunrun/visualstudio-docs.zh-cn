---
title: 扩展 "属性"、"任务列表"、"输出" 和 "选项" 窗口 |Microsoft Docs
ms.date: 11/15/2016
...
2. ProvideOptionPage 构造函数的第一个参数是之前创建的 ToolsOptions 类的类型。 第二个参数 "ToDo" 是 " **选项** " 对话框中的类别名称。 第三个参数 "General" 是 " **选项** " 对话框中可用的子类别的名称。 接下来的两个参数是字符串的资源 Id;第一个是类别的名称，第二个是子类别的名称。 最终参数确定是否可以使用自动化来访问此页。  
...
## <a name="make-data-available-to-the-properties-window"></a>使数据可用于 "属性" 窗口  
 您可以通过创建一个名为 TodoItem 的类来创建一个名为 "" 的类，以便在 ToDo 列表中存储各项的相关信息。  
...
2. 将以下 using 语句添加到 TodoItem.cs 文件。  
...
3. 将 `public` 访问修饰符添加到类声明中。  
    ...
    ```  
     ...
     添加这两个属性，即 Name 和 DueDate。 稍后我们将 UpdateList ( # A1 和 CheckForErrors ( # A3。  
...
6. 在 TodoWindowControl.xaml.cs 中，将存根方法添加到和方法的 TodoWindowControl 类 `CheckForError` `UpdateList` 。 将其放在文件 System.windows.forms.control.processdialogchar 之后和末尾之前。  
...
## <a name="integrate-into-the-properties-window"></a>集成到 "属性" 窗口  
 现在，编写管理 ListBox 的代码，这些代码将绑定到 " **属性** " 窗口。  
  
 必须更改按钮单击处理程序以读取文本框，创建 TodoItem，并将其添加到 ListBox。  
  
1. `button1_Click`使用创建新 TodoItem 的代码替换现有函数，并将其添加到 ListBox。 它会调用 TrackSelection ( # A1，稍后将定义该编号。  
...
3. 实现 TrackSelection ( # A1 方法。 由于需要获取 <xref:Microsoft.VisualStudio.Shell.Interop.SVsUIShell> <xref:Microsoft.VisualStudio.Shell.Interop.STrackSelection> 服务，因此需要 <xref:Microsoft.VisualStudio.Shell.WindowPane.GetService%2A> TodoWindowControl 可访问的服务。 将以下方法添加到 TodoWindow 类：  
...
4. 将以下 using 语句添加到 TodoWindowControl.xaml.cs：  
...
## <a name="add-text-to-the-output-window-and-items-to-the-task-list"></a>向输出窗口添加文本，并将项添加到任务列表  
 对于 **任务列表**，你将创建一个类型为 Task 的新对象，然后通过调用其 add 方法将该任务对象添加到 **任务列表** 中。 若要写入 " **输出** " 窗口，请调用其 GetPane 方法以获取窗格对象，然后调用窗格对象的 OutputString 方法。  
...
2. 若要将项添加到任务列表中，需要将嵌套类添加到 TodoWindowControl 类。 嵌套类需要从派生 <xref:Microsoft.VisualStudio.Shell.TaskProvider> 。 将以下代码添加到 TodoWindowControl 类的末尾。  
...
3. 接下来，将对 TodoTaskProvider 和 CreateProvider ( # A1 方法的私有引用添加到 TodoWindowControl 类。 代码应如下所示：  
...
4. 添加 ClearError ( # A1，这会清除任务列表，将 ReportError 添加任务列表到 TodoWindowControl 类的 ( # A3。  
...
4. 现在，将 "工具" **/"选项"/"待办事项** " 页上的设置从 **2** 改回 **0**。  
