...
- 创建绑定到工作簿中的数据集的 <xref:Microsoft.Office.Tools.Excel.ListObject>，并在打开工作簿时用数据填充 <xref:Microsoft.Office.Tools.Excel.ListObject>。
...
2. 将 `Sheet1_Startup` 事件处理程序替换为以下代码。 此代码使用**AdventureWorksDataSet**项目中定义的 `ProductTableAdapter` 类的实例，以使用数据填充缓存的数据集（如果它当前为空）。
...
1. 在**解决方案资源管理器**中，右键单击**AdventureWorksReport**项目，选择 "**调试**"，然后单击 "**启动新实例**"。
    ...
    - <xref:Microsoft.Office.Tools.Excel.ListObject> 填充数据。

    - <xref:Microsoft.Office.Tools.Excel.ListObject> 第一行的 " **ListPrice** " 列中的值为1431.5。 稍后在本演练中，你将使用控制台应用程序来修改**ListPrice**列中的值。
...
## <a name="change-data-in-the-cached-dataset-by-using-the-console-application"></a>使用控制台应用程序更改缓存数据集中的数据
 使用控制台应用程序中的 <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument> 类将数据读入本地 `AdventureWorksLTDataSet` 对象中，修改此数据，然后将其保存回缓存的数据集。
...
8. 将以下代码添加到 `Main` 方法中。 此代码声明了下列对象：
   ...
   - **AdventureWorksDataSet**项目中定义的 `AdventureWorksLTDataSet` 类型的实例。
...
## <a name="test-the-workbook"></a>测试工作簿
 打开工作簿时，<xref:Microsoft.Office.Tools.Excel.ListObject> 现在会显示对缓存数据集中的数据的 `ListPrice` 列所做的更改。
...
3. 验证 <xref:Microsoft.Office.Tools.Excel.ListObject> 的第一行的 " **ListPrice** " 列中的值现在为1574.65。
