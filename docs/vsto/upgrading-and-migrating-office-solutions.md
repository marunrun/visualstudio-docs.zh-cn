...
# <a name="upgrade-and-migrate-office-solutions"></a>升级和迁移 Office 解决方案
  如果你拥有在 Visual Studio 早期版本中创建的 Microsoft Office 项目，则必须升级该项目以在 Visual Studio 当前版本中使用。 若要升级 Microsoft Office 项目，请在包含 Microsoft Office 开发人员工具的 Visual Studio 版本中打开该项目。 有关 Visual Studio 版本（包括 Microsoft Office 开发人员工具）的详细信息，请参阅[配置计算机以开发 Office 解决方案](../vsto/configuring-a-computer-to-develop-office-solutions.md)。
...
## <a name="changes-to-upgraded-projects"></a>对升级项目的更改
 升级 Microsoft Office 项目时，Visual Studio 会将项目修改为面向以下各项：

- 用于 Office 运行时的可视化工作室 2010 工具。 有关详细信息，请参阅适用于[Office 运行时的可视化工作室工具概述](../vsto/visual-studio-tools-for-office-runtime-overview.md)。
...
- [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)]中的程序集。 有关这些程序集的详细信息，请参阅适用于[Office 运行时的可视化工作室工具概述](../vsto/visual-studio-tools-for-office-runtime-overview.md)。
...
> [!NOTE]
> 你可能需要执行一些附加步骤以在开发和最终用户计算机上运行重定目标的解决方案，并且如果你的项目使用了某些功能，它将无法再编译。 有关详细信息，请参阅将[Office 解决方案迁移到 .NET 框架 4 或更高版本](../vsto/migrating-office-solutions-to-the-dotnet-framework-4-or-later.md)。
...
## <a name="targeted-office-application"></a>目标办公室应用程序
...
> [!NOTE]
> 如果将 VSTO 外接程序项目升级到目标[!INCLUDE[Office_15_short](../vsto/includes/office-15-short-md.md)]或更高版本，请确保 VSTO`ThisAddIn_Startup`外接程序的事件处理程序不包含访问应用程序中的文档的代码。 有关详细信息，请参阅在[Office 应用程序启动时访问文档](../vsto/programming-vsto-add-ins.md#AccessingDocuments)。

 对于文档级自定义，[!INCLUDE[vs_current_short](../sharepoint/includes/vs-current-short-md.md)]将具有二进制格式的项目中的文档（如具有 *.xls*或 *.doc*扩展名的文档）转换为 Office Open XML 格式。 有关打开 XML 的详细信息，请参阅[新文件名扩展名和打开 XML 格式简介](https://support.office.com/en-nz/article/Introduction-to-new-file-name-extensions-eca81dcb-5626-4e5b-8362-524d13ae4ec1)。

> [!NOTE]
> Word 2010 和 Excel 2010 中已弃用智能标记。 因此，如果你的解决方案使用了智能标记，则必须先删除它们，然后才能在 Visual Studio 2013 或 Visual Studio 2015 中测试和调试你的解决方案。

## <a name="upgrade-microsoft-office-2003-projects"></a>升级微软 Office 2003 项目
...
### <a name="vsto-add-in-projects"></a>VSTO 外接程序项目
 如果原始项目的解决方案文件包含一个已配置为安装 VSTO 外接程序的安装项目或 InstallShield Limited Edition 项目，Visual Studio 将升级该项目，但不会对项目进行任何进一步的更改。 如果你想要继续使用 Windows Installer 文件来部署 VSTO 外接程序，则必须将安装项目或 InstallShield Limited Edition 项目修改为安装新的必备组件，如 [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)]、Visual Studio 2010 Tools for Office Runtime 和 VSTO 外接程序引用的主互操作程序集（可选）。 有关详细信息，请参阅使用[Windows 安装程序部署 Office 解决方案](../vsto/deploying-a-vsto-solution-by-using-windows-installer.md)。
...
- [将 Office 解决方案迁移到 .NET 框架 4 或更高版本](../vsto/migrating-office-solutions-to-the-dotnet-framework-4-or-later.md)
