...
## <a name="reset-control-placement"></a>重置控件位置
 可以通过将`Placement`属性设置为以下<xref:Microsoft.Office.Interop.Excel.XlPlacement>值之一来重置控件的位置和大小调整：
...
1. 调用控件的 "位置" 属性，并将值设置<xref:Microsoft.Office.Interop.Excel.XlPlacement.xlFreeFloating>为。
