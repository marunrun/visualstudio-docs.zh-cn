...
> [!NOTE]
> 由于  **设置，这些命令将出现在“配置任务”** `contextType`命令下的上下文菜单中。 “生成”、“重新生成”和“清除”为生成命令，因此它们会在上下文菜单中间的生成部分中显示。
...
> [!NOTE]
> 可从以下两个文件位置读取 launch.vs.json`configurations`*中的* 数组属性&mdash;：代码库的根目录和 .vs  目录。 如果存在冲突，则优先考虑 .vs\launch.vs.json  中的值。
