...
部署到 Microsoft Azure App Service 时，选择一个 Python 版本作为[站点扩展](/visualstudio/python/managing-python-on-azure-app-service?view=vs-2019)并手动安装包。 此外，因为 Azure 应用服务从 Visual Studio 部署时不会自动安装 requirements.txt 文件中的包，请遵照 [aka.ms/PythonOnAppService](managing-python-on-azure-app-service.md) 上的配置详细信息操作   。
