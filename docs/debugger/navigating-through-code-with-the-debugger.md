...
若要在调试时在每行代码或语句上停止，请使用 "**调试** > **步骤**"，或按**F11**。
...
在嵌套函数调用上， **“逐语句”** 将进入并单步执行嵌套最深的函数。 例如，如果在调用（如`Func1(Func2())`）时使用**单步**执行，则调试器将单步`Func2`执行函数。
...
> [!NOTE]
> 从开始[!include[vs_dev15](../misc/includes/vs_dev15_md.md)]，**运行以单击**。

### <a name="manually-break-into-code"></a>手动中断代码

若要在正在运行的应用程序中的下一个可用代码行内中断，请选择 "**调试** > **全部中断**"，或按**Ctrl**+**Alt**+**break**。
