...
---
# <a name="using-saved-intellitrace-data-c-visual-basic-c"></a>使用保存的 IntelliTrace 数据C#（、Visual Basic C++）
...
## <a name="Understand"></a> 理解 IntelliTrace 日志
 仅当从特定源（例如，在 SharePoint 应用程序中）收集数据时，才会显示 .Itrace 文件中的以下部分。

|**Section**|**包含**|**集合源**|
|-----------------|------------------|---------------------------|
|[性能冲突](#Performance)|函数调用超出已配置阈值的性能事件|对于在 IIS 上托管的 ASP.NET web 应用，为独立收集器或 System Center 2012 R2 Operations Manager Microsoft Monitoring Agent|
|[异常数据](#ExceptionData)|异常，包括每个异常的完整调用堆栈|所有源|
|[分析](#Analysis)|仅限于 SharePoint 2010 和 SharePoint 2013 应用程序。 诊断 IntelliTrace 和 SharePoint 事件，如调试器事件、ULS 事件、未经处理的异常以及 Microsoft Monitoring Agent 记录的其他数据。|Microsoft Monitoring Agent （独立收集器或 System Center 2012 R2） Operations Manager|
|[系统信息](#SystemInfo)|主机系统的设置和规范|所有源|
|[线程列表](#ThreadsList)|在集合期间运行的线程|所有源|
|[模块](#Modules)|目标进程按其加载顺序加载的模块。|所有源|
|[Web 请求](#Modules)|用于生产 IIS web 应用程序和 SharePoint 2010 和 SharePoint 2013 的 Web 请求数据|Microsoft Monitoring Agent 和独立收集器|
...
1. 从源复制 SharePoint 相关 ID。
    ...
    ![IntelliTrace &#45; SharePoint 错误&#45;相关性 ID](../debugger/media/sharepointerror_intellitrace.png "SharePointError_IntelliTrace")
