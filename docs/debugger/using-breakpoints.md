...
ms.locfileid: "71814360"
---
# <a name="use-breakpoints-in-the-visual-studio-debugger"></a>在 Visual Studio 调试器中使用断点
断点是开发人员的工具箱中最重要的调试技术之一。 您可以在希望暂停调试器执行的任何位置设置断点。 例如，你可能想要查看代码变量的状态或查看特定断点处的调用堆栈。 如果这是你第一次尝试调试代码，可能需要在浏览本文之前阅读[零基础调试](../debugger/debugging-absolute-beginners.md)。

## <a name="BKMK_Overview"></a>在源代码中设置断点
 可以在任意可执行代码行上设置断点。 例如，在下面C#的代码中，可以在变量声明、`for` 循环或 @no__t 2 循环内的任何代码上设置断点。 不能在命名空间、类声明或方法签名上设置断点。

 若要在源代码中设置断点，请单击代码行旁边的最左边距。 您还可以选择行并按**F9**，选择 "**调试** > **切换断点**"，或者右键单击并选择 "**断点** > **插入断点**"。 断点显示为左边距中的一个红点。

在C#代码中，断点和当前执行行自动突出显示。 对于C++代码，你可以通过选择 "**工具**" （或 "**调试**"） >**选项**启用断点和当前行的突出显示， > **调试** >  **突出显示断点和当前的整个源行语句（C++仅限）** 。

 ![设置断点](../debugger/media/basicbreakpoint.png "基本断点")

 调试时，执行将在断点处暂停，然后执行该行上的代码。 断点符号显示黄色箭头。

 在下面的示例中，在下面的示例中，的 @no__t 值仍为1。

 ![断点执行已停止](../debugger/media/breakpointexecution.png "断点执行")

 调试器在断点处停止时，可以查看应用的当前状态，包括变量值和调用堆栈。 有关调用堆栈的详细信息，请参阅 [How to：使用 "调用堆栈" 窗口 @ no__t。

- 该断点是一个开关。 可以单击它，按**F9**，或使用 "**调试**"  > **切换断点**删除或重新插入该断点。
...
1. 若要打开 "**调用堆栈**" 窗口，则必须在调试过程中暂停。 选择 **"调试**@no__t-**1 Windows** > **调用堆栈**"，或按**Ctrl**+**Alt**+**C**。
...
有关调用堆栈的详细信息，请参阅 [How to：使用 "调用堆栈" 窗口 @ no__t。
...
1. 若要打开 "**反汇编**" 窗口，必须在调试过程中暂停。 选择 "**调试**"  >  "**Windows**@no__t"**反汇编**"，或按**Alt**+**8**。
...
1. 选择 "**调试** > "  > **函数断点**的**新断点**，或按**Alt**+**F9** > **Ctrl**0**B**。

   你还可以在 "**断点**" 窗口中选择 "**新建** > **函数断点**"。
...
### <a name="set-a-function-breakpoint-using-a-memory-address-native-c-only"></a>使用内存地址设置函数断点（仅限本机C++ ）
 可以使用对象的地址在类的特定实例所调用的方法上设置函数断点。  例如，给定一个类型为 `my_class` 的可寻址对象，可以在实例调用的 @no__t 1 方法上设置函数断点。

1. 在实例化类的实例后，在某处设置断点。

2. 查找实例的地址（例如 `0xcccccccc`）。

3. 选择 "**调试** > "  > **函数断点**的**新断点**，或按**Alt**+**F9** > **Ctrl**0**B**。
...
1. 在C++项目中，开始调试，并等待，直到到达断点。 在 "**调试**" 菜单上，选择 "**新建断点**"  > **数据断点**
    ...
    您还可以在 "**断点**" 窗口中选择 "**新建** > **数据断点**"，或**在 "自动"、"** **监视**" 或 "**局部变量**" 窗口中右键单击某个项目，然后在上下文菜单中选择 "**值更改时中断**"。
...
若要打开 "**断点**" 窗口，请选择 "**调试**"  > **Windows** >  个**断点**，或按**alt**+**F9**或**Ctrl**0**Alt**2**B**。

"![断点窗口](../debugger/media/breakpointswindow.png "断点\" 窗口")

若要选择要在 "**断点**" 窗口中显示的列，请选择 "**显示列**"。 选择列标题可按该列对断点列表进行排序。
...
3. 选择 "**关闭**" 或按**Ctrl**@no__t 按**enter**关闭 "**断点设置**" 窗口。 或者，在 "**断点**" 窗口中，选择 **"确定"** 以关闭对话框。

带有条件集的断点在源代码和 "**断点**" 窗口中显示为 **+** 符号。
...
选择**条件表达式**时，可以在两个条件之间进行选择：**为 true**或**更改时**为。 选择 "为**true** " 可在满足表达式时中断，或**在更改**表达式的值时中断。
 ...
 如果为 true(../debugger/media/breakpointconditionistrue.png "，")则![断点条件]为 true

 在下面的示例中，仅当 `testInt` 的值更改时才会命中断点：

 更改时断点![时更改](../debugger/media/breakpointwhenchanged.png "断点")
...
4. 在“条件表达式”字段中使用对象 ID。 例如，如果变量 `item` 是要添加到集合中的对象，请选择 "**为 true** "，然后键入**item = = $ \<n >** ，其中 @no__t-DRJ-4N-YZG > 是对象 ID 号。
