...
Visual Studio 图形分析中的图形对象表可帮助你了解支持游戏或应用程序的帧的 Direct3D 对象。
 ...
 ![应用创建的 Direct3D 对象。](media/gfx_diag_demo_object_table_orientation.png "gfx_diag_demo_object_table_orientation")

## <a name="understanding-the-graphics-object-table"></a>了解图形对象表
 通过使用对象表，你可以分析支持特定帧呈现的 Direct3D 对象。 您可以通过检查特定对象的属性和数据（使用之前诊断中的其他图形诊断工具来确定该对象的呈现问题），可以将可能不是您所期望的对象的列表缩小。）找到有问题的对象后，可以使用特定于其类型的可视化效果进行检查，例如，可以使用图像编辑器来查看纹理，或使用*缓冲区可视化工具*查看缓冲区内容。

 “对象表”支持复制和粘贴，这样便可以使用其他工具（例如 Microsoft Excel）来检查它的内容。

 此外，还可以使用左上角的 "**类型**" 下拉列表来切换查看类型为**缓冲区**、**着色**器或**纹理**的对象或同时查看所有这些项。  此外，您还可以使用右上角的 "搜索" 框在显示的所有数据中查找特定的行。  例如，您可以搜索*D32_FLOAT*以查找列表中该格式的对象的所有实例。
...
## <a name="graphics-object-viewers"></a>图形对象查看器
 ...
 ![显示输出合并的纹理预览](media/gfx_diag_texture_preview.png "gfx_diag_texture_preview")
...
### <a name="device-context-object"></a>设备上下文对象
 在 Direct3D 11 和 Direct3D 10 中，设备上下文（“D3D11 设备上下文”或“D3D10 设备”）对象尤为重要，因为它包含最重要的状态信息，而且它链接到当前设置的其他状态对象。 设备上下文详细信息显示在新的文档窗口中，并且每个类别的信息都显示在其自己的选项卡上。选择新事件以反映当前设备状态时，设备上下文会发生变化。
