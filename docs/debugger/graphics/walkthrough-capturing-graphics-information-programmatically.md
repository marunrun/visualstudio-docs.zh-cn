...
- `CaptureCurrentFrame`在手动测试中难以预测和捕获呈现问题，但可以通过在运行时使用有关应用状态的信息以编程方式预测呈现问题。
...
> [!NOTE]
> 以前的编程捕获的实现依赖于的[!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)] Visual Studio 远程工具来提供捕获功能。
...
> [!IMPORTANT]
> 使用编程捕获时，仍必须在图形诊断下运行应用（在中[!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)]使用 Alt + F5），或在[命令行捕获工具](command-line-capture-tool.md)下运行应用。
...
- 请使用以下代码挂钩 DXGI 调试接口的 IDXGraphicsAnalysis 接口。
  ...
  务必检查由[DXGIGetDebugInterface1](/windows/desktop/api/dxgi1_3/nf-dxgi1_3-dxgigetdebuginterface1)返回`HRESULT`的，以确保在使用它之前获得一个有效的接口：
