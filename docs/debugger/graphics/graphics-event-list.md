...
### <a name="event-list-views"></a>事件列表视图
 事件列表支持两种不同的视图，这两种视图以不同的方式组织图形事件，以便支持你的工作流和首选项。 第一个视图是*GPU 工作视图*，它分层地组织事件及其关联的状态。 第二个视图是在简单列表中按时间顺序组织事件的 *时间线视图* 。

 **GPU 工作**视图在层次结构中显示捕获的事件及其状态。 层次结构顶层由事件（例如，绘图调用、清除、显示和处理视图的事件）组成。 在事件列表中，你可以展开绘图调用以显示绘图调用时当前所处的设备状态；你可以进一步展开每种状态以显示设置其值的事件。 在此级别上，你还可以看到是否在之前的帧中设置了某个特定状态，或者自从最后一次绘图调用以后，是否对它进行了多次设置。

 "**时间轴"** 视图按时间顺序显示每个捕获的事件。 这种组织事件列表的方式与早期版本的 Visual Studio 中使用的方式相同。

##### <a name="to-change-the-event-list-view-mode"></a>更改事件列表视图模式

- 在事件列表上方的 **"图形事件列表"** 窗口中，找到 **"视图**"下拉菜单并选择 **"时间轴"** 视图或**GPU 工作**视图。
...
Visual Studio 2017 和更高包含**资源历史记录**窗口。  在 **"事件列表"** 窗口中](media/gfx_watch.png)的条目旁边选择手表图标![手表图标将打开如下所示**的资源历史记录**窗口：

![资源历史记录](media/gfx_diag_resource_history.png)

此窗口允许您查看事件列表中所选项的历史记录。  顶部的下拉可用于选择要查看 的历史记录的其他项。  窗口的上半部分包含**帧设置事件**。  这些事件属于 *"创建*类型"类别，是通常初始化和创建资源的调用。  窗口的下半部分包含 **"帧事件**"部分。  这些是使用资源期间发生的正常读取和写入事件。

| 列 | 说明 |
|-----------| - |
| **类型** | 显示条目的类型，通常*创建*、*读取*和*写入*。 |
| **视图** | 显示该时间时刻的资源缩略图。  双击缩略图可打开此时资源的详细信息视图。 |
| **事件** | 显示生成事件的方法调用。  通过选择相应行上的手表图标![手表图标](media/gfx_watch.png)，可以查看各个项目的任何其他历史记录。  此外，任何以蓝色文本绘制的项目（如`m_commandList`上面的屏幕截图中）都可以选择以了解更多详细信息。 |
