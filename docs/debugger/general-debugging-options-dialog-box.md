...
**启用适用于ASP.NET应用的旧版 Chrome JavaScript 调试器**:为 ASP.NET 应用启用旧版 Chrome JavaScript 脚本调试器。 第一次在 Chrome 中使用时，可能需要登录到浏览器来启用已安装的 Chrome 扩展。
...
- **启用 XAML 热重载**：允许你在应用程序运行时将 XAML 热重载功能与 XAML 代码一起使用。 （此功能之前称为 "XAML 编辑并继续"）
