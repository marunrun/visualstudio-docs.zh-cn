...
在详细介绍了解决问题的步骤之后，在再次尝试调试前，你可能还需要重置 IIS。 可通过打开管理员命令提示符并键入 `iisreset` 来执行此操作。

若要解决此问题，请执行以下步骤：

1. 如果服务器上托管的 Web 应用程序配置为发布版本，则将其重新发布为调试版本，并验证 web.config 文件在编译元素中是否包含 `debug=true`。 重置 IIS，然后重试。

    例如，如果使用发布版本的发布配置文件，请将其更改为 "调试并重新发布"。 否则，在发布时，调试特性将设置为 `false`。

2. IIS验证物理路径是否正确。 在 IIS 中，你可以在 "**基本设置" > 物理路径**（或较早版本的 IIS 中的**高级设置**）中找到此设置。

    如果将 web 应用程序复制到另一台计算机、手动重命名或移动，则物理路径可能不正确。 重置 IIS，然后重试。

3. 如果要在 Visual Studio 中进行本地调试，请验证是否在属性中选择了正确的服务器。 (打开**属性 > Web > 服务器**或**属性 > 调试** 具体取决于你的项目类型。 对于 Web 窗体项目，打开“属性页”>“开始选项”>“服务器”)。

    如果使用的是外部（自定义）服务器（如 IIS），则 URL 必须正确。 否则，请选择 IIS Express，然后重试。

4. IIS请确保在服务器上安装了 ASP.NET 的正确版本。
...
4. 如果 IIS 中的 `maxConnection` 限制太低，并且连接太多，则可能需要[提高连接限制](/iis/configuration/system.applicationhost/sites/sitedefaults/limits)。
