...
若要调试 Linux Docker 容器上的 .NET Core 进程，请参阅[附加到 Linux Docker 容器](#BKMK_Linux_Docker_Attach)。
::: moniker-end
...
|调试 Linux 上的 .NET Core|使用“附加到进程” |*dotnet.exe*|若要使用 SSH，请参阅[使用 SSH 远程调试正在 Linux 上运行的 .NET Core](../debugger/remote-debugging-dotnet-core-linux-with-ssh.md)。 |
