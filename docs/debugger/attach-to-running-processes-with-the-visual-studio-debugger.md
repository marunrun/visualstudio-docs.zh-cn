...
4. 在“可用进程”列表中，查找并选择要附加到的一个或多个进程。
   ...
   - 若要查找所有用户帐户下运行的进程，请选择“显示所有用户的进程”复选框。
     ...
     >[!NOTE]
     >如果尝试附加到不受信任的用户帐户拥有的进程，则会出现安全警告对话框确认。 有关详细信息，请参阅 [安全警告：附加到不受信任的用户所拥有的进程可能很危险。如果以下信息看上去可疑或无法确定，请不要附加到此进程](../debugger/security-warning-attaching-to-a-process-owned-by-an-untrusted-user.md)。
...
如果这两种解决方法都不可行，第三种方法是通过从 Windows 命令行运行 `vsjitdebugger.exe -p <ProcessId>` 来附加到进程。 可以使用*tlist.exe*来确定进程 ID。 若要获取“tlist.exe”，请从 [WDK 和 WinDbg 下载](/windows-hardware/drivers/download-the-wdk)中下载并安装适用于 Windows 的调试工具。

::: moniker range=">= vs-2019"

## <a name="attach-to-a-net-core-process-running-on-linux-using-ssh"></a>使用 SSH 附加到 Linux 上运行的 .NET Core 进程

有关详细信息，请参阅[在 Linux 上使用 SSH 运行的远程调试 .Net Core](../debugger/remote-debugging-dotnet-core-linux-with-ssh.md)。
...
|IIS 服务器上的远程调试 ASP.NET Core|使用远程工具并**附加到进程**|*dotnet*或*appname*|有关应用部署，请参阅[发布到 IIS](https://docs.asp.net/en/latest/publishing/iis.html)。 对于调试，请参阅远程[调试 ASP.NET Core 在远程 IIS 计算机上](../debugger/remote-debugging-aspnet-on-a-remote-iis-computer.md)|
...
|调试不是从 Visual Studio 启动的通用 Windows 应用（UWP）、OneCore、HoloLens 或 IoT 应用|调试安装的应用包|不可用|请参阅[调试已安装的应用包](debug-installed-app-package.md)，而不是使用 "**附加到进程**"|
