...
1. （仅 Visual Basic）在解决方案资源管理器（右窗格）中，右键单击项目节点，然后选择 "**属性**"。 在 "**应用程序**" 选项卡下，将**启动对象**更改为 "**简单**"。
...
1. 在 `Thread.Sleep` 或 `std::this_thread::sleep_for` 语句的左滚动条槽中左键单击，以插入新断点。

    在装订线中，红色圆圈指示在此位置设置断点。
...
1. 在 "调试" 工具栏中，选择 "**在源中显示线程**" 按钮![显示源中的线程](../debugger/media/dbg-multithreaded-show-threads.png "ThreadMarker")。
...
1. 通过选择“调试” **“窗口”** “并行堆栈” >  **，打开“并行堆栈”**  > 窗口。 此时会看到如下所示的内容。 确切信息取决于每个线程的当前位置、硬件以及编程语言。
    ...
    - 主线程（左侧）已在 `Thread.Start`上停止，其中，停止点由线程标记图标![线程标记](../debugger/media/dbg-thread-marker.png "ThreadMarker")指示。
...
2. 右键单击并选择 "**标志**"。
    ...
    所有选定的线程都已标记。 现在，可以筛选以仅显示标记的线程。

3. 在 "**并行监视**" 窗口中，选择 "**仅显示标记的线程**" 按钮![显示已标记的线程](../debugger/media/dbg-threads-show-flagged.png "ThreadMarker")。
...
4. 选择 "**仅显示标记的线程**" 按钮，切换回 "**显示所有线程**" 模式。
...
### <a name="bkmk_follow_a_thread"></a>使用条件断点的单个线程

可以在调试器中对单线程的执行情况进行跟踪。 一种方法是冻结不感兴趣的线程。 在某些情况下，可能需要在不冻结其它线程的情况下跟踪单个线程，例如重现特定 Bug。 若要在不冻结其他线程的情况下跟踪某个线程，必须避免在不感兴趣的线程上中断。 可以通过设置[条件断点](../debugger/using-breakpoints.md#BKMK_Specify_a_breakpoint_condition_using_a_code_expression)来实现此目的。

可以根据不同的条件（例如，线程名称或线程 ID）来设置断点。 如果你知道数据对于每个线程都是唯一的，则可根据该数据设置条件断点。 这是常见的调试场景，即相比于特定的线程，你对某些特定的数据值更感兴趣。

1. 右键单击前面创建的断点，然后选择 "**条件**"。
...
4. 选择 "重新启动![应用程序](../debugger/media/dbg-tour-restart.png "重启应用")" 按钮以重新启动调试会话。

    将在数据变量的值为 5 的线程中，中断代码执行。 请在“并行监视”窗口中，寻找表示当前调试器上下文的黄色箭头。

5. 现在，您可以逐过程执行代码（**F10**）和单步执行代码（**F11**）并执行单个线程。

    只要断点条件对于线程是唯一的，调试器就不会命中其他线程上的任何其他断点（您可能需要禁用它们），您可以逐过程执行代码并单步执行代码，而无需切换到其他线程。

    > [!NOTE]
    > 前进调试器时，所有线程都将运行。 但是，调试器不会中断其他线程上的代码，除非其中一个线程命中了断点。
