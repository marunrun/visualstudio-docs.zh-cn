...
编写代码时，[IntelliSense](../ide/using-intellisense.md) 是非常有用的功能。 它可显示某个类型的可用成员信息，或某个方法中不同重载的参数详情。 它还可以在你输入足以消除歧义的字符后自动补全单词。 接下来试试添加代码行，将有序字符串呈现到控制台窗口，也就是是程序输出的标准位置。
