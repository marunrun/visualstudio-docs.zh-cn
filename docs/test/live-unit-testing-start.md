...
8. 由于单元测试代码包含一些非 ASCII 字符，因此 Visual Studio 显示以下对话框来进行警告，如果以其默认的 ASCII 格式保存文件，某些字符将会丢失。 选择“以其他编码保存”按钮  。
...
1. 单击 `StartsWithUpper` 方法中写着 `return Char.IsUpper(s[0])` 的行上的绿色复选标记。 如下图所示，Live Unit Testing 指示只有两个测试均覆盖该行的代码，并且都已成功执行。
...
1. 选择失败的“TestHasEmbeddedSpaces”测试  。 Live Unit Testing 提供了多个选项，例如运行所有测试、运行选定测试、调试所有测试以及调试选定测试，如下图所示：
