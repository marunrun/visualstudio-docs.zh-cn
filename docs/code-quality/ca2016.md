---
title: CA2016：将 CancellationToken 参数转发到采用一个的方法
ms.date: 06/18/2020
ms.topic: reference
f1_keywords:
...
ms.locfileid: "85837710"
---
# <a name="ca2016-forward-the-cancellationtoken-parameter-to-methods-that-take-one"></a>CA2016：将 CancellationToken 参数转发到采用一个的方法
...
还可以显式传入 `default` c # （ `Nothing` 在 Visual Basic 中）或 <xref:System.Threading.CancellationToken.None> 禁止显示规则冲突。
