...
## <a name="cause"></a>原因
安全透明方法调用未用 <xref:System.Security.AllowPartiallyTrustedCallersAttribute> （APTCA）特性标记的程序集中的方法，或者安全透明方法满足某个类型或方法的 <xref:System.Security.Permissions.SecurityAction> @ no__t。
...
若要修复与此规则的冲突，请使用 <xref:System.Security.SecurityCriticalAttribute> 或 <xref:System.Security.SecuritySafeCriticalAttribute> 属性标记访问方法，或从访问的方法中删除 LinkDemand。
