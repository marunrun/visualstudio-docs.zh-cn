...
- 该类型没有采用对象的构造函数 <xref:System.Runtime.Serialization.SerializationInfo> 和一个 <xref:System.Runtime.Serialization.StreamingContext> 对象（序列化构造函数的签名）。
