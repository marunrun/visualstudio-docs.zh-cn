...
此警告触发方法，该方法将标记为的<xref:System.Security.SecurityCriticalAttribute>委托绑定到透明或标记为的<xref:System.Security.SecuritySafeCriticalAttribute>方法。 还会对另一个具有以下特点的方法引发此警告：该方法将透明的或安全关键的委托绑定到一个关键方法。
