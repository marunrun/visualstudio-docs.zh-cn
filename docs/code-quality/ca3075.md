...
- 使用不安全的 @no__t 1 实例或根本不使用任何实例调用 <xref:System.Xml.XmlReader.Create%2A?displayProperty=nameWithType> 方法。
...
- 确保从受信任的源中分配 @no__t 0 属性。

**.NET 3.5 及更早版本**

- 如果要处理不受信任的源，请将 @no__t 设置为**true**，以禁用 DTD 处理。

- XmlTextReader 类具有完全信任继承要求。

**.NET 4 及更高版本**

- 如果要处理不受信任的源，请将 @no__t 设置为**禁止**或**忽略**，以避免启用 DtdProcessing。
