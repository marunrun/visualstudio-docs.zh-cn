...
ms.locfileid: "72017552"
---
# <a name="c6332"></a>C6332
警告 C6332：无效的参数：不允许将零作为 dwFreeType 参数传递到 \<function >。 这会导致此调用失败

 此警告表明传递到 VirtualFree 或 VirtualFreeEx 的参数无效。 VirtualFree 和 VirtualFreeEx 都拒绝0的 dwFreeType 参数。 DwFreeType 参数可以是 MEM_DECOMMIT 或 MEM_RELEASE。 但是，值 MEM_DECOMMIT 和 MEM_RELEASE 不能在同一调用中一起使用。 此外，请确保不忽略 VirtualFree 函数的返回值。
