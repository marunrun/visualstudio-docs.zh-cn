...
透明方法（用 <xref:System.Security.SecuritySafeCriticalAttribute> 方法标记的方法）或包含方法的类型使用 <xref:System.Security.SuppressUnmanagedCodeSecurityAttribute> 属性进行标记。
