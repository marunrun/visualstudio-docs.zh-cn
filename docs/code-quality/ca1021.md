...
实现**Try\<某个 >** 模式（如 <xref:System.Int32.TryParse%2A?displayProperty=fullName>）的方法不会引发此冲突。 下面的示例演示实现 <xref:System.Int32.TryParse%2A?displayProperty=fullName> 方法的结构（值类型）。
