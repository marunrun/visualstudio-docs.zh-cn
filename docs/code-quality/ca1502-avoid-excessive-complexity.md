...
如果复杂性无法轻易降低，并且方法易于理解、测试和维护，则可以安全地禁止显示此规则发出的警告。 具体而言，包含大型`switch` （`Select` in [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)]）语句的方法是排除的候选项。 在开发周期后期破坏代码的风险，或在之前发布的代码中引入运行时行为的意外更改可能超过重构代码的可维护性优势。
...
[CA1501避免过度继承](../code-quality/ca1501-avoid-excessive-inheritance.md)
