...
- <xref:System.Xml.XmlDocument> 或 <xref:System.Xml.XmlTextReader> 类使用默认的冲突解决程序值进行 DTD 处理。
...
- 使用 XmlResolver 的 @no__t 0instead 来限制 XmlTextReader 可以访问的资源。
