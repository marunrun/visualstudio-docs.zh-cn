...
由于重载仅在字符串或 <xref:System.Uri> 参数上存在差异，因此假定字符串表示统一资源标识符（URI）。 URI 的字符串表示形式容易导致分析和编码错误，并且可造成安全漏洞。 @No__t_0 类以安全安全的方式提供这些服务。 为了获得 <xref:System.Uri> 类的优点，字符串重载应使用字符串参数调用 <xref:System.Uri> 重载。
