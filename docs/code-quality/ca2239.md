...
## <a name="cause"></a>原因
类型具有使用 <xref:System.Runtime.Serialization.OptionalFieldAttribute?displayProperty=fullName> 特性标记的字段，并且该类型不提供反序列化事件处理方法。

## <a name="rule-description"></a>规则描述
<xref:System.Runtime.Serialization.OptionalFieldAttribute> 属性对序列化不起作用;序列化用特性标记的字段。 但是，在反序列化时将忽略字段，并保留与类型关联的默认值。 反序列化事件处理程序应该在反序列化过程中声明为设置字段。
