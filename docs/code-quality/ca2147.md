...
此规则分析程序集中的所有方法和类型，该程序集为 100% 透明或混合透明/关键，并标记 @no__t 的任何声明性或命令性用法。

在运行时，从透明代码对 @no__t 的任何调用都将导致引发 @no__t 1。 这种情况可能发生在 100% 透明程序集，也可能出现在混合透明/关键程序集中，其中方法或类型声明为透明，但包含声明性或命令式断言。
...
若要解决此问题，请将调用断言的代码标记 <xref:System.Security.SecurityCriticalAttribute>，或删除断言。

## <a name="when-to-suppress-warnings"></a>何时禁止显示警告
请勿禁止显示此规则的消息。

## <a name="example"></a>示例
如果 `SecurityTestClass` 是透明的，则当 @no__t 1 方法引发 <xref:System.InvalidOperationException> 时，此代码将失败。
