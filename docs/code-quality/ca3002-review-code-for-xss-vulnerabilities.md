...
当处理 web 请求中的不受信任输入时，请注意跨站点脚本（XSS）攻击。 XSS 攻击会将不受信任的输入注入原始 HTML 输出，使攻击者可以执行恶意脚本或恶意修改网页中的内容。 典型的技术是将`<script>`包含恶意代码的元素放入输入中。 有关详细信息，请参阅[OWASP 的 XSS](https://www.owasp.org/index.php/Cross-site_Scripting_(XSS))。
