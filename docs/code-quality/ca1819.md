...
可以禁止从 <xref:System.Attribute> 类派生的属性的属性引发的警告。 特性可以包含返回数组的属性，但不能包含返回集合的属性。
...
下面的示例通过更改属性来返回 <xref:System.Collections.ObjectModel.ReadOnlyCollection%601?displayProperty=fullName> 来修复冲突：
