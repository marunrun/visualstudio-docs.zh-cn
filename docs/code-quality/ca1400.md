...
## <a name="cause"></a>原因
公共或受保护的方法标记有 <xref:System.Runtime.InteropServices.DllImportAttribute?displayProperty=fullName>。 未能找到非托管库，或者未能将方法与库中的函数匹配。 如果规则找不到指定的方法名称，它将通过使用 "A" 或 "W" suffixing 方法名称来查找方法的 ANSI 或宽字符版本。 如果找不到匹配项，规则将尝试使用 __stdcall 名称格式（_MyMethod@12，其中12表示参数的长度）来查找函数。 如果未找到匹配项，并且方法名称以 "#" 开头，则规则会搜索函数作为序号引用而不是名称引用。
