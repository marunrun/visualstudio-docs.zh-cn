...
核心指导原则： **C++** F.：使用 not_null @ no__t-0T > 指示 "null" 不是有效的值

如果代码曾经检查指针变量的非 null，则它应一致地执行此操作，并验证所有路径上的指针。 有时，overaggressive 检查非 null 仍比复杂分支之一中的硬崩溃更好。 理想情况下，应重构此类代码，使其变得不太复杂（通过拆分为多个函数），并依赖于标记（如 gsl：： not_null （请参阅准则支持库））隔离可对有效指针值作出安全假设的算法部分。 规则 TEST_ON_ALL_PATHS 可帮助查找非 null 检查不一致的位置（因此假设可能需要检查）或实际 bug （其中可能的 null 值可以绕过某些代码路径中的非 null 检查）。
