...
- @no__t 在 @no__t 2 的程序集的外部调用方法 @no__t。

- @no__t 的程序集没有 APTCA 特性，因此不应由或代表部分受信任的调用方来执行。

部分受信任的调用方 `X` 可以调用方法 `M1`，导致 @no__t 2 调用 @no__t。 因为 `M2` 没有 APTCA 特性，所以它的直接调用方（`M1`）必须满足完全信任的链接要求;`M1` 具有完全信任，因此满足此项检查。 安全风险是因为 `X` 不参与满足保护来自不受信任调用方的 @no__t 的链接需求。 因此，具有 APTCA 特性的方法不能调用没有特性的方法。
...
测试应用程序（由上一讨论中 @no__t 0 表示）是部分受信任的应用程序。
...
- [CA2117：APTCA 类型应只扩展 APTCA 基类型 @ no__t-0
