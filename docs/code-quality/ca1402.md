...
若要修复与此规则的冲突，请重命名重载的方法，使名称唯一。 或者，通过将可访问性更改为 `internal` （ `Friend` 在中为 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] ）或 <xref:System.Runtime.InteropServices.ComVisibleAttribute?displayProperty=fullName> 将属性设置为 `false` ，使接口对 COM 不可见。
