...
CSE 指示进程状态已损坏且未被系统捕获。 在损坏状态情况下，如果使用特性标记方法，则常规处理程序仅捕获异常 <xref:System.Runtime.ExceptionServices.HandleProcessCorruptedStateExceptionsAttribute?displayProperty=fullName> 。 默认情况下，[公共语言运行时（CLR）](/dotnet/standard/clr)不会为 cse 调用 catch 处理程序。
