...
设置 `Switch.System.ServiceModel.DisableUsingServicePointManagerSecurityProtocols` 以 `true` 限制 Windows Communication FRAMEWORK (WCF) 传输层安全 (tls) 与使用 tls 1.0 的连接。 TLS 版本将不推荐使用。 有关详细信息，请参阅 [传输层安全性 (TLS) 与 .NET Framework 的最佳实践](/dotnet/framework/network-programming/tls#switchsystemservicemodeldisableusingservicepointmanagersecurityprotocols)。
...
- 否则，请参阅 [传输层安全性 (TLS) 最佳实践和](/dotnet/framework/network-programming/tls) 用于缓解的 .NET Framework。
