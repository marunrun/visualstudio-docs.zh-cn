...
用于 `Switch.System.ServiceModel.DisableUsingServicePointManagerSecurityProtocols` 将 `true` Windows Communication FRAMEWORK （WCF）传输层安全性（TLS）连接限制为使用 TLS 1.0 的设置。 TLS 版本将不推荐使用。 有关详细信息，请参阅[.NET Framework 的传输层安全性（TLS）最佳实践](/dotnet/framework/network-programming/tls#switchsystemservicemodeldisableusingservicepointmanagersecurityprotocols)。
