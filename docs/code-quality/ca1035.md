...
## <a name="cause"></a>原因
公共或受保护类型实现 <xref:System.Collections.ICollection?displayProperty=fullName> 但不为 <xref:System.Collections.ICollection.CopyTo%2A?displayProperty=fullName>提供强类型化方法。 <xref:System.Collections.ICollection.CopyTo%2A> 的强类型版本必须接受两个参数，并且不能将 <xref:System.Array?displayProperty=fullName> 或 <xref:System.Object?displayProperty=fullName> 数组作为其第一个参数。
