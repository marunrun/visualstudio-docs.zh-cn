...
| [CA1409:COM 可见类型应该是可创建的](../code-quality/ca1409.md) | 专门标记为对 COM 可见的某个引用类型包含公共的参数化构造函数，但不包含公共的默认（无参数）构造函数。 没有公共默认构造函数的类型不能由 COM 客户端创建。 |
| [CA1410:应对 COM 注册方法进行匹配](../code-quality/ca1410.md) | 类型声明一个使用特性标记的方法 <xref:System.Runtime.InteropServices.ComRegisterFunctionAttribute?displayProperty=fullName> ，但不声明使用特性标记的方法 <xref:System.Runtime.InteropServices.ComUnregisterFunctionAttribute?displayProperty=fullName> ，反之亦然。 |
...
| [CA1415：正确声明 P/Invoke](../code-quality/ca1415.md) | 此规则查找平台调用方法声明，该方法声明 [!INCLUDE[TLA2#tla_win32](../code-quality/includes/tla2sharptla_win32_md.md)] 具有指向重叠结构参数的指针，并且对应的托管参数不是指向结构的指针 <xref:System.Threading.NativeOverlapped?displayProperty=fullName> 。 |
