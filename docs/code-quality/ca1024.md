...
方法具有以 `Get`开头的名称，不使用任何参数，并且返回的值不是数组。
...
可以通过应用 <xref:System.Diagnostics.DebuggerBrowsableAttribute?displayProperty=fullName>阻止调试器 autoexpanding 属性。 下面的示例演示如何将此特性应用于实例属性。
