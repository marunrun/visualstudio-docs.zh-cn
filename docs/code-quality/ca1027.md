...
在下面的示例中，`DaysEnumNeedsFlags` 是符合使用 <xref:System.FlagsAttribute> 但不具有该要求的枚举。 @No__t_0 枚举的值不是2的幂，而是不正确地指定 <xref:System.FlagsAttribute>。 这违反[了规则 CA2217：请勿用 FlagsAttribute 标记枚举](../code-quality/ca2217.md)。
