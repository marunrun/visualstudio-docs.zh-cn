...
## <a name="cause"></a>原因
当类型成员标记为具有与成员容器的安全属性不同的透明度的 @no__t 0 安全特性标记时，将触发此规则。
...
## <a name="example"></a>示例
在下面的示例中，用 <xref:System.Security.SecuritySafeCriticalAttribute> 特性标记方法，并且它是使用 <xref:System.Security.SecurityCriticalAttribute> 特性标记的类的成员。 应删除安全安全特性。
