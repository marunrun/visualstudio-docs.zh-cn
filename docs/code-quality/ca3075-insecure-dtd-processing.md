...
- <xref:System.Xml.XmlResolver> 使用<xref:System.Xml.XmlSecureResolver>而不是处理不受信任的输入。

- 使用<xref:System.Xml.XmlReader.Create%2A?displayProperty=nameWithType>不安全<xref:System.Xml.XmlReaderSettings>的实例或根本不使用任何实例调用方法。
...
- <xref:System.Xml.XmlSecureResolver>使用来限制 XmlTextReader 可以访问的资源。

- 通过将 <xref:System.Xml.XmlReader> 属性设置为 <xref:System.Xml.XmlResolver> null **，不允许**打开任何外部资源。

- 确保从受<xref:System.Data.DataViewManager.DataViewSettingCollectionString%2A?displayProperty=nameWithType>信任的源中分配了该属性。
...
- 如果要处理不受信任的源，请将<xref:System.Xml.XmlReaderSettings.DtdProcessing%2A?displayProperty=nameWithType>属性设置为**禁止**或**忽略**，以避免启用 DtdProcessing。
