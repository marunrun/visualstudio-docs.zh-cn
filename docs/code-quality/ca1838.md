---
title: CA1838：不要对 P/Invoke 使用 StringBuilder 参数
ms.date: 08/03/2020
ms.topic: reference
f1_keywords:
...
默认情况下，此规则处于禁用状态，因为它可能需要按大小写方式分析冲突是否感兴趣，并有可能不重要的重构来解决冲突。 用户可以通过配置 [分析器规则的严重性](use-roslyn-analyzers.md#configure-severity-levels)来显式启用此规则。
