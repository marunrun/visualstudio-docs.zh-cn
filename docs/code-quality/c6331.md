...
ms.locfileid: "72449658"
---
# <a name="c6331"></a>C6331
警告 C6331：参数无效：不允许将 MEM_RELEASE 和 MEM_DECOMMIT 传递到 \<function >。 这会导致此调用失败

 此消息表示传递给 VirtualFree 或 VirtualFreeEx 的参数无效。 VirtualFree 和 VirtualFreeEx 都同时拒绝标志（MEM_RELEASE &#124; MEM_DECOMMIT）。 因此，值 MEM_DECOMMIT 和 MEM_RELEASE 不能在同一调用中一起使用。
