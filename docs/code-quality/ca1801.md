...
- 用 @no__t @no__t （Visual Basic）修饰符声明的方法。

- 用 @no__t @no__t （Visual Basic）修饰符声明的方法。

- 用 @no__t @no__t （Visual Basic）修饰符声明的方法。

- 用 `extern` （Visual Basic 中的 @no__t 语句）修饰符声明的方法。

如果使用的是[FxCop 分析器](install-fxcop-analyzers.md)，则此规则不会标记使用[丢弃](/dotnet/csharp/discards)符号（例如 `_`、`_1` 和 `_2`）命名的参数。 这会减少签名要求所需的参数的警告性噪音，例如，用作委托的方法、具有特殊属性的参数或其值在运行时由框架隐式访问但未在中被引用的参数编写.
...
- 对于 <xref:Microsoft.VisualStudio.TestTools.UnitTesting.Assert?displayProperty=nameWithType> 的自定义扩展方法中的 `this` 参数。 @No__t-0 类中的函数是静态的，因此无需访问方法体中的 `this` 参数。
