...
|[CA2204:文字应正确拼写](../code-quality/ca2204.md)|方法体中的文本字符串包含一个或多个未被 Microsoft 拼写检查器库识别的单词。|
...
|[CA2230:对可变数量的参数使用 params](../code-quality/ca2230.md)|公共或受保护类型包含一个使用 VarArgs 调用约定（而不是 params 关键字）的公共或受保护方法。|
...
|[CA2249：请考虑使用字符串。包含而不是字符串。 IndexOf](../code-quality/ca2249.md)|对结果的调用 `string.IndexOf` （其中，用于检查是否存在子字符串）可以替换为 `string.Contains` 。|
