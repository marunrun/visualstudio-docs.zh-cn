...
| [CA1002：不要公开泛型列表](../code-quality/ca1002-do-not-expose-generic-lists.md) | @No__t 的 < （of （T >） >）是一个泛型集合，旨在实现性能，而不是继承。 因此，List 不包含任何虚拟成员。 应改为公开针对继承设计的泛型集合。 |
| [CA1003：使用泛型事件处理程序实例](../code-quality/ca1003-use-generic-event-handler-instances.md) | 类型包含一个返回 void 的委托，该委托的签名包含两个参数（第一个对象，第二个参数是可分配给 EventArgs 的类型），而包含的程序集目标 .NET Framework 2.0。 |
...
| [CA1005：避免泛型类型的参数过多](../code-quality/ca1005-avoid-excessive-parameters-on-generic-types.md) | 泛型类型包含的类型参数越多，越难以知道并记住每个类型参数各代表什么。 通常有一个类型参数，如 List @ no__t-0T > 中所述，在某些情况下有两个类型参数，如 Dictionary @ no__t-1TKey、TValue >。 但是，如果存在两个以上的类型参数，则大多数用户都会感到过于困难。 |
...
| [CA1016：用 AssemblyVersionAttribute 标记程序集](../code-quality/ca1016-mark-assemblies-with-assemblyversionattribute.md) | .NET 使用版本号来唯一标识程序集，并绑定到强名称程序集中的类型。 版本号与版本和发行者策略一起使用。 默认情况下，仅使用用于生成应用程序的程序集版本运行应用程序。 |
