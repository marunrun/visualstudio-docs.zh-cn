...
对 `gsl::span` 或 `std::string_view` 的引用可能是生存期问题的指示。
C++核心指导原则：[GSL：视图 @ no__t-0
...
- 规则检测到对 gsl：： span < >、gsl：： basic_string_span < > 和 std：： basic_string_view < （包括实例化的别名）的引用。
- 当前仅在声明和返回语句上发出警告。 这可能会在将来扩展，同时还会标记函数参数。
- 此规则的实现非常轻量不会尝试跟踪实际生存期。 在某些情况下，使用引用可能仍有意义。 在这种情况下，可以安全地禁止显示误报。

## <a name="example-reference-to-a-temporary"></a>例如：引用临时
