...
方法使用从字符串参数生成的字符串为方法设置 <xref:System.Data.IDbCommand.CommandText%2A?displayProperty=fullName> 属性。
...
以下 .NET 类型实现 <xref:System.Data.IDbCommand.CommandText%2A> 属性，或提供使用字符串参数设置属性的构造函数。
...
下面的示例演示了一个 @no__t 为0的方法，该方法违反了规则，以及一个方法，该方法 `SaferQuery`，通过使用参数化命令字符串满足规则。
