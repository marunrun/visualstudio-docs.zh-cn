...
> [!NOTE]
> 从 .NET Framework 4 开始，公共语言运行时（CLR）不再提供操作系统和托管代码（如中的访问冲突）中发生的损坏状态异常， [!INCLUDE[TLA#tla_mswin](../code-quality/includes/tlasharptla_mswin_md.md)] 以由托管代码进行处理。 如果要在 .NET Framework 4 或更高版本中编译应用程序并维护对损坏状态异常的处理，可将属性应用于 <xref:System.Runtime.ExceptionServices.HandleProcessCorruptedStateExceptionsAttribute> 处理损坏状态异常的方法。
