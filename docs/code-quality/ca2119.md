...
## <a name="cause"></a>原因
可继承的公共类型提供 @no__t 的可重写的方法实现（Visual Basic 中 `Friend`）。

## <a name="rule-description"></a>规则说明
接口方法具有公共可访问性，实现类型不能对其进行更改。 内部接口创建一个协定，该协定不应在定义接口的程序集的外部实现。 一个公共类型，该类型通过使用 `virtual` （Visual Basic 中的 @no__t）修饰符来实现内部接口的方法，使该方法可被该程序集之外的派生类型重写。 如果定义程序集中的第二个类型调用方法并且需要仅限内部的协定，则在执行外部程序集中重写的方法时，行为可能会受到影响。 这会产生安全漏洞。

## <a name="how-to-fix-violations"></a>如何解决冲突
若要修复与此规则的冲突，请使用以下方法之一阻止在程序集外重写方法：

- 使声明类型 `sealed` （@no__t 在 Visual Basic 中为1）。

- 将声明类型的可访问性更改为 `internal` （Visual Basic 中 `Friend`）。

- 删除声明类型中的所有公共构造函数。

- 实现方法，而不使用 `virtual` 修饰符。
