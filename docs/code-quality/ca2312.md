...
调用或引用了 @no__t 0 反序列化方法，但 @no__t 属性可以为 null。

## <a name="rule-description"></a>规则说明

[!INCLUDE[insecure-deserializers-description](includes/insecure-deserializers-description-md.md)]

当 <xref:System.Runtime.Serialization.NetDataContractSerializer.Binder> 可能为 null 时，此规则将查找 <xref:System.Runtime.Serialization.NetDataContractSerializer?displayProperty=nameWithType> 反序列化方法调用或引用。 如果要禁止任何反序列化 <xref:System.Runtime.Serialization.NetDataContractSerializer> 而不考虑 @no__t 属性，请禁用此规则并[CA2311](ca2311.md)，并启用规则[CA2310](ca2310.md)。
