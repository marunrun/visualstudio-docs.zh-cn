---
title: CA2312：确保在反序列化之前设置 NetDataContractSerializer
ms.date: 05/01/2019
ms.topic: reference
author: dotpaul
ms.author: paulming
...
ms.locfileid: "74809686"
---
# <a name="ca2312-ensure-netdatacontractserializerbinder-is-set-before-deserializing"></a>CA2312：确保在反序列化之前设置 NetDataContractSerializer
...
- 限制反序列化的类型。 实现自定义 <xref:System.Runtime.Serialization.SerializationBinder?displayProperty=nameWithType>。 在使用 <xref:System.Runtime.Serialization.NetDataContractSerializer>进行反序列化之前，请将 <xref:System.Runtime.Serialization.NetDataContractSerializer.Binder> 属性设置为自定义 <xref:System.Runtime.Serialization.SerializationBinder>的实例。 在重写的 <xref:System.Runtime.Serialization.SerializationBinder.BindToType%2A> 方法中，如果类型不是预期的，则引发异常以停止反序列化。
  - 确保所有代码路径都具有 <xref:System.Runtime.Serialization.NetDataContractSerializer.Binder> 属性集。
