...
在 <xref:System.Net.Http.HttpClient?displayProperty=fullName> 提供特定于平台的处理程序（或）时，如果 <xref:System.Net.Http.WinHttpHandler?displayProperty=fullName> <xref:System.Net.Http.HttpClientHandler?displayProperty=fullName> 其 `CheckCertificateRevocationList` 属性可能设置为，则 `false` 允许被吊销的证书被 <xref:System.Net.Http.HttpClient> 视为有效。
