...
1. 使用以下提示来更正警告：
   ...
   [CA1014：用 CLSCompliantAttribute @ no__t 标记程序集：将代码 `[assembly: CLSCompliant(true)]` 添加到 AssemblyInfo.cs 文件的结尾。

   [CA1032：实现标准异常构造函数 @ no__t：将构造函数 `public demo (String s) : base(s) { }` 添加到类 `demo`。

   [CA1032：实现标准异常构造函数 @ no__t：将构造函数 `public demo (String s, Exception e) : base(s, e) { }` 添加到类 `demo`。

   [CA1032：实现标准异常构造函数 @ no__t：将构造函数 `protected demo (SerializationInfo info, StreamingContext context) : base(info, context) { }` 添加到类演示。 还需要为 <xref:System.Runtime.Serialization?displayProperty=fullName> 添加 `using` 语句。

   [CA1032：实现标准异常构造函数 @ no__t：将构造函数 `public demo () : base() { }` 添加到类 `demo`。

   [CA1709：标识符应采用正确的大小写 @ no__t-0：更改命名空间 `testCode` 到 `TestCode` 的大小写。

   [CA1709：标识符应采用正确的大小写 @ no__t-0：将成员的名称更改为 `Demo`。

   [CA1709：标识符应采用正确的大小写 @ no__t-0：将成员的名称更改为 `Item`。

   [CA1710：标识符应具有正确的后缀 @ no__t-0：将类及其构造函数的名称更改为 `DemoException`。

   [CA2237：用 SerializableAttribute @ no__t 标记 ISerializable 类型-0：将 `[Serializable ()]` 特性添加到类 `demo`。

   [CA2210：程序集应具有有效的强名称 @ no__t：使用强名称密钥对 "CodeAnalysisManagedDemo" 进行签名：
