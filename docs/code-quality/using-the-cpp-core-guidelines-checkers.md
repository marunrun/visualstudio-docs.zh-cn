...
您可以使用命令行选项，通过指定 `/analyze-` 来暂时禁用文件的所有代码分析。 这会生成警告*D9025，并将 "/analyze" 替换为 "/analyze-"*，这会提醒你稍后重新启用代码分析。
...
1. **环境变量**
   ...
   - `set esp.annotationbuildlevel=ignore` 将禁用处理 SAL 注释的逻辑。 批注不会影响C++核心准则检查器中的代码分析，但其处理时间（有时是长时间）。 此设置是可选的，但强烈建议使用此设置。
