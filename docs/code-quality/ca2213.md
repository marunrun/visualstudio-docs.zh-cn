...
实现 <xref:System.IDisposable?displayProperty=fullName> 的类型将声明类型为的字段，这些类型也实现 @no__t。 该字段的 <xref:System.IDisposable.Dispose%2A> 方法不由声明类型的 <xref:System.IDisposable.Dispose%2A> 方法调用。
