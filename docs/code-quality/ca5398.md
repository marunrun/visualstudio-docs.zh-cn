...
传输层安全 (TLS) 保护计算机之间的通信，最常见的是通过超文本传输协议安全 (HTTPS) 。 协议 1.1 1.0 版本不推荐使用，而 TLS 1.2 和 TLS 1.3 是最新的。 未来，TLS 1.2 和 TLS 1.3 可能已弃用。 若要确保应用程序的安全性，请避免硬编码协议版本。 有关详细信息，请参阅 [传输层安全性 (TLS) 与 .NET Framework 的最佳实践](/dotnet/framework/network-programming/tls)。
