...
## <a name="cause"></a>原因
当使用 <xref:System.Security.SecurityCriticalAttribute> 标记的方法覆盖透明或使用 <xref:System.Security.SecuritySafeCriticalAttribute> 标记的方法时，将触发此规则。 当带有 @no__t 的透明或标记的方法重写用 <xref:System.Security.SecurityCriticalAttribute> 标记的方法时，也会触发此规则。
