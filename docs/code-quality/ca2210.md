...
- 对于 .NET Framework 2.0，请使用 `/keyfile` 或 `/keycontainer` 编译器选项[/KEYFILE （指定密钥或密钥对来为程序集签名）](/cpp/build/reference/keyfile-specify-key-or-key-pair-to-sign-an-assembly)或[/KEYCONTAINER （在中指定用于对程序集进行签名的密钥容器）](/cpp/build/reference/keycontainer-specify-a-key-container-to-sign-an-assembly) C++链接器选项。

- 对于 .NET Framework 1.0 或1.1 版，请使用 <xref:System.Reflection.AssemblyKeyFileAttribute?displayProperty=fullName> 或 <xref:System.Reflection.AssemblyKeyNameAttribute?displayProperty=fullName> 属性。
