...
在有限的情况下，添加成员是一项重大更改，即使原始成员保留其原始值也是如此。 主要是，不能从现有代码路径返回新成员，而不会中断对 `switch` `Select` [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 包含整个成员列表的返回值使用（in）语句并在默认情况下引发异常的调用方。 第二个问题是：客户端代码可能无法处理反射方法（如）的行为更改 <xref:System.Enum.IsDefined%2A?displayProperty=fullName> 。 相应地，如果新成员必须从现有方法返回，或者由于反射的使用不正确而发生已知的应用程序不兼容性，则唯一的不间断解决方案是：
