...
- 从 Visual Studio 中的规则集编辑器（需要 Visual Studio 2019 16.5 或更高版本）。 如果你的项目已使用特定规则集文件作为其 `CodeAnalysisRuleSet` ，则可以通过 Visual Studio 中的 "规则集编辑器" 将其转换为等效的 EditorConfig 文件。
...
1. 在解决方案资源管理器中，展开**引用**  >  **分析器**（或 .net Core 项目的**依赖项**  >  **分析器**）。

2. 展开包含要为其设置严重性的规则的程序集。

::: moniker range=">=vs-2019"
3. 右键单击该规则，然后选择 "**设置严重性**"。 在上下文菜单中，选择一个严重性选项。
...
::: moniker range="vs-2017"
3. 右键单击该规则，然后选择 "**设置规则集严重性**"。 在上下文菜单中，选择一个严重性选项。
...
1. 打开活动规则集文件，方法是在**解决方案资源管理器**中双击该文件，在 "**引用**分析器" 节点的右键单击菜单中选择 "**打开活动规则集**"  >  **Analyzers** ，或在项目的 "**代码分析**" 属性页上选择 "**打开**"。
...
- 从**代码编辑器**
  ...
  将光标置于带有冲突的代码行中，然后按**Ctrl** + **句点（.）** 以打开 "**快速操作**" 菜单。 选择**Suppress CAXXXX**  >  **"在源/禁止显示文件中**取消 CAXXXX"。
