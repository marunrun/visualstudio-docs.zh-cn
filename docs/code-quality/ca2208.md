...
- 将不正确的字符串参数传递到 <xref:System.ArgumentException> 的异常类型的参数化构造函数（或派生自）。

## <a name="rule-description"></a>规则说明

不是调用默认构造函数，而是调用允许提供更有意义的异常消息的构造函数重载之一。 异常消息应面向开发人员，并清楚地说明错误情况以及如何纠正或避免异常。

@No__t-0 及其派生类型的一个和两个字符串构造函数的签名与位置 `message` 和 `paramName` 参数不一致。 请确保用正确的字符串参数调用这些构造函数。 签名如下所示：
...
若要修复与此规则的冲突，请调用使用消息的构造函数和/或参数名，并确保参数对于调用的 @no__t 的类型是正确的。
...
下面的代码显示了一个构造函数，该构造函数将 @no__t 的实例错误地实例化。
